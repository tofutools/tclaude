...
| `memoryfiles` | `tclaude memory-files` — inspect/clean Claude's per-project memory markdown (`~/.claude/projects/<encoded>/memory/`, top-level `.md` only). Resolves a project dir + by default its worktree-sibling dirs (matched by encoded-prefix; `--no-siblings` to restrict). Subcmds: `ls` (sizes/mtimes), `cat` (full contents, MEMORY.md first, separator banners), `clean` (delete with `--include`/`--exclude` globs, `--dry-run`/`-y`, to-delete/to-keep preview + confirm). |
