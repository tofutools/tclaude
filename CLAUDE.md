...
| `memoryfiles` | `tclaude memory-files clean` — preview and delete Claude's per-project memory markdown (`~/.claude/projects/<encoded>/memory/`) for a project and, by default, its worktree-sibling dirs (matched by encoded-prefix). Filter with `--include`/`--exclude` globs; `--dry-run`/`-y` and a to-delete/to-keep preview + confirm prompt. |
