...
authoritative poll needs it) and a mount function whose cleanup unmounts the
component. The lifecycle claims hosts, registers state, requires cleanup, and
keeps load failures inside the primary host. Do not write another feature-local
registry or loader lifecycle.
...
call cleanup twice to prove it is idempotent. Keep feature modules behind the
dynamic loader so a missing optional asset cannot prevent the legacy dashboard
module graph from linking.
