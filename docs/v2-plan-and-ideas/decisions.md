# Discussion context and open decisions

This records discussion context, constraints and ideas, not approval of an
implementation plan. Work must be selected separately in AWB. Dates identify
discussion, not implementation commitments.

| ID | Status | Decision / question | Consequence |
|---|---|---|---|
| D1 | Discussion direction, 2026-09-09 | Refactor working main incrementally; the replacement rewrite remains stopped | Short feature branches and independently useful integration; no wholesale branch merge |
| D2 | Discussion direction, 2026-09-09 | Exploration in this folder; work items/epics in AWB | No duplicated task tracker in Markdown; update design with changed assumptions |
| D3 | Discussion direction, 2026-09-09 | Preserve operator functionality and usability; improvements may be explicit | Current main behavior is the baseline, not the reduced replacement model |
| D4 | Discussion direction, 2026-09-09 | Reusable UI dialogs/inputs and consistent small styling matter | Reuse existing framework/components and migrate actual views |
| D5 | Discussed preference; details unresolved | Agent-spawn sandbox inheritance should be identical configuration plus optional denies | Separate policy change from mechanical extraction; establish update/retry examples first |
| D6 | Behavior constraint | Operators may update sandbox profiles while agents run | Stable profile IDs/names; no operator hash workflow or save-time freeze |
