...
The separately launched app-server owns model tool execution. With Codex's
built-in sandbox, tclaude registers the exact generated profile in its native
administrator requirements catalog before launch and selects it without
redefining the profile on the command line. This requires the explicit
[native permission registry setup](codex-native-permission-registry.md).
A tclaude-level outer sandbox keeps its existing complete config-overlay path
and has no registry setup dependency. Both paths preserve filesystem, network
and Unix-socket posture—including the agentd socket floor, private-state denial,
and tmux denial—on both execution owners.
...
| **Usage, cost & context** | ⚠️ followed incrementally from Copilot's durable event log, with a byte-offset checkpoint so a daemon restart resumes rather than rescans. Output tokens advance per turn (`assistant.message`); input tokens, context occupancy and window size advance only at an authoritative disclosure — a compaction, a truncation, or a shutdown — and nothing is written between them, so a real reading is never overwritten by a zero. Agentd mirrors Copilot's authenticated remote model catalog at startup and hourly when both `copilot` and authenticated `gh` CLIs are available; send-keys uses a model's mirrored prompt limit while it is less than 24 hours old, then falls back to tclaude's static assumptions. The same authenticated CLI exposes a normalized account quota; agentd samples a finite monthly premium-request (AIC) allowance every 15 minutes for the dashboard's Groups glance and Usage history. Unlimited plans omit that percentage series. Cost is carried in the **nano-AI units Copilot emits**; no USD figure is derived, because Copilot documents an AI credit as $0.01 in a different structure and nowhere states that one AIU is one credit. Under the [API drive](#copilot-drive-send-keys-vs-api) context occupancy, the window and the token counts are read live over RPC instead, and that locally reported limit takes priority over the mirrored catalog; the durable-log *context* follower stands down while that reading is current — **cost is unchanged**, still the AIU accounting swept from Copilot's own store |
| **Drive (how tclaude talks to the agent)** | ⚠️ tmux `send-keys` by default. An **experimental, opt-in** API drive runs alongside it — `--copilot-api` on `tclaude agent spawn`, a `copilot_api` field on spawn profiles, a checkbox in the dashboard spawn modal — driving `copilot --ui-server`'s embedded JSON-RPC server. It carries message delivery, rename and compaction as typed calls, and reads status/context/usage instead of scraping; soft exit deliberately stays on keystrokes. It **requires a pre-trusted launch directory** and host loopback, and the endpoint is unauthenticated. Off unless you ask. See [below](#copilot-drive-send-keys-vs-api) |
...
- **Runtime** — `--cloud`, `--server`, `--managed-server`, `--ui-server`,
  ...
  Ask for that mode with `--copilot-api`, which is the spelling tclaude records
  and can verify the resulting listener against; a pass-through would move the
  pane's protocol behind every contract above. (`tclaude session new` accepts
  `--copilot-api-port`, but only as agentd's own handoff: the daemon allocates the
  number before forking and passes it with `--managed-launch`. A launch that is not
  agentd's is refused rather than allocating one for itself — see the refusals
  below.)
...
an injection sink, and structured reads where tclaude previously had to infer
state from a file written after the fact. tclaude runs
...
| `tclaude session new` | not a surface you ask on — the flag exists as agentd's handoff and a launch that is not agentd's is refused (TCL-1084) |
...
| **Cost, model & effort** | AIU accounting swept from Copilot's own store | **unchanged.** The sweep keeps running and keeps owning `model` and `effort_level`; no cost figure comes from the API |
| **Resume** | `copilot --resume=<id>` on the argv | `copilot --resume=<id>` on the argv **and** `session.resume` on the RPC session. Never `session.create` for a resumed launch — what create does at an id that already exists depends on whether the server currently holds that session, and a resuming caller cannot tell those cases apart in advance, so the call that is *defined* to continue a conversation is the one used. A failed resume is reported, never downgraded into a create |
...
The rule holds at spawn too. A new agent's rename and its welcome — its
identity, its group, and the pointer to the briefing waiting in its inbox — go
over the channel like everything else, and are not typed in if it is not up.
...
Three other launch-time refusals, all named rather than silent:
...
- **A launch no daemon is behind.** The channel exists only under agentd, so
  `tclaude session new --copilot-api` typed by hand is refused as
  `copilot_api_drive_needs_agentd` rather than binding an unauthenticated endpoint
  nothing will ever dial. The discriminator is `--managed-launch` together with the
  port agentd allocated; a port alone is a side effect of the daemon's involvement
  rather than a declaration of it. A **relaunch** of a conversation that chose the
  drive is treated differently, because the drive was inherited rather than asked
  for: a live managed agent is refused (its mail would HOLD rather than fall back to
  keystrokes), and anything else proceeds on send-keys with a notice, leaving the
  recorded drive untouched so relaunching through agentd picks it back up.
  `tclaude session new -r <conv> --send-keys` — the same spelling as
  `tclaude conv resume --send-keys` — proceeds past that refusal.
...
- **`tclaude agent rename`** fails; the title is not written by any other route
  either, because Copilot has an in-pane rename and so never falls back to the
  out-of-band title store.
- **`tclaude agent compact`** is refused as `control_unavailable`.
- **The live context reading** stops, so the row falls back to the durable log's
  coarser resolution.

Soft stop, hard stop and the escalation ladder are unaffected — they never used
the channel.
...
started yourself used to bind the embedded server anyway — a perfectly good
interactive Copilot session that agentd held no connection to, and an
unauthenticated loopback endpoint nothing would ever dial. Since TCL-1084 that
launch is refused instead, so this symptom now means what it says: if a pane has
the endpoint, agentd is the one that asked for it.
