...
| **Filtered network under `tclaude-layer`** ([contract](sandboxing.md#isolated-with-agentd-network-posture)) | ⚠️ Linux CIDR/ports plus DNS-to-IP host/domain leases; macOS native loopback-only lists; mixed macOS lists are NotEnforced/open; exact provider context and explicit model endpoint coverage required | ⚠️ same harness-independent gateway/native-loopback boundary and provider gate | ⚠️ same Linux packet floor for explicit-provider configs only; opaque/default/dynamic routes refuse, and the local convenience presets remain launch-refused pending TCL-826 |
