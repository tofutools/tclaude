...
| **Usage, cost & context** ([dashboard](dashboard.md)) | ✅ context %, tokens & real/what-if cost from the statusline hook | ✅ context %, tokens & what-if cost read-through from the rollout `token_count` | ✅ context %, tokens & model projected from `/event` SSE usage and seeded from message history on reconnect; blank when model metadata is unavailable. OpenCode's reported non-zero cost is real spend, while zero-cost subscription usage gets a WHAT-IF estimate from the native `/config/providers` model pricing, including reasoning, cache, and long-context tiers |
