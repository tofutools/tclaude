...
| **Usage, cost & context** ([dashboard](dashboard.md)) | ✅ context %, tokens & real/what-if cost from the statusline hook | ✅ context %, tokens & what-if cost read-through from the rollout `token_count` | ✅ context %, tokens & model projected from the `/event` SSE `message.updated` usage + `/config/providers` model limit (seeded from message history on reconnect); blank when the model limit is unavailable. Cost is OpenCode's own figure — $0 / not-applicable on a subscription, real spend on a pay-per-token key |
