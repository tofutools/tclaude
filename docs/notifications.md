...
- `*` → `idle` - the harness finished processing and no tracked sub-agent,
  background shell, or monitor is still working
...
roughly one-minute quiet period. If a sub-agent, background shell, or monitor
is still tracked then, tclaude holds the session at `main_agent_idle` instead
of emitting an Idle notification. Lost child-stop events, shell completion,
and monitors that ended on their own are reconciled by the daemon before that
state is allowed to settle to idle.
