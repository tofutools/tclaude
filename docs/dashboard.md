...
implies `--zip`. A notification that publishes a file may carry a subject and no
body at all — the artifact is then the message, and the row shows its subject
over the download card. Daemon-verified PNG, JPEG, GIF, WebP, and AVIF attachments also
