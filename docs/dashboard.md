...
| `platform-layout` | Scoped browser effects own focus traps, resize, horizontal scroll, navigation history, overlay stacking, and stable shell/dock re-homing. The dashboard profile controls are Preact-owned inside named stable hosts; the hosts move between the toolbar and dock while retaining their browser-rendered listbox triggers. `island-lifecycle.js` owns only its claimed host's load-failure alert. | Effects attach to refs/stable shell nodes and return cleanup. Closing a profile listbox restores focus to its persistent trigger after keyboard cancellation. Island failure rendering replaces only the claimed host; successful cleanup releases host ownership. | Binder lifecycle, profile-listbox focus and real-browser top-layer behavior, overlay stack, dock identity, refresh-generation, and island rollback/ownership tests cover these contracts. |
