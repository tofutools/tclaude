...
| `process-graph` | Preact owns editor/dialog state; `process-graph.js` and `process-graph-adapter.js` exclusively own their SVG/canvas-like host. | The adapter removes pointer/key listeners, connection bands, and graph instances on keyed replacement/unmount. | Graph interaction tests cover selection, drag/connect, rerender identity, and disposal. |
...
| `media-effects` | The Vegas audio player and slop/wizard cosmetic effects own media elements, particles, and the explicitly opaque reel host. They do not own operational state. | Binders/timers/audio are stopped and listeners removed on teardown; identity tokens prevent stale timers from overwriting a newer Preact host. | Reduced-motion, stale-timer, audio teardown, and opaque reel hand-back tests are required. |
| `platform-layout` | Scoped browser effects own focus traps, resize, horizontal scroll, navigation history, overlay stacking, and stable shell/dock re-homing. The static dashboard profile chips are named stable re-homed controls; their picker draft is Preact-owned under `#toolbar-profile-picker-root`. | Effects attach to refs/stable shell nodes and return cleanup. Deferred focus restore must yield to a newer dialog. | Binder lifecycle, focus race, overlay stack, dock identity, and refresh-generation tests cover these contracts. |
...
| `config-adapter` | `config-form-adapter.js`, `remote-admin.js`, and `modal-link-wt.js` are bounded adapters for server-described native controls embedded in Preact-owned config/spawn surfaces. | Activation is generation-guarded; option/list replacement is scoped to the supplied control, and the Config island owns activation cleanup. | Config activation/retry and worktree-option tests must prove stale loads cannot publish into a replacement control. |
