...
| **Usage, cost, and context** | **Native (tokens/context/cost); provider account limits blocked** | `opencode stats`, session/export APIs, and message records expose input, output, reasoning, and cache token counts plus cost and model limits. Subscription-backed test turns reported exact tokens but zero monetary cost. The newer `/api/session/{id}/context` response was empty, so use the established session/message surfaces in 1.18.4. Token counts and context-window occupancy are projected to the dashboard as of TCL-701 (#1492); cost is still open (TCL-673). The underlying provider account's usage/rate limits are **not** exposed. See [OpenCode usage, cost, and provider-limit feasibility](opencode-usage-cost-feasibility.md) for the cost/limit findings (TCL-696). |
