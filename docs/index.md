...
| `--install-default-agent-permissions` | Grants the low-risk permission slugs those skills exercise (`self.rename`, `self.compact`, `self.clone`, `self.schedule`, `self.remote-control`, `self.task`, `self.pr`, `self.tags`, `process.templates.read`) as agent defaults. Self-reincarnation needs no slug. Mutating `process.templates.manage` remains opt-in. | Using [Agent Coordination](agent.md) |
