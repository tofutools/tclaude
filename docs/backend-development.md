...
### Team wave first-turn waiting

Newly authored team waves offer first-turn waiting before dependent waves launch, matching the v1 planning beat. A fresh idle agent does not release the next wave: the runner must observe it working first, then observe every member settled together. Exited executions do not hold a gate; missing or unknown activity holds until the maximum wait. Zero maximum seconds selects the v1 eight-minute default. Waves without dependents do not wait for idle.

The deployment compiles these gates into its work graph. Runtime-owned activation evidence is persisted with the exact gate attempt and execution IDs, so reopening and explicit runtime recovery retain the first-turn observation. Public evidence submission cannot author that evidence kind. The deployment deadline includes the wave budgets instead of expiring at the former five-minute launch deadline. Existing saved waves without the first-turn flag retain their readiness/briefing behavior.

### Agent display labels

Agent display roles and descriptions are editable metadata, separate from role assignments and permissions. The roster displays and searches their literal text. Configure preserves omitted metadata for older clients and supports explicit clearing. Group copies, agent configuration copies, captured teams, and team deployment retain the labels. Offline conversion reads v1 `agent_group_members.role` and `descr`, retaining distinct labels per group; conversation head/generation metadata is not imported as a display role. Membership labels are never flattened into the ungrouped fallback. Removing a membership or disbanding its group deletes that override atomically and advances the agent revision, so stale forms and rejoining cannot resurrect it. Configure on a group roster row edits only that group, including an explicit empty label. Captures and copies use the displayed source-group labels without copying unrelated group overrides. No labels grant authority. Advisory phase recipient matching uses these display roles, as in v1, rather than requiring a permission-role assignment.
