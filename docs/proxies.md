...
tclaude proxy awb comment add awb-a3f9c1 --key agent7-findings-v1 --body-file findings.md
...
All proxy callers share the operator's AWB identity, and therefore share its
per-issue key namespace. Include an agent- or task-specific component in the
key. Reusing a key with a different body returns a conflict.
...
API; metadata appears in JSON issue detail and is omitted from summaries and
compact output.
...
- `ready_polling` — optional map keyed by a unique process name. Each entry
  ...
  when the worker can prove it reached `main` — either because the merged pull
  request it watched merged that exact branch into a trunk, or because the
  branch tip is already contained in the `main` the process monitors. An
  unproven branch is kept and the worktree directory is removed without it, so
  unmerged commits are never destroyed. An agent that has gone back to work
  since the closure is left alone, agent and worktree both. None of this
