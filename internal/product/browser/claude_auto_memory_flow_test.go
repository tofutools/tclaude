...
func TestBrowserClaudeAutoMemorySaveAndReopen(t *testing.T) {
	...
	page.MustElement("#editor [name=name]").MustInput("Claude auto-memory")
	...
	for _, mode := range []bool{true, false} {
		page.MustElement("#editor [name=auto_memory]").MustSelect(map[bool]string{true: "On", false: "Off"}[mode])
		...
		require.Equal(t, mode, *saved.Revision.Options.AutoMemory)
		page.MustElementR("#configuration-list button", "^Edit configuration$").MustClick()
		page.MustWait(`()=>document.querySelector('#editor').open`)
		require.Equal(t, map[bool]string{true: "on", false: "off"}[mode], page.MustElement("#editor [name=auto_memory]").MustProperty("value").Str())
	}
