...
func TestBrowserProfileStartupUsesPinnedSuggestionsBeforeExplicitLaunch(t *testing.T) {
	...
	page.MustElement("#editor [name=startup_role]").MustInput("writer")
	page.MustElement("#editor [name=startup_description]").MustInput("Drafts documentation")
	...
	require.Equal(t, "writer", page.MustElement("#editor [name=role_label]").MustProperty("value").Str())
	...
	require.Equal(t, model.AgentLabels{Description: "Drafts documentation"}, snapshot.Agents[0].Labels)
