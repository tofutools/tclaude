...
function ProfileImport({profiles,actions,close}){
 ...
 const inspect=async()=>{setError('');setBusy('inspect');try{if(new TextEncoder().encode(raw).length>limit)throw Error('Bundle exceeds 1 MiB');const parsed=JSON.parse(raw),found=await actions.inspectProfiles(parsed);if(!mounted.current)return;setPreview(found);const initial={};for(const row of found.profiles)initial[row.key]={include:true,action:'create',name:`${row.name}-copy`,target:'',aliases:(row.aliases||[]).join('\n')};setDecisions(initial)}catch(e){if(mounted.current)setError(message(e))}finally{if(mounted.current)setBusy('')}};
