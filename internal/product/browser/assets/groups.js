...
function renderGroupControls(snapshot,{host,el,button,edit,api,refresh,presentation,attach,displayLabelFields}) {
 ...
 controls.append(button('Launch defaults',async()=>{
  ...
  document.getElementById('editor-fields').append(el('p',(saved.Revision.Options?.Harness||saved.Revision.Desired.Harness||'Inherited harness')+' · '+(saved.Revision.Options?.Model||saved.Revision.Desired.Model||'Inherited model')+' · '+(saved.Revision.Options?.WorkingDirectory||saved.Revision.Desired.WorkingDirectory||'Supply a working directory at launch')+' — creates the agent and membership; start remains explicit.'));
 }));
