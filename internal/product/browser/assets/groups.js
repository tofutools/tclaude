...
function renderGroupControls(snapshot,{host,el,button,edit,api,refresh,presentation,attach,displayLabelFields}) {
 ...
 }),...[false,true].map(start=>button(start?'Spawn member from default':'Create member from default',async()=>{
  const current=await api('/v2/groups/'+encodeURIComponent(group.ID)+'/configuration');if(!card.isConnected)return;
  const selected=current.Profile||(await api('/v2/configuration-defaults')).Global;
  const saved=selected?await api('/v2/configuration-profiles/'+encodeURIComponent(selected.ProfileID)):{Profile:{Name:'Provider defaults'},Revision:{Options:{},Desired:{}}};if(!card.isConnected)return;
  edit(start?'Spawn group member':'Create group member',[{name:'environment',label:'Explicit environment overrides',environment:true,value:{},inherited:{...current.Environment,...(saved.Revision.Options||saved.Revision.Desired).Environment}},{name:'name',label:'Agent name · '+ProfilePresentation.label(saved.Profile),value:saved.Revision.Startup?.AgentName||saved.Profile.Name},...displayLabelFields(saved.Revision.Startup),...(start?initialBriefFields(saved.Revision.Startup):[]),...profileLaunchFields(saved.Revision).filter(field=>field.name!=='environment')],async f=>{const result=await api('/v2/groups/'+encodeURIComponent(group.ID)+'/agents',{request_id:f.requestID,id:f.requestID,name:f.name,...(start?{launch:{initial_message:initialBriefBody(f)}}:{}),labels:{Role:f.role_label,Description:f.description},environment:f.environment,...profileLaunchOverrides({...f,environment:undefined},saved.Revision),expected_group_revision:group.Revision,expected_default_revision:current.Revision});if(result.Operation&&['failed','uncertain'].includes(result.Operation.state)){await refresh();throw new Error('The member was created, but launch is '+result.Operation.state+'. Inspect operation '+result.Operation.id+' before starting more work.')}return result});
  document.getElementById('editor-fields').append(el('p',(saved.Revision.Options?.Harness||saved.Revision.Desired.Harness||'Inherited harness')+' · '+(saved.Revision.Options?.Model||saved.Revision.Desired.Model||'Inherited model')+' · '+(saved.Revision.Options?.WorkingDirectory||saved.Revision.Desired.WorkingDirectory||'Supply a working directory at launch')+(start?' — creates the member and starts its first execution. Review the suggested context and brief before submitting.':' — creates the agent and membership; start remains explicit.')));
 })));
