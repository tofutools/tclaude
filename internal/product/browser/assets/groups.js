...
function renderGroupControls(snapshot,{host,el,button,edit,api,refresh,presentation,attach,displayLabelFields}) {
 ...
 controls.append(button('Clone group',async()=>{
  ...
  const available=ref=>!ref||profiles.some(p=>p.ID===ref.ProfileID&&!p.Archived),blocked=active.filter(a=>!available(a.ConfigurationProfile)),canCopy=!blocked.length&&members.every(Boolean),canDefault=available(current.Profile)&&(current.Profile||current.DefaultDirectory||Object.keys(current.Environment||{}).length);
  edit('Clone group',[
   ...
   {name:'defaults',label:'Group launch default',value:'none',options:[{value:'none',label:'No default'},...(canDefault?[{value:'copy',label:current.Profile?'Copy selected configuration '+(profiles.find(p=>p.ID===current.Profile.ProfileID)?.Name||current.Profile.ProfileID)+' and group environment/directory':'Copy group environment and directory'}]:[])]},
 ...
 controls.append(button('Launch defaults',async()=>{
  ...
  if(!card.isConnected)return;
  const choices=profiles.filter(p=>!p.Archived).map(p=>({key:'current:'+p.ID,label:ProfilePresentation.label(p)+' · '+p.ID,ref:{ProfileID:p.ID,RevisionID:p.CurrentRevisionID}}));let value='';
  ...
  edit('Group launch defaults',[{name:'cwd',label:'Default directory (blank uses launch defaults)',required:false,value:current.DefaultDirectory||''},{name:'environment',label:'Group environment (saved configuration and explicit member values override matching names)',environment:true,value:current.Environment||{}},{name:'profile',label:'Saved configuration for new members (existing agents keep their settings)',required:false,value,options:[{value:'',label:'No group default'},...choices.map(p=>({value:p.key,label:p.label}))]}],async f=>{let profile=null;if(f.profile){const selected=choices.find(p=>p.key===f.profile);if(!selected)throw new Error('Select a listed configuration');profile=(await api('/v2/configuration-profiles/'+encodeURIComponent(selected.ref.ProfileID)+'?revision_id='+encodeURIComponent(selected.ref.RevisionID))).Revision.Ref}return api('/v2/groups/'+encodeURIComponent(group.ID)+'/configuration',{profile,default_directory:f.cwd,environment:f.environment,expected_revision:current.Revision},'PUT')},{skipUnchanged:true});
 }),...[false,true].map(start=>button(start?'Spawn member':'Create member',async()=>{
  const current=await api('/v2/groups/'+encodeURIComponent(group.ID)+'/configuration');if(!card.isConnected)return;
  const globalDefaults=await api('/v2/configuration-defaults'),empty={Profile:{Name:'Provider defaults'},Revision:{Options:{},Desired:{}}};
  const globalProfile=globalDefaults.Global?await api('/v2/configuration-profiles/'+encodeURIComponent(globalDefaults.Global.ProfileID)):empty;
  const fallback=current.Profile?await api('/v2/configuration-profiles/'+encodeURIComponent(current.Profile.ProfileID)):globalProfile;
  const profiles=(await api('/v2/configuration-profiles')).filter(profile=>!profile.Archived);if(!card.isConnected)return;
  let saved=fallback,loading=false,loadedProfile='',selectionGeneration=0;
  const suggestions=profile=>{const out={};for(const tier of [globalProfile,fallback,profile])for(const [key,value] of Object.entries(tier.Revision.Startup||{}))if(value)out[key]=value;return out};
  const initialStartup=suggestions(saved);
  const environmentField={name:'environment',label:'Explicit environment overrides',environment:true,value:{},inherited:{...current.Environment,...(globalProfile.Revision.Options||globalProfile.Revision.Desired).Environment,...(saved.Revision.Options||saved.Revision.Desired).Environment}};
  const checkout=new WorkspaceChoice(snapshot.workspaces||[],api);
  const profileField={name:'profile',label:'Launch profile',required:false,value:'',options:[{value:'',label:'Use group / global / provider defaults'},...profiles.map(profile=>({value:profile.ID,label:ProfilePresentation.label(profile)}))]};
  edit(start?'Spawn group member':'Create group member',[profileField,...checkout.fields(),environmentField,{name:'name',label:'Agent name',value:initialStartup.AgentName||saved.Profile.Name},...displayLabelFields(initialStartup),...(start?initialBriefFields(initialStartup):[]),...profileLaunchFields(saved.Revision).filter(field=>field.name!=='environment')],async f=>{if(loading||f.profile!==loadedProfile)throw new Error('Wait for the selected profile to load successfully.');const overrides=profileLaunchOverrides({...f,environment:undefined},saved.Revision),workspace=await checkout.read(f);const result=await api('/v2/groups/'+encodeURIComponent(group.ID)+'/agents',{request_id:f.requestID,id:f.requestID,name:f.name,...(workspace?{workspace}:{}),...(f.profile?{profile_id:f.profile}:{}),...(start?{launch:{initial_message:initialBriefBody(f)}}:{}),labels:{Role:f.role_label,Description:f.description},environment:f.environment,...overrides,expected_group_revision:group.Revision,expected_default_revision:current.Revision});if(result.Operation&&['failed','uncertain'].includes(result.Operation.state)){await refresh();throw new Error('The member was created, but launch is '+result.Operation.state+'. Inspect operation '+result.Operation.id+' before starting more work.')}return result});
  const form=document.getElementById('editor-form'),fields=document.getElementById('editor-fields'),hint=el('p'),touched=new Set();fields.append(hint);checkout.attach(form);
  const describe=()=>{const desired=saved.Revision.Options||saved.Revision.Desired;hint.textContent=ProfilePresentation.label(saved.Profile)+' · '+(desired.Harness||'Inherited harness')+' · '+(desired.Model||'Inherited model')+' · '+(current.DefaultDirectory||desired.WorkingDirectory||'Daemon working directory')+(start?' — creates the member and starts its first execution. Review the suggested context and brief before submitting.':' — creates the agent and membership; start remains explicit.')};describe();
  for(const input of fields.querySelectorAll('[name]'))input.addEventListener('input',()=>touched.add(input.name));
  form.elements.profile.addEventListener('change',async()=>{
   const generation=++selectionGeneration,id=form.elements.profile.value;loading=true;hint.textContent='Loading profile…';
   try{
    const next=id?await api('/v2/configuration-profiles/'+encodeURIComponent(id)):fallback;
    if(generation!==selectionGeneration||!hint.isConnected)return;
    saved=next;loadedProfile=id;const startup=suggestions(saved);
    for(const [name,value] of Object.entries({name:startup.AgentName||saved.Profile.Name,role_label:startup.Role||'',description:startup.Description||'',context:startup.Context||'',brief:startup.InitialMessage||''}))if(form.elements[name]&&!touched.has(name))form.elements[name].value=value;
    environmentField.control.inherited={...current.Environment,...(globalProfile.Revision.Options||globalProfile.Revision.Desired).Environment,...(fallback.Revision.Options||fallback.Revision.Desired).Environment,...(saved.Revision.Options||saved.Revision.Desired).Environment};environmentField.control.preview();describe();
   }catch(error){if(generation===selectionGeneration&&hint.isConnected){hint.textContent='Selected profile unavailable';showError(error)}}finally{if(generation===selectionGeneration)loading=false}
  });
 })));
 ...
 for(const [label,delta] of [['Move group earlier',-1],['Move group later',1]]){const move=button(label,async()=>{const ids=groups.map(g=>g.ID),a=ids.indexOf(group.ID),b=ids.indexOf(siblings[position+delta].ID);[ids[a],ids[b]]=[ids[b],ids[a]];await presentation.change({GroupOrder:ids})});move.disabled=!presentation?.loaded||position+delta<0||position+delta>=siblings.length;controls.append(move)}
  controls.append(button('Edit name and members',()=>{
   ...
   edit('Group owners and shared launch limits',[
    ...
    autoReviewAuthorityField(prior),
