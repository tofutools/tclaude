...
function renderGroupControls(snapshot,{host,el,button,edit,api,refresh,presentation,attach,displayLabelFields}) {
 ...
 controls.append(button('Launch defaults',async()=>{
  ...
  edit(start?'Spawn group member':'Create group member',[profileField,environmentField,{name:'name',label:'Agent name',value:initialStartup.AgentName||saved.Profile.Name},...displayLabelFields(initialStartup),...(start?initialBriefFields(initialStartup):[]),...profileLaunchFields(saved.Revision).filter(field=>field.name!=='environment')],async f=>{if(loading||f.profile!==loadedProfile)throw new Error('Wait for the selected profile to load successfully.');const result=await api('/v2/groups/'+encodeURIComponent(group.ID)+'/agents',{request_id:f.requestID,id:f.requestID,name:f.name,...(f.profile?{profile_id:f.profile}:{}),...(start?{launch:{initial_message:initialBriefBody(f)}}:{}),labels:{Role:f.role_label,Description:f.description},environment:f.environment,...profileLaunchOverrides({...f,environment:undefined},saved.Revision),expected_group_revision:group.Revision,expected_default_revision:current.Revision});if(result.Operation&&['failed','uncertain'].includes(result.Operation.state)){await refresh();throw new Error('The member was created, but launch is '+result.Operation.state+'. Inspect operation '+result.Operation.id+' before starting more work.')}return result});
  const form=document.getElementById('editor-form'),fields=document.getElementById('editor-fields'),hint=el('p'),touched=new Set();fields.append(hint);
