...
(async()=>{
 const fragment=new URLSearchParams(location.hash.slice(1));const token=fragment.get('login');history.replaceState(null,'',location.pathname+location.search);
 if(token)await api('/session',{token});await presentation.load();await refresh();await selectTab('groups');
