...
class RosterWorkspace {
  ...
  draw(){if(!this.snapshot)return;const{el,button}=this,agents=this.snapshot.agents||[],groups=this.groups||[],grouped=new Set(groups.flatMap(g=>g.Members||[]));
    ...
    if(!group)for(const id of [...keep]){let parent=byID.get(id)?.ParentGroupID;const seen=new Set([id]);while(parent&&byID.has(parent)&&!seen.has(parent)){seen.add(parent);keep.add(parent);parent=byID.get(parent).ParentGroupID}}
    for(const g of all){if(!keep.has(g.ID))continue;const card=el('div',undefined,'group');card.dataset.groupId=g.ID;card.append(el('h2',g.Name));if(g.ID!=='__ungrouped')card.append(el('code',g.ID));
      for(const a of this.visible.filter(a=>(g.Members||[]).includes(a.ID))){const row=this.row(a),check=el('input');check.type='checkbox';check.checked=this.selected.has(a.ID);check.disabled=this.busy;check.setAttribute('aria-label','Select '+a.Name);check.onchange=()=>{check.checked?this.selected.add(a.ID):this.selected.delete(a.ID);this.draw()};row.prepend(check);card.append(row)}cards.set(g.ID,card);
    }
    for(const g of all){const card=cards.get(g.ID);if(!card)continue;const parent=!group&&cards.get(g.ParentGroupID);if(parent&&parent!==card){let children=parent.querySelector(':scope > .roster-children');if(!children){children=el('div',undefined,'roster-children');children.style.marginInlineStart='1rem';parent.append(children)}children.append(card)}else this.list.append(card)}
