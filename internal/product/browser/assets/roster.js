...
class RosterWorkspace {
  ...
  draw(){if(!this.snapshot)return;const{el,button}=this,agents=this.snapshot.agents||[],groups=this.groups||[],grouped=new Set(groups.flatMap(g=>g.Members||[]));
    ...
    for(const g of all){if(!keep.has(g.ID))continue;const card=el('div',undefined,'group');card.dataset.groupId=g.ID;card.append(el('h2',g.Name));appendGroupDetails(card,g.Details||{},el);if(g.ID!=='__ungrouped')card.append(el('code',g.ID));
      for(const a of (group&&g.ID!==group?[]:this.visible.filter(a=>(g.Members||[]).includes(a.ID)))){const row=this.row(a,g.ID),check=el('input');check.type='checkbox';check.checked=this.selected.has(a.ID);check.disabled=this.busy;check.setAttribute('aria-label','Select '+a.Name);check.onchange=()=>{check.checked?this.selected.add(a.ID):this.selected.delete(a.ID);this.draw()};row.prepend(check);card.append(row)}cards.set(g.ID,card);
    }
