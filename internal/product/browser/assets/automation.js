...
export function automationWorkspace({api, el, button, edit, getSnapshot, openWork}) {
  ...
  async function render(target = parent) {
    ...
    for (const result of records) {
      ...
      card.append(el('p', `Action: ${revision.Action.Kind} · ${revision.Delegation.NoExpiry ? "authority has no expiry" : "authority expires " + new Date(revision.Delegation.ExpiresAt).toLocaleString()}`));
  ...
  async function open(result, kind, selected, authority) {
    ...
    if (d.action.Kind === 'message') {
      ...
      fields.push(field('subject', 'Message subject (optional)', m.Subject), field('body', 'Message or guidance', m.Body, {required: true, multiline: true}), field('recipients', 'Explicit agent recipients', m.AgentIDs || [], {multiple: true, options: agents.map(a => choice(a.ID, a.Name))}), field('group', 'Recipient group', m.GroupID || '', {options: [choice('', 'No group restriction'), ...groups.map(g => choice(g.ID, g.Name))]}), field('role_label', 'Recipient display role (within selected group)', m.RoleLabel || ''), ...(m.RoleID?[field('role', 'Existing permission-role filter', m.RoleID, {options: [choice('', 'Remove permission-role filter'), ...(authority.Roles || []).map(r => choice(r.ID, r.Name))]})]:[]));
      buildAction = f => {if(f.role_label&&(!f.group||f.role||f.recipients.length))throw new Error('A display role requires one group without explicit recipients or a permission-role filter.');return {Kind: 'message', Message: {...m, Subject:f.subject, Body: f.body, AgentIDs: f.recipients, GroupID: f.group, RoleID: f.role||'',RoleLabel:f.role_label}};};
    } else {
    ...
    edit(result ? 'Edit automation rule' : 'Create ' + kind.replaceAll('_', ' '), fields, async f => {
      ...
      const expiry = new Date(f.authority_expiry); if (f.authority_lifetime !== 'no_expiry' && !Number.isFinite(expiry.getTime())) throw new Error('Invalid authority expiry.');
