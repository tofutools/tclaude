...
export function automationWorkspace({api, el, button, edit, getSnapshot, openWork}) {
  ...
  async function open(result, kind, selected, authority) {
    ...
    const fields = [field('name', 'Rule name', d.name, {required: true}), field('enabled', 'Rule state', d.enabled ? 'enabled' : 'disabled', {options: ['disabled', 'enabled']})];
    if (kind === 'schedule') {
