...
class TeamEditor {
  ...
  member(original) {
    ...
    const overrideProperties = {harness:"Harness",model:"Model",effort:"Effort",tool_governance:"ToolGovernance",fast_mode:"FastMode",auto_review:"AutoReview",auto_memory:"AutoMemory",approval:"Approval",sandbox:"Sandbox"};
    const fields = [
 ...
 {key:'fast_mode',label:'Codex fast mode',options:launchFastModeChoices().map(v=>opt(v.value,v.label)),value:desired.FastMode||'',required:false},
    ...
    const form = this.form('Member', fields, f => {
      ...
      const overrides = {};
      if(f.profile)for(const [key,property] of Object.entries(overrideProperties))if(f['override_'+key])overrides[property]=(key==='auto_review'||key==='auto_memory')?f[key]==='on':f[key];
    ...
    select.onchange = () => {
      ...
      for (const [key, property] of Object.entries({harness: 'Harness', model: 'Model', effort: 'Effort',tool_governance:'ToolGovernance',fast_mode:'FastMode',auto_review:'AutoReview',auto_memory:'AutoMemory', cwd: 'WorkingDirectory', approval: 'Approval', sandbox: 'Sandbox'})) form.elements[key].value = launchSettingValue(key,d[property]);
 ...
 const syncFast=attachFastModeControl(form,()=>!form.elements.profile.value||form.elements.override_fast_mode.checked);
    ...
    const updateProfile = (initialize=false) => {
      ...
      if (profile) {
        ...
        if(d.Harness&&form.elements.harness.value!==d.Harness)for(const key of ['model','effort','auto_review','auto_memory','fast_mode','tool_governance'])if(!form.elements['override_'+key].checked)form.elements[key].value='';
        showEnvironment(d.Environment); showSandbox(d.HostSandbox);
        form.elements.harness.dispatchEvent(new Event('change', {bubbles:true}));
      }
      for (const key of ['harness','model','effort','auto_review','auto_memory','fast_mode','tool_governance','cwd','approval','sandbox']) form.elements[key].disabled = selected && !form.elements['override_'+key]?.checked;
      ...
      syncTools();syncFast();syncReview();syncMemory();
