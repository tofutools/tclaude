...
function attachLaunchSupportPreview({host,api}) {
 ...
 const update=async()=>{
  ...
  try{
   ...
   status.textContent=selected+' adapter supports approval: '+(approvals.join(', ')||'none')+'; confinement: '+(sandboxes.join(', ')||'none')+'. '+(problems.length?'Unsupported '+problems.join(' and ')+'. Change these settings before launch. ':(hasHostSandbox?'Selected native approval and confinement are supported by the adapter. ':'Selected policy is supported by the adapter. '))+hostSupport+defaultSupport+(support.PreparedInitialInput?'Prepared initial input is supported. ':'Prepared initial input is not declared. ')+support.Basis;
