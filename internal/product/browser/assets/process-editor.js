    ...
    if (node.Kind === 'task') {
      ...
      if (performer.Kind === 'agent') {
        ...
        fields.push({name:'operator', label:'Operator performs this task', type:'checkbox', value:performer.Human.Operator}, {name: 'agent', label: 'Human task recipient agent', options: [option('', 'Use role instead'), ...this.agents.map(a => option(a.ID, a.Name))], value: performer.Human.AgentID || ''},
          ...
          {name: 'ask', label: 'Human task question', multiline: true, value: performer.Human.Ask || ''},
          {name: 'prompt', label: 'Human task context', multiline: true, value: performer.Human.Prompt},
        ...
        changes.push((n, f) => {
          if(!f.ask.trim() && !f.prompt.trim()) throw new Error('Provide a human task question or context.');
          ...
          if(f.ask) n.Performer.Human.Ask=f.ask; else delete n.Performer.Human.Ask;
