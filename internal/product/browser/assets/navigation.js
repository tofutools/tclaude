...
class WorkspaceNavigation {
 constructor({select,report}){
  ...
  window.addEventListener('popstate',()=>{this.select(this.initialTab(false)).catch(report)});
  document.addEventListener('keydown',event=>{
   ...
   if(event.altKey&&!event.ctrlKey&&!event.metaKey&&!event.shiftKey&&/^[1-9]$/.test(event.key)&&!event.target.closest('input,textarea,select,[contenteditable=true]')){const tab=this.tabs()[Number(event.key)-1];if(tab){event.preventDefault();tab.click()}}
  });
 }
 ...
 record(tab,replace=false){if(!this.tabs().some(t=>t.dataset.tab===tab))return;try{sessionStorage.setItem('workspace-tab-v1',tab)}catch{};const url=new URL(location.href);if(url.searchParams.get('tab')===tab)return;url.searchParams.set('tab',tab);history[replace?'replaceState':'pushState'](history.state,'',url)}
 open(){if(document.querySelector('dialog[open]')||document.querySelector('main').inert)return;this.previousFocus=document.activeElement;this.input.value='';this.draw();this.dialog.showModal();this.input.focus()}
