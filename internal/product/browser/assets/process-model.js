...
export function validationMessages(draft) {
  ...
  for (const node of graph.Nodes) {
    ...
    if(node.Kind==='start' && (node.ID!==graph.EntryNodeID || (incoming.get(node.ID)||0)!==0 || (outgoing.get(node.ID)||[]).length!==1 || (node.RoutingMode!=='single-route-v1' && (outgoing.get(node.ID)||[]).some(e=>e.Verdict)) || graph.Nodes.filter(n=>n.Kind==='start').length>1))messages.push('Use one start node as the entry, with no incoming and exactly one outgoing route. Native start routes must be unlabelled.');
