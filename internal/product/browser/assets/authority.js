...
class AuthorityWorkspace {
 ...
 boundsText(b={}){return ['Harnesses','Models','WorkingDirectoryRoots','ApprovalModes','SandboxModes'].every(k=>b[k]?.length)?`Configuration limits: ${(b.Harnesses||[]).join(', ')} / ${(b.Models||[]).join(', ')} / ${(b.WorkingDirectoryRoots||[]).join(', ')} / ${(b.ApprovalModes||[]).join(', ')} / ${(b.SandboxModes||[]).join(', ')} / automatic approval review: ${b.AutoReview?'allowed':'not allowed'} / environments: ${JSON.stringify(b.Environments||[])} / host sandbox: ${b.HostSandboxProfiles?.length?b.HostSandboxProfiles.join(', '):'only without a selected host policy'}`:'No configuration-bearing authority (incomplete or empty bounds).'}
 ...
 editRole(role){this.edit(role?'Edit role actions for every assignment':'Create role',[
  {name:'name',label:'Role name',value:role?.Name||''},{name:'description',label:'Description',value:role?.Description||'',required:false},{name:'brief',label:'Startup role brief (up to 16 KiB)',multiline:true,value:role?.Brief||'',required:false},{name:'actions',label:'Allowed actions',required:false,multiple:true,value:role?.Actions||[],options:AUTHORITY_ACTIONS}
 ],async f=>{await this.api('/v2/authority/roles/'+encodeURIComponent(role?.ID||f.requestID),{name:f.name,description:f.description,brief:f.brief,actions:f.actions,expected_revision:role?.Revision||0},'PUT');await this.render()},{skipUnchanged:!!role})}
