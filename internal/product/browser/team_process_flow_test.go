...
func TestBrowserTeamProcessShowsGuidanceAndRecordsNamedMoves(t *testing.T) {
	...
	team := model.TeamDefinition{WorkspacePolicy: model.WorkspacePolicyShared, Members: []model.TeamMemberSpec{{Key: "worker", Name: "Worker", Desired: desired, Required: true}}, Waves: []model.TeamWave{{ID: "initial", MemberKeys: []string{"worker"}, RequiredReady: true}}, AdvisoryProcess: []model.TeamPhase{{Name: "Investigate", Roles: []string{"all"}, Criteria: "Record <evidence> literally."}, {Name: "Review", Roles: []string{"all"}, Criteria: "Discuss the findings."}}}
