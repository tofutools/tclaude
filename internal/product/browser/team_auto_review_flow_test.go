...
func TestBrowserTeamAutoReviewOverrideAndProfileCopy(t *testing.T) {
	ctx, page, operator := processEditorBrowser(t, &codex.Provider{})
	desired := model.DesiredConfiguration{Harness: "codex", Model: "fixture", WorkingDirectory: "/tmp", Approval: model.ApprovalOnRequest, Sandbox: model.SandboxUnconfined, AutoReview: true}
	...
	require.Equal(t, "on", page.MustElement("#team-editor [name=auto_review]").MustProperty("value").Str())
	require.True(t, page.MustElement("#team-editor [name=auto_review]").MustProperty("disabled").Bool())
	page.MustElement("#team-editor [name=override_auto_review]").MustClick()
	page.MustElement("#team-editor [name=auto_review]").MustSelect("No automatic-review override")
	...
	require.Equal(t, false, *saved.Revision.Team.Members[0].Overrides.AutoReview)
	...
	require.Equal(t, "", page.MustElement("#team-editor [name=auto_review]").MustProperty("value").Str())
	page.MustElement("#team-editor [name=override_auto_review]").MustClick()
	require.Equal(t, "on", page.MustElement("#team-editor [name=auto_review]").MustProperty("value").Str())
	...
	require.Equal(t, true, saved.Revision.Team.Members[0].Desired.AutoReview)
	page.MustElementR("#team-editor button", "^Edit Worker$").MustClick()
	...
	saved = app.DefinitionResult{}
	require.NoError(t, operator.Call(ctx, "GET", "/v2/definitions/"+string(definitions[0].ID), nil, &saved))
	require.Empty(t, saved.Revision.Team.Members[0].Desired.AutoReview)
