...
func TestBrowserTerminalSplitAndReloadRetainExactDisconnectedPanes(t *testing.T) {
	...
	require.False(t, page.MustEval(`() => {const event=new KeyboardEvent('keydown',{code:'KeyK',key:'k',ctrlKey:true,bubbles:true,cancelable:true});document.querySelector('.xterm textarea').dispatchEvent(event);return event.defaultPrevented||document.querySelector('#command-picker').open}`).Bool())
	...
	require.Equal(t, 1, child.MustEval(`() => history.length`).Int())
	require.False(t, child.MustEval(`() => new URL(location.href).searchParams.has('tab')`).Bool())
	child.MustEval(`async () => {history.back();await new Promise(resolve=>setTimeout(resolve,30));dispatchEvent(new PopStateEvent('popstate'))}`)
	require.False(t, child.MustElement("#terminals").MustProperty("hidden").Bool())
	require.True(t, child.MustElement("#terminal-dashboard-link").MustVisible())
