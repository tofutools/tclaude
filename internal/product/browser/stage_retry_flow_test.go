...
func TestBrowserIndependentStageRetryAndModeReopen(t *testing.T) {
	...
	page.MustElement("#process-inspector [name=attempts]").MustSelectAllText().MustInput("9007199254740993")
	...
	page.MustElementR("#process-inspector button", "^Edit check 1$").MustClick()
	require.Equal(t, "9007199254740993", page.MustElement("#process-inspector [name=attempts]").MustProperty("value").Str())
	...
	for _, node := range saved.Revision.Process.Graph.Nodes {
		if node.Stages != nil {
			found = true
			require.Equal(t, model.RetryAttempts(9007199254740993), node.Stages.Checks[0].Retry.MaxAttempts)
