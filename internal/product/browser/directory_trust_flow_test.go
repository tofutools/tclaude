...
func TestBrowserDirectoryTrustSaveAndReopen(t *testing.T) {
	...
	page.MustElement("#editor [name=name]").MustInput("Directory trust")
	...
	for _, mode := range []bool{true, false} {
		page.MustElement("#editor [name=trust_directory]").MustSelect(map[bool]string{true: "On", false: "Off"}[mode])
		...
		require.Equal(t, mode, *saved.Revision.Options.TrustDirectory)
		page.MustElementR("#configuration-list button", "^Edit configuration$").MustClick()
		page.MustWait(`()=>document.querySelector('#editor').open`)
		require.Equal(t, map[bool]string{true: "on", false: "off"}[mode], page.MustElement("#editor [name=trust_directory]").MustProperty("value").Str())
	}
