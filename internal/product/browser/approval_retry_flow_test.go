...
func TestBrowserPlanApprovalRetryAuthorsReopensAndClears(t *testing.T) {
	...
	page.MustElement("#process-inspector [name=approval_attempts]").MustInput("3")
	...
	page.MustElementR("#process-inspector button", "^Edit plan$").MustClick()
	require.Equal(t, "3", page.MustElement("#process-inspector [name=approval_attempts]").MustProperty("value").Str())
