...
func TestBrowserLaunchSupportExplainsPoliciesWithoutChangingOfflineSettings(t *testing.T) {
	...
	page.MustElementR("#team-editor [aria-label='Configured launch support']", "codex: no provider is configured")
	page.MustEval(`()=>window.releaseSupport()`)
	page.MustWait(`()=>window.supportDelivered`)
	page.MustWait(`()=>document.querySelector('#team-editor [aria-label="Configured launch support"]').textContent.startsWith('codex: no provider')`)
	require.NoError(t, operator.Call(ctx, "GET", "/v2/snapshot", nil, &snapshot))
	require.Empty(t, snapshot.Executions)
}
