...
func TestBrowserTeamAutoMemoryOverrideAndProfileCopy(t *testing.T) {
	ctx, page, operator := processEditorBrowser(t, &claude.Provider{})
	desired := model.DesiredConfiguration{Harness: "claude", Model: "fixture", WorkingDirectory: "/tmp", Approval: model.ApprovalManual, Sandbox: model.SandboxWorkspaceWrite, AutoMemory: true}
	require.NoError(t, operator.Call(ctx, "POST", "/v2/configuration-profiles", map[string]any{"request_id": "fast_profile", "id": "worker", "revision_id": "one", "name": "Memory profile", "desired": desired}, nil))
	...
	page.MustElement("#team-editor [name=profile]").MustSelect("Memory profile")
	require.Equal(t, "on", page.MustElement("#team-editor [name=auto_memory]").MustProperty("value").Str())
	require.True(t, page.MustElement("#team-editor [name=auto_memory]").MustProperty("disabled").Bool())
	page.MustElement("#team-editor [name=override_auto_memory]").MustClick()
	page.MustElement("#team-editor [name=auto_memory]").MustSelect("Off")
	...
	require.Equal(t, false, *saved.Revision.Team.Members[0].Overrides.AutoMemory)
	...
	require.Equal(t, "off", page.MustElement("#team-editor [name=auto_memory]").MustProperty("value").Str())
	page.MustElement("#team-editor [name=override_auto_memory]").MustClick()
	require.Equal(t, "on", page.MustElement("#team-editor [name=auto_memory]").MustProperty("value").Str())
	page.MustElement("#team-editor [name=profile]").MustSelect("Custom settings")
	page.MustElement("#team-editor [aria-label='Copy saved configuration']").MustSelect("Memory profile · one")
	require.Equal(t, "on", page.MustElement("#team-editor [name=auto_memory]").MustProperty("value").Str())
	page.MustElementR("#team-editor button", "^Apply changes$").MustClick()
	page.MustElementR("#team-editor button", "^Save team revision$").MustClick()
	...
	require.Equal(t, true, saved.Revision.Team.Members[0].Desired.AutoMemory)
	page.MustElementR("#team-editor button", "^Edit Worker$").MustClick()
	...
	saved = app.DefinitionResult{}
	require.NoError(t, operator.Call(ctx, "GET", "/v2/definitions/"+string(definitions[0].ID), nil, &saved))
	require.Empty(t, saved.Revision.Team.Members[0].Desired.AutoMemory)
