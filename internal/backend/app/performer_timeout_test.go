...
func TestPerformerTimeoutQueueRestartAndFreshRetryDeadline(t *testing.T) {
	for _, retry := range []bool{false, true} {
		t.Run(map[bool]string{false: "queued", true: "retry"}[retry], func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, initial.Add(2*time.Second), admitted.Run.NodeAttempts[0].Deadline)
			if retry {
			...
			if retry {
				...
				for _, attempt := range read.Run.NodeAttempts {
					if attempt.Ref.NodeID == "program" && attempt.Ref.Attempt == 2 {
						found = true
						require.Equal(t, initial.Add(12*time.Second), attempt.Deadline)
					}
				}
				...
				require.Equal(t, initial.Add(2*time.Second), read.Run.NodeAttempts[0].Deadline)
			}
		})
	}
}
