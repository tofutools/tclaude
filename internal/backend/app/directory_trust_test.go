...
func TestDirectoryTrustPersistsOnAgentAndAdmittedExecution(t *testing.T) {
	...
	desired := model.DesiredConfiguration{Harness: "claude", Model: "first", TrustDirectory: true, WorkingDirectory: t.TempDir(), Approval: model.ApprovalSupervised, Sandbox: model.SandboxUnconfined}
	...
	require.Equal(t, false, durableAgent.Desired.TrustDirectory)
	execution, err := store.Execution(ctx, launched.Execution.ID)
	require.NoError(t, err)
	require.Equal(t, "first", execution.Spec.Model)
	require.Equal(t, true, execution.Spec.TrustDirectory)
