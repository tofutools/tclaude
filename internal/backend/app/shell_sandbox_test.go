...
func TestShellSandboxRetainsAdmissionAndCurrentAuthorityAcrossReopen(t *testing.T) {
	...
	require.NoError(t, err)
	grant, err := service.PutGrant(ctx, app.PutGrantRequest{Principal: operator, Grant: model.AuthorityGrant{ID: "grant", Subject: model.AuthoritySubject{Kind: model.AuthorityAgent, AgentID: caller.Agent.ID}, Action: model.ActionStartShell, Resource: model.ResourceSelector{Kind: model.ResourceWorkspace, WorkspaceID: workspace.Workspace.ID}, Bounds: model.ConfigurationBounds{HostSandboxProfiles: []string{string(profile.Profile.ID)}}}})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.True(t, model.SameSandboxProfiles(&selected, started.Execution.Spec.HostSandbox))
	require.Len(t, shell.prepared, 1)
	require.NotEmpty(t, shell.prepared[0].HostSandboxPolicy.ContentHash)
	...
	require.Equal(t, started.Execution.Spec.HostSandbox, repeated.Execution.Spec.HostSandbox)
