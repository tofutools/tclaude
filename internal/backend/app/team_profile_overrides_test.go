...
func TestTeamHarnessOverrideResolvesInheritedConfinementBeforeAdmission(t *testing.T) {
	for _, explicit := range []bool{false, true} {
		t.Run(map[bool]string{false: "inherited", true: "explicit"}[explicit], func(t *testing.T) {
			...
			_, err = service.SaveConfigurationProfile(ctx, app.SaveConfigurationProfileRequest{Context: app.RequestContext{Principal: op, RequestID: "profile"}, ID: "profile", RevisionID: "one", Name: "Claude profile", Desired: model.DesiredConfiguration{Harness: "claude", Model: "sonnet", WorkingDirectory: t.TempDir(), Approval: model.ApprovalSupervised, Sandbox: model.SandboxWorkspaceWrite}})
			require.NoError(t, err)
			harness := "copilot"
