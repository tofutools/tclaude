...
func TestPeerMessagingPersistsOnAgentAndAdmittedExecution(t *testing.T) {
	...
	desired := model.DesiredConfiguration{Harness: "claude", Model: "first", PeerMessaging: true, WorkingDirectory: t.TempDir(), Approval: model.ApprovalSupervised, Sandbox: model.SandboxUnconfined}
	...
	require.Equal(t, false, durableAgent.Desired.PeerMessaging)
	execution, err := store.Execution(ctx, launched.Execution.ID)
	require.NoError(t, err)
	require.Equal(t, "first", execution.Spec.Model)
	require.Equal(t, true, execution.Spec.PeerMessaging)
	require.Equal(t, &profile.Revision.Ref, execution.Spec.ConfigurationProfile)
}

type peerMessagingProvider struct{ *fakeProvider }

func (*peerMessagingProvider) Name() string { return "claude" }
func (p *peerMessagingProvider) Prepare(ctx context.Context, request ports.PreparationRequest) (ports.PreparedAttempt, error) {
