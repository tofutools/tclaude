...
func TestTeamAdvisoryProcessPreservesGuidanceAndDeliversBeforeWork(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = store.PutRole(ctx, model.Role{ID: "review_role", Name: " Reviewer ", Revision: 1, CreatedAt: now, UpdatedAt: now}, 0)
	require.NoError(t, err)
	_, err = store.PutRoleAssignment(ctx, model.RoleAssignment{RoleID: "review_role", Subject: model.AuthoritySubject{Kind: model.AuthorityAgent, AgentID: deployed.Deployment.Members["worker"]}, Resource: model.ResourceSelector{Kind: model.ResourceGroupPeers, GroupID: deployed.Deployment.GroupID}, Revision: 1, CreatedAt: now, UpdatedAt: now}, 0)
	require.NoError(t, err)
