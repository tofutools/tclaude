...
func (h *Handler) registerGroupConfiguration(api app.GroupConfigurationAPI) {
	...
	h.mux.HandleFunc("POST /v2/groups/{id}/agents", func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := api.CreateGroupMember(r.Context(), app.CreateGroupMemberRequest{Workspace: body.Workspace, ProfileID: body.ProfileID, Launch: body.Launch, ConfigurationOverrides: body.ConfigurationOverrides, Labels: body.Labels, Context: app.RequestContext{Principal: p, RequestID: body.RequestID, WriteProofToken: body.WriteProofToken}, GroupID: model.GroupID(r.PathValue("id")), Environment: body.Environment, ID: body.ID, Name: body.Name, ExpectedGroupRevision: body.ExpectedGroupRevision, ExpectedDefaultRevision: body.ExpectedDefaultRevision})
