...
func (h *Handler) registerGroupConfiguration(api app.GroupConfigurationAPI) {
	...
	h.mux.HandleFunc("PUT /v2/groups/{id}/configuration", func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := api.SetGroupConfiguration(r.Context(), app.SetGroupConfigurationRequest{Principal: p, GroupID: model.GroupID(r.PathValue("id")), Profile: body.Profile, ExpectedRevision: body.ExpectedRevision})
		if err != nil {
			applicationError(w, err)
			return
		}
	...
	h.mux.HandleFunc("POST /v2/groups/{id}/agents", func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := api.CreateGroupMember(r.Context(), app.CreateGroupMemberRequest{Context: app.RequestContext{Principal: p, RequestID: body.RequestID}, GroupID: model.GroupID(r.PathValue("id")), ID: body.ID, Name: body.Name, ExpectedGroupRevision: body.ExpectedGroupRevision, ExpectedDefaultRevision: body.ExpectedDefaultRevision})
		if err != nil {
			applicationError(w, err)
			return
		}
