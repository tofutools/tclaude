...
func NewHandler(application app.API, auth Authenticator) (*Handler, error) {
	...
	if defaults, ok := application.(app.SandboxDefaultsAPI); ok {
		h.registerSandboxDefaults(defaults)
	}
	...
	if clone, ok := application.(app.GroupCloneAPI); ok {
		h.registerGroupClone(clone)
	}
...
func applicationError(w http.ResponseWriter, err error) {
	...
	if errors.As(err, &disabled) {
		...
		}{"profile_disabled", disabled.Error()})
		return
	}
