...
func TestAutoCompactWindowJSONAndExplicitClear(t *testing.T) {
	var options ConfigurationOptions
	require.NoError(t, json.Unmarshal([]byte(`{"AutoCompactWindow":"0.5M"}`), &options))
	require.Equal(t, AutoCompactWindow("500000"), *options.AutoCompactWindow)
	require.Error(t, json.Unmarshal([]byte(`{"AutoCompactWindow":450000}`), &options))
	require.NoError(t, json.Unmarshal([]byte(`{"AutoCompactWindow":""}`), &options))
	require.Empty(t, options.Apply(DesiredConfiguration{Harness: "claude", AutoCompactWindow: "450000"}).AutoCompactWindow)
	foreign := "codex"
	require.Empty(t, (&NativeConfigurationOptions{Harness: &foreign}).Apply(DesiredConfiguration{Harness: "claude", AutoCompactWindow: "450000"}).AutoCompactWindow)
}
