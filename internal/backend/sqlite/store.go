...
CREATE TABLE IF NOT EXISTS agents (
  ...
  harness TEXT NOT NULL, model TEXT NOT NULL, effort TEXT NOT NULL DEFAULT '', tool_governance TEXT NOT NULL DEFAULT '', fast_mode TEXT NOT NULL DEFAULT '', auto_review INTEGER NOT NULL DEFAULT 0, auto_memory INTEGER NOT NULL DEFAULT 0, working_directory TEXT NOT NULL,
...
func scanAgent(row scanner) (model.Agent, error) {
	...
	var created, updated int64
	err := row.Scan(&labels, &hostSandbox, &environment, &profile, &a.ID, &a.Name, &a.TaskReference, &a.ParentAgentID, &a.CloneSourceAgentID, &a.Lifecycle, &retired, &a.RetiredBy.Kind, &a.RetiredBy.AgentID, &a.RetiredBy.ExecutionID, &a.RetirementReason, &a.Notifications.DirectMessage, &a.Desired.Harness, &a.Desired.Model, &a.Desired.Effort, &a.Desired.ToolGovernance, &a.Desired.FastMode, &a.Desired.AutoReview, &a.Desired.AutoMemory, &a.Desired.WorkingDirectory, &a.Desired.Approval, &a.Desired.Sandbox, &a.PrimaryExecutionID, &a.Revision, &created, &updated)
	if err != nil {
...
func (s *Store) FindLaunchAdmission(ctx context.Context, in app.LaunchRetryLookup) (app.AdmissionResult, bool, error) {
	...
	desired := model.DesiredConfiguration{HostSandbox: model.CloneSandboxSelection(spec.HostSandbox), Harness: spec.Harness, Model: spec.Model, Effort: spec.Effort, ToolGovernance: spec.ToolGovernance, FastMode: spec.FastMode, AutoReview: spec.AutoReview, AutoMemory: spec.AutoMemory, WorkingDirectory: spec.WorkingDirectory, Approval: spec.Approval, Sandbox: spec.Sandbox, Environment: spec.Environment.Clone()}
