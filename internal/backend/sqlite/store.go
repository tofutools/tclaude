...
func (s *Store) RecordRecovery(ctx context.Context, id model.ExecutionID, state model.ExecutionState, native *model.NativeConversationEvidence, evidence model.ProviderEvidence, at time.Time) (model.Execution, error) {
	...
	_, err := s.db.ExecContext(ctx, `UPDATE executions SET state=?,evidence_provider=CASE WHEN ?='' THEN evidence_provider ELSE ? END,evidence_version=CASE WHEN ?='' THEN evidence_version ELSE ? END,evidence_payload=CASE WHEN ?='' THEN evidence_payload ELSE ? END,native_namespace=CASE WHEN ?='' THEN native_namespace ELSE ? END,native_reference=CASE WHEN ?='' THEN native_reference ELSE ? END,native_observed_at=CASE WHEN ?='' THEN native_observed_at ELSE ? END,revision=revision+1,updated_at=? WHERE id=?`, state, evidence.Provider, evidence.Provider, evidence.Provider, evidence.Version, evidence.Provider, evidence.Payload, reference, namespace, reference, reference, reference, observed, nanos(at), id)
	if err != nil {
		return model.Execution{}, err
	}
	if err = s.bump(ctx); err != nil {
