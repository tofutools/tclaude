...
func (s *Store) CreateTeamDeployment(ctx context.Context, deployment model.TeamDeployment, group model.Group, agents []model.Agent, assignments []model.RoleAssignment, principal model.Principal, at time.Time) (model.TeamDeployment, bool, error) {
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO team_deployments(id,definition_json,dependency_closure_json,mission,parameters_json,group_id,members_json,role_pins_json,automation_rule_ids_json,work_run_id,advisory_phase,state,revision,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, deployment.ID, definition, closure, deployment.Mission, parameters, deployment.GroupID, members, rolePins, rules, deployment.WorkRunID, deployment.AdvisoryPhase, deployment.State, deployment.Revision, nanos(deployment.CreatedAt), nanos(deployment.UpdatedAt)); err != nil {
		return model.TeamDeployment{}, false, classify(err)
	}
...
func (s *Store) TeamDeployment(ctx context.Context, id model.DeploymentID) (model.TeamDeployment, error) {
	...
	var definition, closure, parameters, members, rolePins, rules []byte
	var created, updated int64
	err := s.db.QueryRowContext(ctx, `SELECT id,definition_json,dependency_closure_json,mission,parameters_json,group_id,members_json,role_pins_json,automation_rule_ids_json,work_run_id,advisory_phase,state,revision,created_at,updated_at FROM team_deployments WHERE id=?`, id).Scan(&deployment.ID, &definition, &closure, &deployment.Mission, &parameters, &deployment.GroupID, &members, &rolePins, &rules, &deployment.WorkRunID, &deployment.AdvisoryPhase, &deployment.State, &deployment.Revision, &created, &updated)
	if err != nil {
		return deployment, classify(err)
	}
	if err = unmarshalMany([][]byte{definition, closure, parameters, members, rolePins, rules}, []any{&deployment.Definition, &deployment.DependencyClosure, &deployment.Parameters, &deployment.Members, &deployment.RolePins, &deployment.AutomationRuleIDs}); err != nil {
