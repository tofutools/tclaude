	...
	for _, mode := range []model.ApprovalMode{model.ApprovalNever, model.ApprovalOnRequest, model.ApprovalOnFailure, model.ApprovalUntrusted} {
		t.Run(string(mode), func(t *testing.T) {
			...
			for _, intent := range []ports.StartIntent{ports.StartFresh, ports.StartContinue} {
				...
				request := ports.PreparationRequest{Intent: intent, Spec: model.ResolvedExecutionSpec{ExecutionID: id, Attempt: 1, Harness: Name, WorkingDirectory: cwd, Approval: mode, Sandbox: model.SandboxReadOnly}}
