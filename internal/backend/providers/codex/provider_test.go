...
func TestProviderOwnsTerminalCredentialAndRecovery(t *testing.T) {
	...
	request := ports.PreparationRequest{Intent: ports.StartFresh, InitialInput: &ports.PreparedInitialInput{Body: "prepared codex brief", Correlation: "brief-codex", RequiredBeforeFirstWork: true}, Spec: model.ResolvedExecutionSpec{ExecutionID: "execution_codex", Attempt: 3, Harness: Name, Model: "test-model", WorkingDirectory: root, Approval: model.ApprovalAutomatic, Sandbox: model.SandboxUnconfined}, ActionCredential: &ports.ActionCredentialMaterial{ExecutionID: "execution_codex", Generation: 1, DeliveryID: "delivery", Secret: []byte("codex-secret"), ExpiresAt: expires}}
	...
	require.Eventually(t, func() bool {
		...
		return readErr == nil && strings.Contains(string(raw), "never") && strings.Contains(string(raw), "danger-full-access") && strings.Contains(string(raw), "prepared codex brief")
