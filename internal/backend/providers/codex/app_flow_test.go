...
func TestApplicationSQLiteTerminalFlowWithNativeDouble(t *testing.T) {
	...
	root, err := os.MkdirTemp("/tmp", "tcl-codex-app-")
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, os.RemoveAll(root)) })
	input := filepath.Join(root, "input")
	executable := filepath.Join(root, "codex-native-double")
	script := "#!/bin/sh\nwhile IFS= read -r line; do printf '%s\\n' \"$line\" >> \"" + input + "\"; done\n"
	require.NoError(t, os.WriteFile(executable, []byte(script), 0o700))
	provider, err := codex.New(codex.Config{Executable: executable, PrivateRoot: filepath.Join(root, "provider"), AgentSocket: filepath.Join(root, "agent.sock")})
	require.NoError(t, err)
	...
	desired := model.DesiredConfiguration{Harness: codex.Name, Model: "test-model", WorkingDirectory: root, Approval: model.ApprovalSupervised, Sandbox: model.SandboxWorkspaceWrite}
