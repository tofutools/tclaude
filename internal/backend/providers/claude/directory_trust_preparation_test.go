...
func TestProviderDirectoryTrustIsExplicitAndMalformedStoreIsNonfatal(t *testing.T) {
	for _, kind := range []string{"off", "on", "malformed"} {
		t.Run(kind, func(t *testing.T) {
			...
			initial := []byte("{\"counter\":9007199254740993}")
			if kind == "malformed" {
				initial = []byte("{\"projects\":[]}")
			}
			path := filepath.Join(nativeHome, ".claude.json")
			...
			require.NoError(t, err)
			prepared, err := provider.Prepare(ctx, ports.PreparationRequest{Intent: ports.StartFresh, Spec: model.ResolvedExecutionSpec{ExecutionID: "execution", Attempt: 1, Harness: Name, Model: "fixture", Approval: model.ApprovalSupervised, Sandbox: model.SandboxWorkspaceWrite, WorkingDirectory: root, TrustDirectory: kind != "off"}})
			require.NoError(t, err)
