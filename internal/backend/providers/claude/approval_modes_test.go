...
func TestNativeApprovalModesReachFreshAndContinuedLaunch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{model.ApprovalInherit, ""}, {model.ApprovalDefault, "manual"}, {model.ApprovalManual, "manual"}, {model.ApprovalPlan, "plan"}, {model.ApprovalAcceptEdits, "acceptEdits"}, {model.ApprovalAuto, "auto"}, {model.ApprovalDontAsk, "dontAsk"}, {model.ApprovalBypassPermissions, "bypassPermissions"},
	} {
		mode := tc.mode
		t.Run(string(mode), func(t *testing.T) {
			...
			for _, intent := range []ports.StartIntent{ports.StartFresh, ports.StartContinue} {
				...
				request := ports.PreparationRequest{Intent: intent, Spec: model.ResolvedExecutionSpec{ExecutionID: id, Attempt: 1, Harness: Name, WorkingDirectory: cwd, Approval: mode, Sandbox: model.SandboxWorkspaceWrite}}
				...
				require.Eventually(t, func() bool {
					...
					if tc.native == "" {
						return !strings.Contains(string(raw), "--permission-mode\n")
					}
					return strings.Contains(string(raw), "--permission-mode\n"+tc.native+"\n")
