...
func TestProviderHostSandboxPreparesExactCommandAndRefusesChangedCredentialBeforeRelease(t *testing.T) {
	...
	root, err := os.MkdirTemp("/tmp", "sb-claude-")
	...
	request.Intent = ports.StartContinue
	request.Spec.HostSandbox, request.HostSandboxPolicy = nil, nil
	request.PriorEvidence = description.Evidence
	request.Continuation = &model.NativeConversationEvidence{Namespace: NativeNamespace, Reference: recorded.NativeID}
	request.ActionCredential = nil
	resumed, err := provider.Prepare(ctx, request)
	require.NoError(t, err)
	require.Contains(t, resumed.(*prepared).command.Env, "CLAUDE_CONFIG_DIR="+provider.nativeHome, "changing confinement must not switch the conversation's configuration root")
	require.NoError(t, resumed.Abort(ctx))
}
...
`
	...
	require.NoError(t, err)
	nativeHome := filepath.Join(private, "native-config")
	...
	require.Equal(t, nativeHome, retained.NativeHome)
	history, err := os.ReadFile(filepath.Join(nativeHome, "history-fixture"))
	require.NoError(t, err)
	require.Equal(t, "history", string(history))
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
