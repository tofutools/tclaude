...
func TestAutoCompactWindowReachesFreshAndContinuedLaunch(t *testing.T) {
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Skip("tmux unavailable")
	}
	t.Setenv("CLAUDE_CODE_AUTO_COMPACT_WINDOW", "ambient-conflict")
	for _, enabled := range []bool{false, true} {
		t.Run(map[bool]string{false: "off", true: "on"}[enabled], func(t *testing.T) {
			...
			require.NoError(t, os.WriteFile(executable, []byte("#!/bin/sh\nprintf '%s\\n' \"$@\" > \"$PWD/argv\"\nprintf '%s' \"$CLAUDE_CODE_AUTO_COMPACT_WINDOW\" > \"$PWD/memory\"\nwhile IFS= read -r line; do :; done\n"), 0700))
			...
			for _, intent := range []ports.StartIntent{ports.StartFresh, ports.StartContinue} {
				...
				request := ports.PreparationRequest{Intent: intent, Spec: model.ResolvedExecutionSpec{ExecutionID: id, Attempt: 1, Harness: Name, WorkingDirectory: cwd, Approval: model.ApprovalManual, AutoCompactWindow: map[bool]model.AutoCompactWindow{false: "", true: "450000"}[enabled], Sandbox: model.SandboxWorkspaceWrite}}
				...
				require.Eventually(t, func() bool {
					...
					return err == nil && string(memory) == map[bool]string{false: "ambient-conflict", true: "450000"}[enabled]
