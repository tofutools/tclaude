...
func TestImportClaudeNativeApprovalPreservesProfileAndAgentBirthPolicy(t *testing.T) {
	for _, mode := range []model.ApprovalMode{model.ApprovalInherit, model.ApprovalDefault, model.ApprovalManual, model.ApprovalPlan, model.ApprovalAcceptEdits, model.ApprovalAuto, model.ApprovalDontAsk, model.ApprovalBypassPermissions} {
 ...
 VALUES('7','Claude native','[]','[]','[]','claude','fixture','/tmp','deny','workspace_write');
 UPDATE agents SET initial_spawn_config='{"harness":"claude","model":"fixture","working_directory":"/tmp","approval":"deny","sandbox":"workspace_write"}';
 `, "deny", string(mode)))
