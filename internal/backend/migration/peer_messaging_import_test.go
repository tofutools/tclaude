...
func TestImportClaudePeerMessagingPreservesProfileAndAgentBirthPolicy(t *testing.T) {
 ...
 INSERT INTO spawn_profiles(id,name,permission_overrides,environment_json,role_refs,harness,model,working_directory,approval,peer_messaging,sandbox)
 VALUES('7','Claude native','[]','[]','[]','claude','fixture','/tmp','manual',FAST,'unconfined');
 `, "FAST", map[bool]string{true: "1", false: "0"}[mode]))
			alterFixture(t, bundle, strings.ReplaceAll(`UPDATE agents SET initial_spawn_config='{"harness":"claude","model":"fixture","cwd":"/tmp","approval":"manual","sandbox":"unconfined","peer_messaging":BOOL}';`, "BOOL", map[bool]string{true: "true", false: "false"}[mode]))
			...
			require.NoError(t, err)
			require.Equal(t, mode, profile.Revision.Desired.PeerMessaging)
			...
			require.Equal(t, mode, snapshot.Agents[0].Desired.PeerMessaging)
			...
			require.NoError(t, err)
			require.Equal(t, mode, created.Agent.Desired.PeerMessaging)
		})
	}
}

func TestImportPeerMessagingRequiresSourceColumn(t *testing.T) {
	bundle := buildFixture(t, fixtureOptions{})
	alterFixture(t, bundle, "ALTER TABLE spawn_profiles DROP COLUMN peer_messaging")
	destination := filepath.Join(t.TempDir(), "target.sqlite")
...
func TestImportPeerMessagingResolvedChoiceOverridesBirth(t *testing.T) {
	bundle := buildFixture(t, fixtureOptions{})
	alterFixture(t, bundle, `UPDATE agents SET initial_spawn_config='{"harness":"claude","model":"fixture","cwd":"/tmp","approval":"manual","sandbox":"unconfined","peer_messaging":true}',relaunch_profile='{"version":1,"peer_messaging":false}';`)
	destination := filepath.Join(t.TempDir(), "target.sqlite")
	_, err := ImportSnapshot(context.Background(), bundle, ImportOptions{DestinationPath: destination})
	require.NoError(t, err)
	store, err := backendsqlite.Open(destination)
	...
	require.NoError(t, err)
	require.Len(t, snapshot.Agents, 1)
	require.False(t, snapshot.Agents[0].Desired.PeerMessaging)
}

func TestImportPartialPeerMessagingRetainsExplicitOff(t *testing.T) {
	bundle := buildFixture(t, fixtureOptions{})
	alterFixture(t, bundle, `INSERT INTO spawn_profiles(id,name,permission_overrides,environment_json,role_refs,peer_messaging) VALUES('7','Memory off','[]','[]','[]',0);`)
	destination := filepath.Join(t.TempDir(), "target.sqlite")
	_, err := ImportSnapshot(context.Background(), bundle, ImportOptions{DestinationPath: destination})
	require.NoError(t, err)
	store, err := backendsqlite.Open(destination)
	...
	require.False(t, *saved.Revision.Options.PeerMessaging)
}
