...
func TestImportClaudeAutoMemoryPreservesProfileAndAgentBirthPolicy(t *testing.T) {
 ...
 INSERT INTO spawn_profiles(id,name,permission_overrides,environment_json,role_refs,harness,model,working_directory,approval,auto_memory,sandbox)
 VALUES('7','Claude native','[]','[]','[]','claude','fixture','/tmp','manual',FAST,'unconfined');
 `, "FAST", map[bool]string{true: "1", false: "0"}[mode]))
			alterFixture(t, bundle, strings.ReplaceAll(`UPDATE agents SET initial_spawn_config='{"harness":"claude","model":"fixture","cwd":"/tmp","approval":"manual","sandbox":"unconfined","auto_memory":BOOL}';`, "BOOL", map[bool]string{true: "true", false: "false"}[mode]))
			...
			require.NoError(t, err)
			require.Equal(t, mode, profile.Revision.Desired.AutoMemory)
			...
			require.Equal(t, mode, snapshot.Agents[0].Desired.AutoMemory)
			...
			require.NoError(t, err)
			require.Equal(t, mode, created.Agent.Desired.AutoMemory)
		})
	}
}

func TestImportAutoMemoryRequiresSourceColumn(t *testing.T) {
	bundle := buildFixture(t, fixtureOptions{})
	alterFixture(t, bundle, "ALTER TABLE spawn_profiles DROP COLUMN auto_memory")
	destination := filepath.Join(t.TempDir(), "target.sqlite")
...
func TestImportAutoMemoryResolvedChoiceOverridesBirth(t *testing.T) {
	bundle := buildFixture(t, fixtureOptions{})
	alterFixture(t, bundle, `UPDATE agents SET initial_spawn_config='{"harness":"claude","model":"fixture","cwd":"/tmp","approval":"manual","sandbox":"unconfined","auto_memory":true}',relaunch_profile='{"version":1,"auto_memory":false}';`)
	destination := filepath.Join(t.TempDir(), "target.sqlite")
	_, err := ImportSnapshot(context.Background(), bundle, ImportOptions{DestinationPath: destination})
	require.NoError(t, err)
	store, err := backendsqlite.Open(destination)
	...
	require.NoError(t, err)
	require.Len(t, snapshot.Agents, 1)
	require.False(t, snapshot.Agents[0].Desired.AutoMemory)
}

func TestImportPartialAutoMemoryRetainsExplicitOff(t *testing.T) {
	bundle := buildFixture(t, fixtureOptions{})
	alterFixture(t, bundle, `INSERT INTO spawn_profiles(id,name,permission_overrides,environment_json,role_refs,auto_memory) VALUES('7','Memory off','[]','[]','[]',0);`)
	destination := filepath.Join(t.TempDir(), "target.sqlite")
	_, err := ImportSnapshot(context.Background(), bundle, ImportOptions{DestinationPath: destination})
	require.NoError(t, err)
	store, err := backendsqlite.Open(destination)
	...
	require.NotNil(t, saved.Revision.Options.AutoMemory)
	require.False(t, *saved.Revision.Options.AutoMemory)
}
