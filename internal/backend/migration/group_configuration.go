...
func (t *translator) translateGroupConfiguration(batch *app.ImportBatch, row sourcev228.Row, group model.Group) {
	...
	if sourceID == "" {
		if sourcev228.String(row.Values["default_profile"]) != "" {
			t.launchMetadataDiagnostic(batch, "agent_groups", row.Key, "group_default_retained_unmapped", "legacy group default name has no stable source profile ID; exact selection remains in retained source evidence")
		}
		return
	}
