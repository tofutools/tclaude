...
// resolveLaunchOSSandbox mirrors what production's session/new.go records at
// the launch boundary: whether the OS sandbox will ACTUALLY confine this agent,
// and what decided that (TCL-729). Omitting it here would leave simulated rows
// claiming no verdict for launches that always have one in production, and a
// dashboard read path exercised against those rows would quietly under-report.
//
// It resolves against the flow test's temp $HOME, so a test that writes a
...
func (s *simSpawner) SpawnNew(args clcommon.SpawnArgs) error {
	...
	s.w.RecordSpawnSandboxPolicy(cc.ConvID, args.EffectiveSandbox)
	...
	s.w.RecordSpawnSandboxImplementation(cc.ConvID, args.SandboxImplementation)
	...
	// harness exited on startup, so only a retained dead pane survives. Production
...
func (s *simSpawner) spawnNewCodex(args clcommon.SpawnArgs) error {
	...
	s.w.RecordSpawnSandboxPolicy(cx.ConvID, args.EffectiveSandbox)
	...
	s.w.RecordSpawnSandboxImplementation(cx.ConvID, args.SandboxImplementation)
