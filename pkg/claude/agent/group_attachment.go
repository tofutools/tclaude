...
func groupAttachmentCmd() *cobra.Command {
	return boa.CmdT[struct{}]{
		...
		Long: "Set, clear, or show the persistent http(s) reference attached to a group — " +
			...
			"renders it as a floating paperclip on group-header hover. Writes require `groups.attachment` " +
...
type groupAttachmentSetParams struct {
	...
	Label    string `long:"label" short:"l" optional:"true" help:"Optional display label overriding the auto-derived one (Linear/AWB issue id, GitHub repository/number, or hostname)"`
