...
func completePermissionSlugs(_ *cobra.Command, _ []string, toComplete string) []string {
	...
	if len(slugs) == 0 {
		...
		slugs = []slugEntry{
			...
			{"groups.description", "Set group descriptions"},
			{"groups.default-dir", "Set group default spawn directories"},
			{"groups.default-context", "Set group startup context"},
			{"groups.default-spawn-group", "Set the default spawn group"},
			{"groups.default-profile", "Set group default spawn profiles"},
			{"groups.max-members", "Set group member caps"},
			{"groups.notifications", "Set group notification policy"},
			{"groups.remote-control", "Set group remote-control policy"},
			{"groups.permissions", "Set live group permission grants"},
			{"groups.owner-scopes", "Set group owner-bypass scopes"},
