...
func completePermissionSlugs(_ *cobra.Command, _ []string, toComplete string) []string {
	...
	if len(slugs) == 0 {
		...
		slugs = []slugEntry{
			...
			{"self.remote-control", "Toggle own Claude Code Remote Access"},
			...
			{"agent.debug-export", "Export ANOTHER agent's launch and sandbox configuration"},
			...
			{"groups.clone", "Clone a group and its selected configuration"},
			{"groups.rename", "Rename groups"},
			{"groups.settings.description", "Set group descriptions"},
			{"groups.settings.default-dir", "Set group default spawn directories"},
			{"groups.settings.default-context", "Set group startup context"},
			{"groups.settings.default-spawn-target", "Set the default spawn group"},
			{"groups.settings.default-profile", "Set group default spawn profiles"},
			{"groups.settings.max-members", "Set group member caps"},
			{"groups.settings.notifications", "Set group notification policy"},
			{"groups.settings.remote-control-policy", "Set group remote-control policy"},
			...
			{"groups.members.debug-export", "Export a group member's launch and sandbox configuration"},
			{"groups.members.add", "Add members to a group"},
			{"groups.members.remove", "Remove members from a group"},
			{"groups.members.update", "Edit role/descr on group members"},
			{"groups.owners.manage", "Grant or revoke group ownership"},
			{"groups.archive", "Archive or unarchive groups"},
			{"groups.nest", "Set or clear a parent group"},
			{"groups.attachment", "Set or clear a group's external reference"},
			{"groups.link.add", "Create inter-group messaging links"},
			{"groups.link.remove", "Remove inter-group messaging links"},
			{"groups.export", "Export a group archive"},
			{"groups.import", "Import a group archive"},
			...
			{"proxy.awb.read", "Read AWB issues through the daemon's AWB account"},
			{"proxy.awb.write", "Create/update/claim/close/delete AWB issues as the operator"},
		}
	}
	out := []string{}
	for _, s := range slugs {
