...
func installExtras(params *Params) error {
	if params.InstallAgentSkills || params.InstallAll {
		fmt.Println("\n=== Agent Skills ===")
		if err := installAgentSkills(!params.InstallProxySkills); err != nil {
			return err
		}
	}
...
func installAgentSkills(retireLegacyProxySkills bool) error {
	...
	// behavior as fresh installations. Skip retirement entirely when
	// --install-proxy-skills is present; its section refreshes them later.
	if retireLegacyProxySkills {
		claudeRetired, err := agent.RetireLegacyProxySkills()
		if err != nil {
			return fmt.Errorf("retire legacy Claude Code proxy skills: %w", err)
		}
		codexRetired, err := agent.RetireLegacyCodexProxySkills()
		if err != nil {
			return fmt.Errorf("retire legacy Codex CLI proxy skills: %w", err)
		}
		for _, skill := range append(claudeRetired, codexRetired...) {
			fmt.Printf("✓ Retired legacy %s manifest to %s (rename it to SKILL.md to recover)\n", skill.Name, skill.Path)
		}
	}
