...
// ExclusiveV7Executor is the deploy-disabled schema-7 execution integration.
// It is concrete-FS-only and has no live host, scheduler, or CLI caller. Every
// adapter side effect occurs after the exact claim is durably appended and
// after the coherent view callback has released all store locks.
...
func NewExclusiveV7(st *store.FS, adapters map[model.PerformerKind]Adapter) *ExclusiveV7Executor {
	...
	return &ExclusiveV7Executor{Store: st, Adapters: copied}
}
...
func (e *ExclusiveV7Executor) exclusiveRequest(attempt *pathv1.ExclusiveAttemptPlan) (Request, Adapter, error) {
	...
	request := Request{
		Command: plan.Command{
			ID: command.ID, IdempotencyKey: command.IdempotencyKey, Kind: plan.CommandKindStartAttempt,
