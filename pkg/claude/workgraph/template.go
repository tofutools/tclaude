// Package workgraph defines, parses, and validates tclaude workgraph templates.
//
// A workgraph template is user data on disk: a directory holding a mermaid flow
// chart (flow.mmd), a metadata file (workgraph.yaml), and one YAML file per node
...
// are advanced by agentd; see the Workgraphs epic on Linear (JOH-9).
package workgraph
...
// the instance at create (workgraph_instances.engine_mode).
...
// Param is a workgraph instantiation parameter.
type Param struct {
...
// Template is a fully-loaded, validated workgraph template.
