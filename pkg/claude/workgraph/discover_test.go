...
func writeTemplate(t *testing.T, parent, dirName, wfName string) string {
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, "workgraph.yaml"), []byte("name: "+wfName+"\nentry: a\n"), 0o644))
...
func TestList_BrokenTemplateReportsErr(t *testing.T) {
	...
	// Has a workgraph.yaml (so it's a candidate) but no flow.mmd (so it fails to load).
	require.NoError(t, os.WriteFile(filepath.Join(dir, "workgraph.yaml"), []byte("name: broken\n"), 0o644))
...
func TestUserDir(t *testing.T) {
	if ud := UserDir(); ud != "" {
		assert.Equal(t, "workgraphs", filepath.Base(ud))
	}
}
