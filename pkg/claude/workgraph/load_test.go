...
func tmplFS(workgraphYAML, mmd string, nodes map[string]string) fstest.MapFS {
	m := fstest.MapFS{
		"workgraph.yaml": &fstest.MapFile{Data: []byte(workgraphYAML)},
...
// problem list.
func loadProblems(t *testing.T, workgraphYAML, mmd string, nodes map[string]string) []string {
	t.Helper()
	_, err := LoadFS(tmplFS(workgraphYAML, mmd, nodes), "bad", SourceUser, "")
	require.Error(t, err)
...
// JOH-15 B1: the workgraph.yaml `engine:` field is system|agent, default system.
