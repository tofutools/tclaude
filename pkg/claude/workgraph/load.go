...
// templateMeta is the workgraph.yaml schema.
...
func (e *ValidationError) Error() string {
	...
	if e.Ref != "" {
		head = fmt.Sprintf("workgraph %q has %d problem(s)", e.Ref, len(e.Problems))
	}
...
// template directory (so it holds workgraph.yaml, flow.mmd and nodes/). dir is
...
func LoadFS(fsys fs.FS, ref string, source Source, dir string) (*Template, error) {
	...
	metaRaw, err := fs.ReadFile(fsys, "workgraph.yaml")
	if err != nil {
		return nil, fmt.Errorf("read workgraph.yaml: %w", err)
	}
	var meta templateMeta
	if err := yaml.Unmarshal(metaRaw, &meta); err != nil {
		return nil, fmt.Errorf("parse workgraph.yaml: %w", err)
	}
	t.Name = meta.Name
...
func (t *Template) validate() []string {
	...
	if strings.TrimSpace(t.Name) == "" {
		add("workgraph.yaml: name is required")
	}
	...
	if t.Engine != EngineSystem && t.Engine != EngineAgent {
		add("workgraph.yaml: engine %q must be %q or %q", t.Engine, EngineSystem, EngineAgent)
	}
	...
	if len(t.Entry) > 0 {
		...
		if len(t.MermaidNodes) > 0 && len(t.Entry) == 0 {
			add("no entry node: every node has an incoming edge (a pure cycle). Declare `entry:` in workgraph.yaml")
		}
	}
	...
	for _, p := range t.Params {
		if strings.TrimSpace(p.Name) == "" {
			add("workgraph.yaml: a param has an empty name")
			continue
		}
		if seenParam[p.Name] {
			add("workgraph.yaml: duplicate param %q", p.Name)
		}
