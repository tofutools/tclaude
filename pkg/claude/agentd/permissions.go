...
var permissionRegistry = []PermSlug{
	...
	},
	{
		...
		Description: "Merge a GitHub pull request through the daemon's gh credentials (tclaude proxy github pr merge). " +
			...
			"checks still apply. agent.git_proxy.protected_refs does NOT, because that list bounds direct pushes and its " +
			"defaults (main, master) are the branches pull requests target — honouring it here would make every merge grant a " +
			"no-op. Not default-granted and not owner-implied.",
	},
	{
		Slug:      PermLinearRead,
