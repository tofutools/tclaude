...
// one that takes no identifier at all.
func TestBrokerRefusals_AttributeToTheResolvedRow(t *testing.T) {
	...
	assert.Nil(t, r.forSession("spwn-claimed"),
		"nothing may appear against a row the caller merely named")
}
...
func TestBrokerRefusals_UnplaceableIsCountOnly(t *testing.T) {
	...
	bySession, unplaceable := r.snapshot()
	assert.Empty(t, bySession, "an unplaceable refusal must not name any row")
	assert.Equal(t, 2, unplaceable.Count)
}
...
func TestBrokerRefusals_EmptyResolvedIDFallsToTheCounter(t *testing.T) {
	...
	assert.Equal(t, 1, unplaceable.Count, "no row means no attribution, not a blank key")
}
...
func TestBrokerRefusals_ExpireOutOfTheWindow(t *testing.T) {
	...
	bySession, unplaceable := r.snapshot()
	assert.Empty(t, bySession)
	assert.Zero(t, unplaceable.Count)
}
