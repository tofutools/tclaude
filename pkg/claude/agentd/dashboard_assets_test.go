...
//   - dashboard.css hides the .group-descr chip by default and restores it under
//     body.show-group-description; creation settings remain available;
//   - config.js + dashboard.html expose the Config-tab checkbox.
//
...
func TestDashboardAssets_ShowGroupDescriptionWired(t *testing.T) {
	for _, needle := range []string{
		...
		// The editable creation form exposes description independently of headers.
		`.group-create-descr-row { display: flex; }`,
