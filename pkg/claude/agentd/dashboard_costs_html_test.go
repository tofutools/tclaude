...
func TestDashboardHTML_CostsTabWired(t *testing.T) {
	...
	must("state.toggleProvider(entry.provider)", "dropdown checkbox re-derives chart + summary + table without refetch")
	must(`id="filter-costs-models"`, "model filter mount present")
	must("tclaude.dash.costs.models", "model filter persisted")
	must("state.toggleModel(entry.model)", "model dropdown checkbox narrows chart, summary, and table without refetch")
	...
	must("summary::-webkit-details-marker { display: none; }", "native details marker does not duplicate the custom filter chevron")
	must("details.cost-filter-menu[open] > summary::before { content: none; }", "open filters override the shared details marker")
	must(`summary::after { content: "▸"`, "closed filters use the conventional right-pointing disclosure marker")
	must(`summary::after { content: "▾"`, "open filters use the conventional down-pointing disclosure marker")
	...
	must(`id="filter-costs-breakdown"`, "chart breakdown menu is present beside the data filters")
	must("tclaude.dash.costs.stackProvider", "provider chart grouping is persisted")
	must("tclaude.dash.costs.stackModel", "model chart grouping is persisted")
