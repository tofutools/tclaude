...
// each node's updated_at to a precise idle age (SetWorkflowNodeUpdatedAtForTest),
...
func stuckHumanNotices(t *testing.T) []*db.HumanMessage {
	...
	for _, m := range all {
		if strings.Contains(m.Subject, "[workflow]") {
			out = append(out, m)
		}
	}
	return out
...
func bindRunningAt(t *testing.T, id int64, nodeID, conv string, idle time.Duration) {
	...
	asg := conv
	_, err := db.UpdateWorkflowNode(id, nodeID, db.WorkflowNodePatch{Status: &running, Assignee: &asg})
	require.NoError(t, err)
	require.NoError(t, db.SetWorkflowNodeUpdatedAtForTest(id, nodeID, idleBy(idle)))
}
...
func parkAwaitingAt(t *testing.T, id int64, nodeID string, idle time.Duration) {
	...
	cleared := ""
	_, err := db.UpdateWorkflowNode(id, nodeID, db.WorkflowNodePatch{Status: &awaiting, Assignee: &cleared})
	require.NoError(t, err)
	require.NoError(t, db.SetWorkflowNodeUpdatedAtForTest(id, nodeID, idleBy(idle)))
}
...
func TestEscalation_AILiveAgent_WarnPingsAssignee(t *testing.T) {
	...
	t.Cleanup(agentd.SetWorkflowNodeSLAForTest(escalationTestSLA))
	...
	assert.Equal(t, agentd.WorkflowEngineAssigneeForTest(), pings[0].FromConv, "ping is from the workflow-engine sentinel")
...
func TestEscalation_TerminalFailNotSuppressedByMarker(t *testing.T) {
	...
	n, err := db.GetWorkflowNode(id, "build")
	require.NoError(t, err)
	marker := fmt.Sprintf("%s:%d:%d:terminal", "build", n.UpdatedAt.UnixNano(), n.Visits)
	_, err = db.AppendWorkflowEvent(&db.WorkflowEvent{
		InstanceID: id, NodeID: "build", Kind: db.WorkflowEventNodeEscalation, Message: marker,
	})
	require.NoError(t, err)
...
func TestEscalation_HumanApproveGate_NotifiesNeverFails(t *testing.T) {
	...
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
	t.Cleanup(agentd.SetWorkflowHumanNodeSLAForTest(escalationTestSLA))
	...
	got, err := db.GetWorkflowNode(id, "gate")
	require.NoError(t, err)
...
func TestEscalation_HumanExecutorNode_NotifiesNeverFails(t *testing.T) {
	...
	require.NoError(t, db.SetWorkflowNodeUpdatedAtForTest(id, "review", idleBy(9*time.Minute)))
	agentd.RunWorkflowEngineTickForTest()

	require.Len(t, stuckHumanNotices(t), 1, "an idle human-executor node notifies the human")
	got, err := db.GetWorkflowNode(id, "review")
	require.NoError(t, err)
...
func TestEscalation_Idempotent_AcrossTicksAndRestart(t *testing.T) {
	...
	agentd.RunWorkflowEngineTickForTest()
	agentd.RunWorkflowEngineTickForTest() // re-derives from durable markers (restart-equivalent)
...
func TestEscalation_Reactivation_FiresFreshWarn(t *testing.T) {
	...
	require.NoError(t, db.SetWorkflowNodeUpdatedAtForTest(id, "build", idleBy(6*time.Minute)))
	agentd.RunWorkflowEngineTickForTest()
...
func TestEscalation_PerNodeSLAOverride(t *testing.T) {
	...
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
	t.Cleanup(agentd.SetWorkflowNodeSLAForTest(time.Hour)) // huge engine default
