...
func TestSessionNewArgs_Approval(t *testing.T) {
	if slices.Contains(sessionNewArgs("lbl", "/tmp/x", "", "", "codex", "", ""), "--ask-for-approval") {
		t.Fatalf("unset approval must omit --ask-for-approval")
	}
	args := sessionNewArgs("lbl", "/tmp/x", "", "", "codex", "workspace-write", "never")
	...
	rargs := sessionResumeArgs("conv-1", "/tmp/x", "", "", "codex", "workspace-write", "never")
