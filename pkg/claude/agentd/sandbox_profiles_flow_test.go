...
type wireSandboxProfile struct {
	...
	DarwinDisableKeychainWrite bool     `json:"darwin_disable_keychain_write"`
	Filesystem                 []struct {
...
func TestSandboxProfileExportUsesHighestVersionAcrossOrderedProfiles(t *testing.T) {
	...
	for _, profile := range []map[string]any{
		...
		{"name": "root-third", "filesystem_root": "separate"},
		{
	...
	rec := profileReq(t, f, http.MethodGet,
		"/v1/sandbox-profiles/export?name=harness-config-first&name=private-second&"+
			"name=root-third&name=caller-identity-last", nil)
...
func TestSandboxProfilesExportImportRoundTrip(t *testing.T) {
	...
	// mach-register capability. v18 adds the macOS Keychain write opt-out.
	...
	assert.Equal(t, float64(18), bundle["format_version"])
	bundle["format_version"] = 17
	assert.Equal(t, http.StatusBadRequest, profileReq(t, f, http.MethodPost, "/v1/sandbox-profiles/import/inspect", bundle).Code)
	bundle["format_version"] = 18
