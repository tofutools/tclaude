...
func TestDashboardHTML_VegasTab(t *testing.T) {
	...
	// vegas.js gates on the tclaude:slop event slop.js dispatches; if
	// that contract breaks we want a test failure, not silent dead
	// music. Both ends asserted.
	must("new CustomEvent('tclaude:slop'", "slop.js broadcasts state changes")
	must("'tclaude:slop'", "vegas.js listens for the slop state event")
	...
	// CSS: the nav button is hidden unless body.slop — the tab must not
	// leak into the plain (non-slop) dashboard.
	...
	must("body.slop nav button[data-tab=\"vegas\"]",
		"the Vegas nav button is revealed only in slop mode")
}
