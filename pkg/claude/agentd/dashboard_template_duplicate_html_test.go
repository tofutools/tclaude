...
func TestDashboardHTML_TemplateDuplicate(t *testing.T) {
	...
	// (b) The Preact card routes directly to the owned name dialog.
	must("onClick=${() => actions.openTemplateDuplicate(template.name)}", "the card routes duplicate to the owned dialog")
	must("function TemplateDuplicateDialog", "the duplicate dialog component is wired")
	...
	must("onSubmitHotkey: busy ? null : submit", "the duplicate dialog uses the shared keyboard-submit boundary")
	must("disabled=${busy}", "the submit paths are guarded against a double-POST in flight")
	...
	must("`${descriptor.source.name}-copy`", "the dialog prefills the copy name with a -copy suffix")
	...
	must("const payload = { ...source, name: name.trim() };", "duplicate clones the source template JSON with the name swapped")
