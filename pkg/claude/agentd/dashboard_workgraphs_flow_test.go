...
// wfReq drives one dashboard workgraph request through the cookie-injecting test
...
// wfDetail is the subset of GET /api/workgraphs/{id} the tests assert on.
...
func wfCreate(t *testing.T, mux http.Handler, ref, title string, params map[string]any) int64 {
	t.Helper()
	rec := wfReq(t, mux, http.MethodPost, "/api/workgraphs", map[string]any{
		"template_ref": ref, "title": title, "params": params,
	})
	require.Equal(t, http.StatusOK, rec.Code, "POST /api/workgraphs body=%s", rec.Body.String())
	var resp struct {
		ID int64 `json:"id"`
	}
...
func wfGet(t *testing.T, mux http.Handler, id int64) wfDetail {
	t.Helper()
	rec := wfReq(t, mux, http.MethodGet, "/api/workgraphs/"+strconv.FormatInt(id, 10), nil)
...
func wfPatch(t *testing.T, mux http.Handler, id int64, nodeID string, body map[string]any) wfPatchResult {
	...
	rec := wfReq(t, mux, http.MethodPatch,
		"/api/workgraphs/"+strconv.FormatInt(id, 10)+"/nodes/"+nodeID, body)
...
func TestDashboardWorkgraphs_InstantiateAndWalkToCompleted(t *testing.T) {
...
func TestDashboardWorkgraphs_EnumBranchSkipAndJoin(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
...
func TestDashboardWorkgraphs_EnumNodeRequiresOutcome(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	root := t.TempDir()
	writeDiamondTemplate(t, root)
	...
	rec := wfReq(t, mux, http.MethodPatch,
		"/api/workgraphs/"+strconv.FormatInt(id, 10)+"/nodes/start",
...
func TestDashboardWorkgraphs_MissingRequiredParam(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_CancelAndDelete(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
	...
	rec := wfReq(t, mux, http.MethodPost, "/api/workgraphs/"+strconv.FormatInt(id, 10)+"/cancel", nil)
	...
	del := wfReq(t, mux, http.MethodDelete, "/api/workgraphs/"+strconv.FormatInt(id, 10), nil)
	require.Equal(t, http.StatusNoContent, del.Code, "delete body=%s", del.Body.String())

	gone := wfReq(t, mux, http.MethodGet, "/api/workgraphs/"+strconv.FormatInt(id, 10), nil)
...
func TestDashboardWorkgraphs_NodeAudit(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
	...
	rec := wfReq(t, mux, http.MethodGet,
		"/api/workgraphs/"+strconv.FormatInt(id, 10)+"/nodes/plan/audit", nil)
...
// wfNodePost drives POST /api/workgraphs/{id}/nodes/{node}/{sub} (start/attach/approve).
func wfNodePost(t *testing.T, mux http.Handler, id int64, node, sub string, body any) *httptest.ResponseRecorder {
	...
	return wfReq(t, mux, http.MethodPost,
		"/api/workgraphs/"+strconv.FormatInt(id, 10)+"/nodes/"+node+"/"+sub, body)
}
...
func TestDashboardWorkgraphs_GroupBindingOnCreate(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	g := f.HaveGroup("squad")
	mux := agentd.BuildDashboardHandlerForTest()
	...
	var found bool
	for _, wfi := range snap.Workgraphs {
	...
	bad := wfReq(t, mux, http.MethodPost, "/api/workgraphs", map[string]any{
		"template_ref": "example:implement-microservice",
	...
	unbound := wfReq(t, mux, http.MethodPost, "/api/workgraphs", map[string]any{
...
func TestDashboardWorkgraphs_StartSpawnsAgentIntoGroup(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_StartGuards(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_AttachExistingMember(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	...
	assert.Contains(t, msgs[0].Subject, "Workgraph task", "inbox message names the workgraph task")
}
...
func TestDashboardWorkgraphs_HumanApproveAdvancesFrontier(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_HumanRejectRecordsNoAdvance(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_ApproveGuards(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
// #230 hardening.)
func TestDashboardWorkgraphs_ManualSkipRejected(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
	...
	for _, bad := range []string{"skipped", "pending", "ready", "awaiting_verify"} {
		rec := wfReq(t, mux, http.MethodPatch,
			"/api/workgraphs/"+strconv.FormatInt(id, 10)+"/nodes/plan",
...
func TestDashboardWorkgraphs_RejectsEngineSentinelAssignee(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
	...
	rec := wfReq(t, mux, http.MethodPatch,
		"/api/workgraphs/"+strconv.FormatInt(id, 10)+"/nodes/plan",
		map[string]any{"assignee": "<workgraph-engine>"})
	assert.Equal(t, http.StatusBadRequest, rec.Code,
...
func TestDashboardWorkgraphs_NodeJSONAgentVerifyAndWarnings(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_Warnings(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	root := t.TempDir()
	...
	require.Equal(t, http.StatusOK, snapRec.Code)
	var snap struct {
		WorkgraphTemplates []struct {
			Ref      string   `json:"ref"`
			Warnings []string `json:"warnings"`
		} `json:"workgraph_templates"`
	}
	require.NoError(t, json.Unmarshal(snapRec.Body.Bytes(), &snap))
	var tplWarned bool
	for _, tpl := range snap.WorkgraphTemplates {
	...
	detRec := wfReq(t, mux, http.MethodGet, "/api/workgraphs/"+strconv.FormatInt(id, 10), nil)
...
func wfCreateInGroup(t *testing.T, mux http.Handler, ref, title string, params map[string]any, group string) int64 {
	...
	require.Equal(t, http.StatusOK, rec.Code, "POST /api/workgraphs (group=%s) body=%s", group, rec.Body.String())
	var resp struct {
		ID int64 `json:"id"`
	}
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &resp))
...
// Scenario: the snapshot carries workgraph instances (with node counts) and the
// discoverable templates — what the Workgraphs tab renders off the 2s poll.
func TestDashboardWorkgraphs_Snapshot(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
	...
	assert.True(t, found, "instance %d should appear in snapshot.workgraphs", id)
	...
	assert.True(t, sawExample, "example template should appear in snapshot.workgraph_templates")
}

// Scenario: the workgraph routes are behind the dashboard cookie gate — an
// uncookied create is refused.
func TestDashboardWorkgraphs_AuthRequired(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)

	mux := http.NewServeMux()
	agentd.RegisterDashboardRoutesForTest(mux) // no cookie injection
	r := httptest.NewRequest(http.MethodPost, "/api/workgraphs",
		strings.NewReader(`{"template_ref":"example:implement-microservice","params":{"service_name":"x"}}`))
	r.Header.Set("Content-Type", "application/json")
	rec := testharness.Serve(mux, r)
	assert.NotEqual(t, http.StatusOK, rec.Code,
		"uncookied POST /api/workgraphs should be refused; body=%s", rec.Body.String())
}
...
func TestDashboardWorkgraphs_TerminalInstanceNotResurrected(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
func TestDashboardWorkgraphs_ReSettleRejected(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	mux := agentd.BuildDashboardHandlerForTest()
...
func writeDiamondTemplate(t *testing.T, root string) {
	...
	write("workgraph.yaml", "name: diamond\ndescription: branch and join\nentry: start\n")
	write("flow.mmd", "flowchart TD\n"+
		"  start{Pick} -->|left| a\n"+
		"  start -->|right| b\n"+
		"  a --> j\n"+
