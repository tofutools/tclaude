...
// Dashboard workgraph routes — the human-facing CRUD + manual-driving surface
// for workgraph instances. Sibling of dashboard_cron.go: every handler is
...
// advance nodes (which follows the template's branches via workgraph.Advance),
// cancel and delete.
//
...
// lockWorkgraphInstance so two concurrent drives never read-modify-write stale
// node state. The execution engine that AUTO-drives nodes is Step 6; the manual
// PATCH / start / approve paths and that engine all share workgraph.Advance.

// workgraphProjectDirsFn yields the project-source directories searched when
// resolving / listing workgraph templates. The daemon serves the whole machine,
...
// override it (see SetWorkgraphProjectDirsForTest) to point at a fixture dir.
var workgraphProjectDirsFn = defaultWorkgraphProjectDirs

func defaultWorkgraphProjectDirs() []string {
	wd, err := os.Getwd()
	if err != nil || wd == "" {
		return nil
	}
...
func registerDashboardWorkgraphsRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/workgraphs", handleDashboardWorkgraphsCreate)
	mux.HandleFunc("/api/workgraphs/", handleDashboardWorkgraphsAPI)
}

// handleDashboardWorkgraphsCreate handles POST /api/workgraphs: resolve+load the
...
func handleDashboardWorkgraphsCreate(w http.ResponseWriter, r *http.Request) {
...
// workgraphCreateBody is the instantiate wire shape shared by the dashboard POST
// and the /v1 socket twin.
type workgraphCreateBody struct {
...
// createWorkgraphInstance is the shared instantiation core: resolve + snapshot
...
func createWorkgraphInstance(body workgraphCreateBody) (map[string]any, *spawnFailure) {
	ref := strings.TrimSpace(body.TemplateRef)
	if ref == "" {
		return nil, &spawnFailure{http.StatusBadRequest, "invalid_arg", "template_ref is required"}
	}
	tmpl, err := workgraph.Resolve(ref, workgraphProjectDirsFn()...)
	if err != nil {
	...
	inst := &db.WorkgraphInstance{
		TemplateRef:  tmpl.Ref,
		TemplateName: tmpl.Name,
		Title:        title,
		Status:       db.WorkgraphStatusRunning,
	...
	id, err := db.InsertWorkgraphInstance(inst)
	if err != nil {
	...
	nodes := make([]*db.WorkgraphNode, 0, len(ids))
	for _, nid := range ids {
		n := tmpl.Nodes[nid]
		detail, _ := json.Marshal(n) // node-def snapshot; round-trips back via RebuildFromSnapshot
		status := db.WorkgraphNodeStatusPending
		if entry[nid] {
			status = db.WorkgraphNodeStatusReady
		}
		nodes = append(nodes, &db.WorkgraphNode{
			NodeID:       nid,
	...
	if err := db.InsertWorkgraphNodes(id, nodes); err != nil {
		// Roll back the instance so a failed node batch leaves no orphan.
		_ = db.DeleteWorkgraphInstance(id)
	...
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
		InstanceID: id,
		Kind:       db.WorkgraphEventInstanceCreated,
		Message:    "instantiated " + tmpl.Ref,
	})
	for _, nid := range ids {
		if entry[nid] {
			_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
				InstanceID: id, NodeID: nid, Kind: db.WorkgraphEventNodeReady,
			})
		}
	}
...
// handleDashboardWorkgraphsAPI dispatches the /api/workgraphs/{id}... surface:
//
//	GET    /api/workgraphs/{id}                         → full detail
//	DELETE /api/workgraphs/{id}                         → delete (cascades nodes+events)
//	POST   /api/workgraphs/{id}/cancel                  → cancel
//	PATCH  /api/workgraphs/{id}/nodes/{nodeId}          → manual node update + advance
//	GET    /api/workgraphs/{id}/nodes/{nodeId}/audit    → that node's event timeline
//	POST   /api/workgraphs/{id}/nodes/{nodeId}/start    → spawn the node's ai agent into the bound group
//	POST   /api/workgraphs/{id}/nodes/{nodeId}/attach   → attach an existing group member to the node
//	POST   /api/workgraphs/{id}/nodes/{nodeId}/approve  → human-verify gate (approve/reject)
func handleDashboardWorkgraphsAPI(w http.ResponseWriter, r *http.Request) {
	if !checkDashboardAuth(w, r) {
		return
	}
	rest := strings.TrimSuffix(strings.TrimPrefix(r.URL.Path, "/api/workgraphs/"), "/")
	if rest == "" {
		http.Error(w, "expected /api/workgraphs/{id}", http.StatusNotFound)
		return
	}
	...
	if inst == nil {
		http.Error(w, "workgraph "+strconv.FormatInt(id, 10)+" not found", http.StatusNotFound)
		return
	}
	...
	case len(parts) == 1:
		switch r.Method {
		case http.MethodGet:
			dashboardWorkgraphDetail(w, inst)
		...
		default:
			http.Error(w, "GET or DELETE on /api/workgraphs/{id}", http.StatusMethodNotAllowed)
		}
	case parts[1] == "cancel" && len(parts) == 2:
		...
		dashboardWorkgraphCancel(w, inst)
	case parts[1] == "nodes" && len(parts) >= 3 && parts[2] != "":
		sub := ""
		if len(parts) == 4 {
			sub = parts[3]
		}
		dashboardWorkgraphNode(w, r, inst, parts[2], sub)
	default:
		http.Error(w, "unknown path under /api/workgraphs/"+strconv.FormatInt(id, 10), http.StatusNotFound)
	}
}

// dashboardWorkgraphDetail returns one instance with its nodes, snapshotted
// chart, params/vars, and recent timeline — everything the detail view renders.
func dashboardWorkgraphDetail(w http.ResponseWriter, inst *db.WorkgraphInstance) {
	writeJSON(w, http.StatusOK, workgraphDetailJSON(inst))
}

// workgraphDetailJSON builds the full instance-detail payload (instance + mermaid
// + params/vars + nodes + recent events + topology warnings) shared by the
// dashboard GET and the /v1 socket twin.
func workgraphDetailJSON(inst *db.WorkgraphInstance) map[string]any {
	nodes, _ := db.ListWorkgraphNodes(inst.ID)
	events, _ := db.ListWorkgraphEvents(inst.ID)
	const maxEvents = 200
	...
	nodeViews := make([]workgraphNodeJSON, 0, len(nodes))
	for _, n := range nodes {
		nodeViews = append(nodeViews, nodeToJSON(n, tmpl))
	}
	eventViews := make([]workgraphEventJSON, 0, len(events))
...
// dashboardWorkgraphNode dispatches the per-node subroutes.
func dashboardWorkgraphNode(w http.ResponseWriter, r *http.Request, inst *db.WorkgraphInstance, nodeID, sub string) {
	...
	case "":
		...
		dashboardWorkgraphNodePatch(w, r, inst, node)
	case "audit":
		if r.Method != http.MethodGet {
			http.Error(w, "GET only", http.StatusMethodNotAllowed)
			return
		}
		events, _ := db.ListWorkgraphEvents(inst.ID, nodeID)
	...
	case "start", "attach", "approve":
		...
		switch sub {
		case "start":
			dashboardWorkgraphNodeStart(w, inst, node.NodeID)
		case "attach":
			dashboardWorkgraphNodeAttach(w, r, inst, node.NodeID)
		case "approve":
			dashboardWorkgraphNodeApprove(w, r, inst, node.NodeID)
		}
	default:
...
// dashboardWorkgraphNodePatch applies a manual node update — the MVP driving
// path. A status of done/failed is a settle: it runs workgraph.Advance to ready
...
func dashboardWorkgraphNodePatch(w http.ResponseWriter, r *http.Request, inst *db.WorkgraphInstance, node *db.WorkgraphNode) {
	var body workgraphNodePatchBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		http.Error(w, "bad JSON: "+err.Error(), http.StatusBadRequest)
		return
	}
	res, fail := applyWorkgraphNodePatch(inst.ID, node.NodeID, body)
	if fail != nil {
		http.Error(w, fail.Msg, fail.Status)
		return
	}
...
// workgraphNodePatchBody is the wire shape for a manual node update — shared by
// the dashboard PATCH and the /v1 socket twin. All fields optional; a settling
// status (done/failed) drives Advance.
type workgraphNodePatchBody struct {
...
// applyWorkgraphNodePatch is the shared core behind both the dashboard PATCH and
...
func applyWorkgraphNodePatch(instanceID int64, nodeID string, body workgraphNodePatchBody) (map[string]any, *spawnFailure) {
	...
	inst, ierr := db.GetWorkgraphInstance(instanceID)
	if ierr != nil {
		return nil, &spawnFailure{http.StatusInternalServerError, "io", "lookup instance: " + ierr.Error()}
	}
	if inst == nil {
		return nil, &spawnFailure{http.StatusNotFound, "not_found", "workgraph not found"}
	}
	node, ferr := db.GetWorkgraphNode(instanceID, nodeID)
	if ferr != nil {
	...
	if isSettledWorkgraphNodeStatus(node.Status) && body.Status != nil &&
		isSettledWorkgraphNodeStatus(strings.TrimSpace(*body.Status)) {
	...
	now := time.Now()
	patch := db.WorkgraphNodePatch{}
	newStatus := node.Status
	if body.Status != nil {
		s := strings.TrimSpace(*body.Status)
		if !validWorkgraphNodeStatus(s) {
		...
		if s == db.WorkgraphNodeStatusDone && node.Status == db.WorkgraphNodeStatusRunning &&
		...
		if s == db.WorkgraphNodeStatusFailed && node.Status == db.WorkgraphNodeStatusRunning &&
			node.ExecutorKind == string(workgraph.ExecAI) && aiVerifyCanRun(inst) &&
		...
		patch.Status = &s
		newStatus = s
		if s == db.WorkgraphNodeStatusRunning && node.StartedAt.IsZero() {
			patch.StartedAt = &now
		}
		if isSettledWorkgraphNodeStatus(s) {
	...
	settling := body.Status != nil && (newStatus == db.WorkgraphNodeStatusDone || newStatus == db.WorkgraphNodeStatusFailed)
	var settleOutcome string
	if settling {
		if newStatus == db.WorkgraphNodeStatusFailed {
			settleOutcome = workgraph.OutcomeFail
		} else { // done
			...
			if settleOutcome == "" {
				...
				settleOutcome = workgraph.OutcomePass
			}
	...
	if _, err := db.UpdateWorkgraphNode(instanceID, nodeID, patch); err != nil {
	...
	instanceRunning := inst.Status == db.WorkgraphStatusRunning
	advanced := workgraph.AdvanceResult{Ready: []string{}, Skipped: []string{}}
	if settling && tmpl != nil && instanceRunning {
		advanced = workgraph.Advance(tmpl, nodeID, settleOutcome, nodeStateMap(instanceID))
		applyWorkgraphAdvance(instanceID, nodeID, tmpl, advanced, now)
	}

	newInstStatus := inst.Status
	if instanceRunning {
		nodes, _ := db.ListWorkgraphNodes(instanceID)
		newInstStatus = recomputeWorkgraphInstanceStatus(tmpl, nodes)
		if newInstStatus != inst.Status {
			if _, err := db.UpdateWorkgraphInstanceStatus(instanceID, newInstStatus); err == nil {
...
func parkNodeForAIVerify(instanceID int64, nodeID string, body workgraphNodePatchBody) (map[string]any, *spawnFailure) {
	awaiting := db.WorkgraphNodeStatusAwaitingVerify
	cleared := ""
	patch := db.WorkgraphNodePatch{Status: &awaiting, Assignee: &cleared}
	if body.Output != nil {
		patch.Output = body.Output
	}
	if _, err := db.UpdateWorkgraphNode(instanceID, nodeID, patch); err != nil {
	...
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
		InstanceID: instanceID, NodeID: nodeID, Kind: db.WorkgraphEventNodeAwaitingVerify,
	...
	return map[string]any{
		...
		"node_id":         nodeID,
		"status":          awaiting,
		"instance_status": db.WorkgraphStatusRunning,
		"ready":           []string{},
		"skipped":         []string{},
	}, nil
}
...
func rearmAINodeForRetry(instanceID int64, nodeID, output string) (map[string]any, *spawnFailure) {
	...
	cleared := ""
	patch := db.WorkgraphNodePatch{Status: &ready, Assignee: &cleared}
	if output != "" {
		patch.Output = &output
	}
	if _, err := db.UpdateWorkgraphNode(instanceID, nodeID, patch); err != nil {
	...
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
		InstanceID: instanceID, NodeID: nodeID, Kind: db.WorkgraphEventNodeRetry,
...
// applyWorkgraphAdvance writes the proposed transitions: pending → ready for each
...
func applyWorkgraphAdvance(instanceID int64, settledID string, tmpl *workgraph.Template, res workgraph.AdvanceResult, now time.Time) {
	for _, nid := range res.Ready {
		cur, _ := db.GetWorkgraphNode(instanceID, nid)
		if cur == nil || cur.Status != db.WorkgraphNodeStatusPending {
			continue
		}
		st := db.WorkgraphNodeStatusReady
		_, _ = db.UpdateWorkgraphNode(instanceID, nid, db.WorkgraphNodePatch{Status: &st})
		_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: nid, Kind: db.WorkgraphEventNodeReady})
	}
	for _, nid := range res.Skipped {
		...
		st := db.WorkgraphNodeStatusSkipped
		fin := now
		_, _ = db.UpdateWorkgraphNode(instanceID, nid, db.WorkgraphNodePatch{Status: &st, FinishedAt: &fin})
		_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: nid, Kind: db.WorkgraphEventNodeSkipped})
	}
...
// target to the back-edge source settledID — see workgraph.Template.LoopBody) is
...
func reenterLoop(instanceID int64, settledID, target string, tmpl *workgraph.Template) {
	if tmpl == nil {
		return
	}
	body := tmpl.LoopBody(target, settledID)
	ready := db.WorkgraphNodeStatusReady
	pending := db.WorkgraphNodeStatusPending
	cleared := ""
	var zeroTime time.Time
	for id := range body {
		cur, _ := db.GetWorkgraphNode(instanceID, id)
		if cur == nil {
			continue
		}
		if id == target {
			// The re-entry point itself becomes ready for the new iteration.
			_, _ = db.UpdateWorkgraphNode(instanceID, id, db.WorkgraphNodePatch{
				Status: &ready, Outcome: &cleared, FinishedAt: &zeroTime, Assignee: &cleared})
			_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: id,
				Kind: db.WorkgraphEventNodeReentry, Message: "engine: loop re-entry via back-edge from " + settledID})
			continue
		}
		...
		switch cur.Status {
		case db.WorkgraphNodeStatusDone, db.WorkgraphNodeStatusFailed, db.WorkgraphNodeStatusSkipped:
			_, _ = db.UpdateWorkgraphNode(instanceID, id, db.WorkgraphNodePatch{
				Status: &pending, Outcome: &cleared, FinishedAt: &zeroTime, Assignee: &cleared})
			_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: id,
				Kind: db.WorkgraphEventNodeReentry, Message: "engine: reset for loop re-entry"})
		}
	}
}
...
// workgraphInstanceLocks serialises mutating operations on a single workgraph
...
var workgraphInstanceLocks sync.Map // int64 → *sync.Mutex

// lockWorkgraphInstance acquires the per-instance mutex and returns its unlock
// func, for use as `defer lockWorkgraphInstance(id)()`. Mutating handlers re-read
// instance/node state AFTER acquiring it, so the guards act on current state.
func lockWorkgraphInstance(id int64) func() {
	actual, _ := workgraphInstanceLocks.LoadOrStore(id, &sync.Mutex{})
...
func isManualDriveStatus(s string) bool {
	switch s {
	case db.WorkgraphNodeStatusRunning, db.WorkgraphNodeStatusDone, db.WorkgraphNodeStatusFailed:
		return true
	default:
		return false
	}
...
// (on the frontier). The caller MUST already hold lockWorkgraphInstance(instID).
...
func readyAINodeOrFail(instID int64, nodeID string) (*db.WorkgraphInstance, *db.WorkgraphNode, *spawnFailure) {
	inst, err := db.GetWorkgraphInstance(instID)
	if err != nil {
		return nil, nil, &spawnFailure{http.StatusInternalServerError, "io", "lookup: " + err.Error()}
	}
	if inst == nil {
		return nil, nil, &spawnFailure{http.StatusNotFound, "not_found", "workgraph not found"}
	}
	...
	node, err := db.GetWorkgraphNode(instID, nodeID)
	if err != nil {
	...
	if node.ExecutorKind != string(workgraph.ExecAI) {
	...
	if node.Status != db.WorkgraphNodeStatusReady {
...
// MUST already hold lockWorkgraphInstance(instID).
func reloadReadyAINode(w http.ResponseWriter, instID int64, nodeID string) (*db.WorkgraphInstance, *db.WorkgraphNode, bool) {
...
func boundGroupOrFail(inst *db.WorkgraphInstance) (*db.AgentGroup, *spawnFailure) {
	if inst.GroupID == 0 {
		return nil, &spawnFailure{http.StatusBadRequest, "invalid_arg",
			"instance has no bound group; bind one at create (POST /api/workgraphs {\"group\":\"<name>\"})"}
	}
...
func boundGroup(w http.ResponseWriter, inst *db.WorkgraphInstance) (*db.AgentGroup, bool) {
...
func spawnWorkerIntoNodeCore(instanceID int64, nodeID, seedContext string) (map[string]any, *spawnFailure) {
	...
	return map[string]any{
		"ok": true, "node_id": nodeID, "status": db.WorkgraphNodeStatusRunning,
...
func claimManualAISpawn(instanceID int64, nodeID, seedContext string) (*aiNodeClaim, *spawnFailure) {
	...
	if len(missing) > 0 {
		slog.Warn("workgraph start: node prompt has unresolved refs",
	...
	// agent-engine driver routed in (additive to the worker's own `workgraph
	// where`/`status` self-view).
	initMsg = foldSeedContext(initMsg, seedContext)
	if initMsg != "" && !isValidInitialMessage(initMsg) {
		slog.Warn("workgraph start: node brief is not a valid initial message; spawning without it",
	...
	if _, err := db.UpdateWorkgraphNode(instanceID, nodeID,
	...
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: nodeID,
		Kind: db.WorkgraphEventNodeStarted, Message: "spawning agent for ai node"})
...
// foldSeedContext appends the workgraph driver's --context seed (JOH-15 B2a) to a
...
func foldSeedContext(prompt, seed string) string {
	...
	case seed == "":
		return prompt
	case prompt == "":
		return "Context from the workgraph driver:\n\n" + seed
	default:
		return prompt + "\n\n---\nContext from the workgraph driver:\n\n" + seed
	}
}

// dashboardWorkgraphNodeStart is the dashboard HTTP wrapper over
// spawnWorkerIntoNodeCore — it maps a typed failure onto http.Error (matching the
// dashboardWorkgraphNodePatch twin) and writes the JSON result on success. The
// dashboard start button seeds no driver context (""); that channel is the
// agent-engine driver's `--context`, surfaced on the /v1 twin.
func dashboardWorkgraphNodeStart(w http.ResponseWriter, inst *db.WorkgraphInstance, nodeID string) {
...
// dashboardWorkgraphNodeAttach assigns an EXISTING member of the bound group to a
...
func dashboardWorkgraphNodeAttach(w http.ResponseWriter, r *http.Request, inst *db.WorkgraphInstance, nodeID string) {
	unlock := lockWorkgraphInstance(inst.ID)
	defer unlock()
	...
	now := time.Now()
	st := db.WorkgraphNodeStatusRunning
	asg := convID
	if _, err := db.UpdateWorkgraphNode(inst.ID, nodeID,
		db.WorkgraphNodePatch{Status: &st, Assignee: &asg, StartedAt: &now}); err != nil {
		http.Error(w, "update node: "+err.Error(), http.StatusInternalServerError)
		return
	}
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
		InstanceID: inst.ID, NodeID: nodeID, Kind: db.WorkgraphEventNodeStarted,
	...
	if tmpl, _ := rebuildInstanceTemplate(inst); tmpl != nil && tmpl.Nodes[nodeID] != nil {
		...
		case !isValidInitialMessage(prompt):
			slog.Warn("workgraph attach: node prompt is not a valid inbox message; assigned without delivering it",
				"conv", convID, "node", nodeID)
		default:
			if _, err := db.InsertAgentMessage(&db.AgentMessage{
				GroupID:      g.ID,
				ToConv:       convID,
				Subject:      "Workgraph task: " + node.Label,
				Body:         prompt,
				ToRecipients: []string{convID},
			}); err != nil {
				slog.Warn("workgraph attach: failed to deliver task to inbox",
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"ok": true, "node_id": nodeID, "status": db.WorkgraphNodeStatusRunning, "assignee": convID,
	})
}

// dashboardWorkgraphNodeApprove is the human-verify gate for a node whose
...
// (workgraph.Advance + applyWorkgraphAdvance — no duplicated frontier logic); a
...
func dashboardWorkgraphNodeApprove(w http.ResponseWriter, r *http.Request, inst *db.WorkgraphInstance, nodeID string) {
	unlock := lockWorkgraphInstance(inst.ID)
	defer unlock()
	...
	if fresh, _ := db.GetWorkgraphInstance(inst.ID); fresh != nil {
		inst = fresh
	}
	node, err := db.GetWorkgraphNode(inst.ID, nodeID)
	if err != nil {
	...
	if tmpl == nil || tmpl.Nodes[nodeID] == nil || tmpl.Nodes[nodeID].Verify.Kind != workgraph.VerifyHuman {
		http.Error(w, "node "+nodeID+" is not human-verified; the approve gate only applies to verify.kind: human",
			http.StatusBadRequest)
		return
	}
	if isSettledWorkgraphNodeStatus(node.Status) {
		http.Error(w, "node "+nodeID+" is already "+node.Status+"; cannot approve/reject a settled node",
			http.StatusConflict)
		return
	}
	if node.Status != db.WorkgraphNodeStatusRunning && node.Status != db.WorkgraphNodeStatusAwaitingVerify {
	...
	if decision == "reject" {
		...
		_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
			InstanceID: inst.ID, NodeID: nodeID, Kind: db.WorkgraphEventNodeRejected, Message: msg,
		})
		writeJSON(w, http.StatusOK, map[string]any{
	...
	st := db.WorkgraphNodeStatusDone
	out := workgraph.OutcomePass
	fin := now
	patch := db.WorkgraphNodePatch{Status: &st, Outcome: &out, FinishedAt: &fin}
	if node.StartedAt.IsZero() {
		patch.StartedAt = &now
	}
	if _, err := db.UpdateWorkgraphNode(inst.ID, nodeID, patch); err != nil {
		http.Error(w, "update node: "+err.Error(), http.StatusInternalServerError)
		return
	}
	msg := "approved"
	...
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{
		InstanceID: inst.ID, NodeID: nodeID, Kind: db.WorkgraphEventNodeApproved, Message: msg,
	})
	appendNodeStatusEvent(inst.ID, nodeID, db.WorkgraphNodeStatusDone) // node_done

	instanceRunning := inst.Status == db.WorkgraphStatusRunning
	advanced := workgraph.AdvanceResult{Ready: []string{}, Skipped: []string{}}
	newInstStatus := inst.Status
	if instanceRunning {
		advanced = workgraph.Advance(tmpl, nodeID, workgraph.OutcomePass, nodeStateMap(inst.ID))
		applyWorkgraphAdvance(inst.ID, nodeID, tmpl, advanced, now)
		nodes, _ := db.ListWorkgraphNodes(inst.ID)
		newInstStatus = recomputeWorkgraphInstanceStatus(tmpl, nodes)
		if newInstStatus != inst.Status {
			if _, err := db.UpdateWorkgraphInstanceStatus(inst.ID, newInstStatus); err == nil {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"status": db.WorkgraphNodeStatusDone, "instance_status": newInstStatus,
...
// dashboardWorkgraphCancel marks every non-terminal node skipped and the
// instance cancelled — a clean terminal snapshot for the dashboard.
func dashboardWorkgraphCancel(w http.ResponseWriter, inst *db.WorkgraphInstance) {
	if fail := cancelWorkgraphInstance(inst.ID); fail != nil {
		http.Error(w, fail.Msg, fail.Status)
		return
	}
...
// cancelWorkgraphInstance is the shared cancel core: under the per-instance lock,
...
func cancelWorkgraphInstance(instanceID int64) *spawnFailure {
	unlock := lockWorkgraphInstance(instanceID)
	defer unlock()
	nodes, _ := db.ListWorkgraphNodes(instanceID)
	now := time.Now()
	for _, n := range nodes {
		if isSettledWorkgraphNodeStatus(n.Status) {
			continue
		}
		st := db.WorkgraphNodeStatusSkipped
		fin := now
		_, _ = db.UpdateWorkgraphNode(instanceID, n.NodeID, db.WorkgraphNodePatch{Status: &st, FinishedAt: &fin})
		_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: n.NodeID, Kind: db.WorkgraphEventNodeSkipped})
	}
	if _, err := db.UpdateWorkgraphInstanceStatus(instanceID, db.WorkgraphStatusCancelled); err != nil {
		return &spawnFailure{http.StatusInternalServerError, "io", "cancel: " + err.Error()}
	}
	appendInstanceStatusEvent(instanceID, db.WorkgraphStatusCancelled)
	return nil
}

// ----- snapshot integration -------------------------------------------

// dashboardWorkgraphInstance is the snapshot row for one running/finished
// instance — enough for the Workgraphs tab to render a status card with node
// progress without a per-row detail fetch.
type dashboardWorkgraphInstance struct {
	ID           int64  `json:"id"`
	Title        string `json:"title"`
	TemplateRef  string `json:"template_ref"`
...
// dashboardWorkgraphTemplate is the snapshot row for one discoverable template —
...
type dashboardWorkgraphTemplate struct {
	Ref         string   `json:"ref"`
...
// collectWorkgraphsSnapshot builds the Workgraphs tab rows. Empty (non-nil) slice
// on error so the page's .map() is safe.
func collectWorkgraphsSnapshot() []dashboardWorkgraphInstance {
	out := []dashboardWorkgraphInstance{}
	...
	for _, inst := range insts {
		row := dashboardWorkgraphInstance{
			ID:           inst.ID,
			Title:        inst.Title,
		...
		for _, n := range nodes {
			switch n.Status {
			case db.WorkgraphNodeStatusDone:
				row.Done++
			case db.WorkgraphNodeStatusFailed:
				row.Failed++
...
// collectWorkgraphTemplatesSnapshot lists every discoverable template (project /
// user / example). Empty (non-nil) slice when none are found.
func collectWorkgraphTemplatesSnapshot() []dashboardWorkgraphTemplate {
	out := []dashboardWorkgraphTemplate{}
	for _, e := range workgraph.List(workgraphProjectDirsFn()...) {
		out = append(out, dashboardWorkgraphTemplate{
			Ref:         e.Ref,
			Name:        e.Name,
...
type workgraphEventJSON struct {
	ID      int64  `json:"id"`
...
func nodeToJSON(n *db.WorkgraphNode, tmpl *workgraph.Template) workgraphNodeJSON {
	v := workgraphNodeJSON{
		NodeID:       n.NodeID,
		Label:        n.Label,
...
func eventToJSON(e *db.WorkgraphEvent) workgraphEventJSON {
	v := workgraphEventJSON{
		ID:      e.ID,
		NodeID:  e.NodeID,
		Kind:    e.Kind,
		Message: e.Message,
...
func instanceDetailJSON(inst *db.WorkgraphInstance) map[string]any {
	m := map[string]any{
		"id":            inst.ID,
		"title":         inst.Title,
...
// instantiation).
func rebuildInstanceTemplate(inst *db.WorkgraphInstance) (*workgraph.Template, error) {
	nodes, err := db.ListWorkgraphNodes(inst.ID)
	if err != nil {
		return nil, err
	}
	defs := map[string]*workgraph.Node{}
	for _, n := range nodes {
		var def workgraph.Node
	...
	return workgraph.RebuildFromSnapshot(inst.Mermaid, defs)
}

// nodeStateMap reads the current node statuses and maps them onto the coarse
// run-states workgraph.Advance reasons over.
func nodeStateMap(instanceID int64) map[string]workgraph.NodeRunState {
	nodes, _ := db.ListWorkgraphNodes(instanceID)
	m := make(map[string]workgraph.NodeRunState, len(nodes))
	for _, n := range nodes {
		m[n.NodeID] = workgraphNodeRunState(n.Status)
	}
	return m
}

func workgraphNodeRunState(status string) workgraph.NodeRunState {
	switch status {
	case db.WorkgraphNodeStatusReady, db.WorkgraphNodeStatusRunning, db.WorkgraphNodeStatusAwaitingVerify:
		return workgraph.NodeLive
	case db.WorkgraphNodeStatusDone, db.WorkgraphNodeStatusFailed, db.WorkgraphNodeStatusSkipped:
		return workgraph.NodeSettled
	default:
		return workgraph.NodePending
	}
}

// recomputeWorkgraphInstanceStatus derives the instance status from its nodes: a
...
func recomputeWorkgraphInstanceStatus(tmpl *workgraph.Template, nodes []*db.WorkgraphNode) string {
	halted := false
	allTerminal := true
	for _, n := range nodes {
		switch n.Status {
		case db.WorkgraphNodeStatusDone, db.WorkgraphNodeStatusSkipped:
			// terminal-ok
		case db.WorkgraphNodeStatusFailed:
	...
	switch {
	case halted:
		return db.WorkgraphStatusFailed
	case allTerminal:
		return db.WorkgraphStatusCompleted
	default:
		return db.WorkgraphStatusRunning
	}
}
...
func appendNodeStatusEvent(instanceID int64, nodeID, status string) {
	...
	switch status {
	case db.WorkgraphNodeStatusRunning:
		kind = db.WorkgraphEventNodeStarted
	case db.WorkgraphNodeStatusDone:
		kind = db.WorkgraphEventNodeDone
	case db.WorkgraphNodeStatusFailed:
		kind = db.WorkgraphEventNodeFailed
	case db.WorkgraphNodeStatusSkipped:
		kind = db.WorkgraphEventNodeSkipped
	case db.WorkgraphNodeStatusReady:
		kind = db.WorkgraphEventNodeReady
	default:
		return // pending / awaiting_verify carry no well-known event kind
	}
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, NodeID: nodeID, Kind: kind})
}

func appendInstanceStatusEvent(instanceID int64, status string) {
	_, _ = db.AppendWorkgraphEvent(&db.WorkgraphEvent{InstanceID: instanceID, Kind: "instance_" + status})
}

func validWorkgraphNodeStatus(s string) bool {
	switch s {
	case db.WorkgraphNodeStatusPending, db.WorkgraphNodeStatusReady, db.WorkgraphNodeStatusRunning,
		db.WorkgraphNodeStatusAwaitingVerify, db.WorkgraphNodeStatusDone,
		db.WorkgraphNodeStatusFailed, db.WorkgraphNodeStatusSkipped:
		return true
	default:
		return false
	}
...
func isSettledWorkgraphNodeStatus(s string) bool {
	switch s {
	case db.WorkgraphNodeStatusDone, db.WorkgraphNodeStatusFailed, db.WorkgraphNodeStatusSkipped:
		return true
	default:
...
func isEnumNode(tmpl *workgraph.Template, nodeID string) bool {
	n := tmpl.Nodes[nodeID]
	return n != nil && n.Verify.Kind == workgraph.VerifyEnum
}

func sortedTemplateNodeIDs(tmpl *workgraph.Template) []string {
