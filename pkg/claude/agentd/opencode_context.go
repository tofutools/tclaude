...
func openCodeModelLimits(ctx context.Context, runtime db.OpenCodeRuntime) map[string]int64 {
	...
	if err != nil {
		...
		if prior := openCodeLimitCache.byServer[runtime.ServerURL]; len(prior.limits) > 0 {
			return prior.limits
		}
		openCodeLimitCache.byServer[runtime.ServerURL] = openCodeLimitEntry{at: time.Now(), ttl: openCodeLimitFailureBackoff}
		return nil
	}

	openCodeLimitCache.byServer[runtime.ServerURL] = openCodeLimitEntry{limits: limits, at: time.Now(), ttl: openCodeLimitCacheTTL}
	return limits
}
...
func fetchOpenCodeModelLimits(ctx context.Context, runtime db.OpenCodeRuntime) (map[string]int64, error) {
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("OpenCode /config/providers returned HTTP %d", response.StatusCode)
	}
	var payload struct {
		Providers []struct {
			ID     string `json:"id"`
	...
	for _, provider := range payload.Providers {
		...
		for modelID, model := range provider.Models {
			if model.Limit.Context > 0 {
				limits[provider.ID+"/"+modelID] = model.Limit.Context
			}
		}
	}
	return limits, nil
