...
func TestDashboardHTML_DockPalette(t *testing.T) {
	...
	must(`popover="auto"`,
		"the re-homed selectors use a browser-contained top-layer popover instead of a native menu")
	must(`placement: 'bottom-end'`,
		"the selector popup aligns inward from the dock's right edge")
