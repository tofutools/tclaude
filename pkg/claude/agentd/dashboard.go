...
type snapshotPayload struct {
	...
	// Workgraphs are the running/finished workgraph instances rendered in
	// the Workgraphs tab, each with node-progress counts. Empty slice (not
	// nil) so JS .map() is safe.
	Workgraphs []dashboardWorkgraphInstance `json:"workgraphs"`
	// WorkgraphTemplates are the discoverable workgraph templates (project /
	// user / example) offered for instantiation. Empty slice (not nil).
	WorkgraphTemplates []dashboardWorkgraphTemplate `json:"workgraph_templates"`
...
func handleDashboardSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	out.Workgraphs = collectWorkgraphsSnapshot()
	out.WorkgraphTemplates = collectWorkgraphTemplatesSnapshot()
