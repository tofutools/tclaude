...
// TestDashboardProcessSnippetNameInputChrome is the focused TCL-570 visual
// proof. The state exercises the real dialog and browser input engine in both
// skins, with computed-style and contrast checks before each capture.
func TestDashboardProcessSnippetNameInputChrome(t *testing.T) {
	if os.Getenv("TCLAUDE_DASHSNAP") == "" {
		t.Skip("browser smoke — set TCLAUDE_DASHSNAP=1 (needs local Chrome)")
	}
	...
	var base dashsnap.State
	for _, state := range baseStates() {
		if state.Key == "process-editor-snippet-name-input" {
			base = state
			break
		}
	}
	if base.Key == "" {
		t.Fatal("snippet name input state missing from the dashboard smoke matrix")
	}
	regular := base
	regular.Key = "default-" + base.Key
	wizard := base
	...
	outDir := filepath.Join(dashSnapOutRoot(t), "process-editor-snippet-name-"+time.Now().Format("20060102-150405.000"))
	shots, err := dashsnap.Capture(dashsnap.Config{
		BaseURL: srv.URL,
		OutDir:  outDir,
		Width:   1680,
	...
	if len(failed) != 0 {
		t.Fatalf("process snippet name browser smoke failed:\n%s\ncontact sheet: %s",
			strings.Join(failed, "\n"), filepath.Join(outDir, "index.html"))
	}
	t.Logf("process snippet name browser smoke: %s", filepath.Join(outDir, "index.html"))
}
