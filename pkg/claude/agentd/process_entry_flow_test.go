...
func TestProcessRuntimeDirectEntryKeepsUnsupportedShapesRefused(t *testing.T) {
	...
	// A bounded fresh-attempt retry is executable; the backoff wait between
	// attempts is not, and a direct parallel entry must not relax that.
	join.Retry = &model.RetryPolicy{MaxAttempts: 2, Backoff: "30s"}
	...
	assert.Contains(t, refused.Body.String(), "nodes.join.retry.backoff")
	assert.Contains(t, refused.Body.String(), "retry backoff waits are not executable in this engine yet")
