...
// workflow's data-flow is visible agent-to-agent traffic over tclaude's inbox.
...
// workflow_events audit marker, and the TmuxSim nudge.
...
func settleDoneWithOutput(t *testing.T, id int64, nodeID, output, outcome string) {
	...
	_, err := db.UpdateWorkflowNode(id, nodeID,
		db.WorkflowNodePatch{Status: &done, Output: &output, Outcome: &outcome})
	require.NoError(t, err)
}
...
// nudges its pane.
func TestWorkflowEngine_HandoffDeliveredToBoundSuccessor(t *testing.T) {
	...
	h := msgs[0]
	assert.Equal(t, agentd.WorkflowEngineAssigneeForTest(), h.FromConv, "handoff is from the workflow-engine sentinel")
	...
	events, err := db.ListWorkflowEvents(id, "b")
	require.NoError(t, err)
	var marked bool
...
func TestWorkflowEngine_HandoffJoinDeliversPerPredecessor(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
...
// suppresses a resend.
func TestWorkflowEngine_HandoffIdempotentAcrossTicks(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
...
func TestWorkflowEngine_HandoffDeferredUntilSuccessorBound(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
	...
	_, err := db.UpdateWorkflowNode(id, "b", db.WorkflowNodePatch{Status: &ready})
	require.NoError(t, err)
...
func TestWorkflowEngine_HandoffSuppressedWhenEngineOff(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
...
func TestWorkflowEngine_HandoffOnlyAlongTakenEdge(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
...
func TestWorkflowEngine_HandoffSkippedPredecessorSilent(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkflowEngineEnabledForTest(true))
	...
	skipped := db.WorkflowNodeStatusSkipped
	_, err := db.UpdateWorkflowNode(id, "dead", db.WorkflowNodePatch{Status: &skipped})
	require.NoError(t, err)
