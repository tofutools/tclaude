...
// workgraph_events audit trail, and the instance status.
...
func countEvents(t *testing.T, id int64, nodeID, kind string) int {
	t.Helper()
	events, err := db.ListWorkgraphEvents(id, nodeID)
...
func TestWorkgraphEngine_InPlaceRetryExhausts(t *testing.T) {
	...
	agentd.RunWorkgraphEngineTickForTest() // attempt 2
	agentd.RunWorkgraphEngineTickForTest() // attempt 3 → budget spent → failed

	st, v = nodeStatusVisits(t, id, "a")
	assert.Equal(t, "failed", st, "node fails after retries are exhausted")
	assert.Equal(t, int64(3), v, "1 initial + 2 retries = 3 executions")
	assert.Equal(t, 2, countEvents(t, id, "a", db.WorkgraphEventNodeRetry), "two in-place retries recorded")
	d := wfGet(t, mux, id)
...
func TestWorkgraphEngine_InPlaceRetrySucceeds(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	agentd.RunWorkgraphEngineTickForTest() // attempt 1 fails → retry
	agentd.RunWorkgraphEngineTickForTest() // attempt 2 passes → done → readies `done`
	agentd.RunWorkgraphEngineTickForTest() // `done` runs → instance completes

	st, v := nodeStatusVisits(t, id, "a")
	assert.Equal(t, "done", st, "node settles done once verify passes")
	assert.Equal(t, int64(2), v, "passed on the 2nd attempt")
	assert.Equal(t, 1, countEvents(t, id, "a", db.WorkgraphEventNodeRetry), "one retry before success")
	d := wfGet(t, mux, id)
...
// completes.
func TestWorkgraphEngine_FixLoopConverges(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	assert.GreaterOrEqual(t, countEvents(t, id, "impl", db.WorkgraphEventNodeReentry), 1,
...
// spins forever.
func TestWorkgraphEngine_MaxVisitsBreaksRunawayLoop(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	events, err := db.ListWorkgraphEvents(id, "impl")
	require.NoError(t, err)
	var sawCap bool
	for _, e := range events {
		if e.Kind == db.WorkgraphEventNodeFailed && strings.Contains(e.Message, "max_visits") {
			sawCap = true
		}
	}
...
// instance.
func TestWorkgraphEngine_AINodeRetryReArmsOnFail(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	n, _ := db.GetWorkgraphNode(id, "work")
	assert.Empty(t, n.Assignee, "the assignee is cleared so the engine re-spawns a fresh agent")
	assert.Equal(t, 1, countEvents(t, id, "work", db.WorkgraphEventNodeRetry), "a retry was recorded")
...
func TestWorkgraphEngine_LoopBackClearsAssigneeForReSpawn(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	_, err := db.UpdateWorkgraphNode(id, "check", db.WorkgraphNodePatch{Status: &ready})
	require.NoError(t, err)
	asg := conv
	_, err = db.UpdateWorkgraphNode(id, "worker", db.WorkgraphNodePatch{Assignee: &asg})
	require.NoError(t, err)

	// One tick: check runs (tool) → verify fails → |fail| loop-back re-arms worker.
	agentd.RunWorkgraphEngineTickForTest()

	w, err := db.GetWorkgraphNode(id, "worker")
	require.NoError(t, err)
	...
	assert.GreaterOrEqual(t, countEvents(t, id, "worker", db.WorkgraphEventNodeReentry), 1,
