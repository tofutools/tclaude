...
// The Workflows tab is a front-end-only feature (Step 5): a new tab in
// the dashboard SPA that consumes the workflows snapshot + /api/workflows
...
// TestDashboardHTML_WorkflowsTabWired pins the page-shell wiring: the nav
...
func TestDashboardHTML_WorkflowsTabWired(t *testing.T) {
	...
	for _, needle := range []string{
		...
		if !strings.Contains(html, needle) {
			t.Errorf("dashboard.html missing %q — Workflows tab not wired into the shell", needle)
		}
	}
...
// TestDashboardAssets_WorkflowsJSWired guards the JS wiring across the
// module split: the workflows module exports its render + bind hooks,
// the refresh loop and bootstrap call them, and the module talks to the
// real /api/workflows surface + window.mermaid. Asserting on the
// embedded concatenation catches a rename in any one file at `go test`.
func TestDashboardAssets_WorkflowsJSWired(t *testing.T) {
	for _, needle := range []string{
		"function renderWorkflowsTab(",       // defined in workflows.js
		"function bindWorkflowsUI(",          // defined in workflows.js
		"renderWorkflowsTab, bindWorkflowsUI", // exported
		"renderWorkflowsTab();",              // called by refresh.js poll
		"bindWorkflowsUI();",                 // called by dashboard.js bootstrap
		"window.mermaid",                     // consumes the vendored global
		"/api/workflows",                     // hits the backend surface
		"resolveBoundMember(",                // vitals overlay match logic
	} {
		if !strings.Contains(dashboardAssets, needle) {
			t.Errorf("dashboard JS missing %q — Workflows tab wiring broken", needle)
		}
	}
}
