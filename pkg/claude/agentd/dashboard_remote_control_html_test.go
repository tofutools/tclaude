...
// surfaces (JOH-259): the at-a-glance "📱 remote" badge, the ⚙-menu toggle
// gated on the harness's Remote Access capability, and the click handler that
// POSTs the intent to the dashboard route. The pieces span four files
// (helpers.js builds + exports the badge/menu-item/capability helper, render.js
// wires them into the member cell, row-actions.js dispatches the toggle,
// dashboard.css styles the badge); a rename in one silently breaks the feature
// in the browser, and the repo has no JS test runner, so this asserts on the
// embedded concatenation at `go test ./...` — the same guard style as the
...
func TestDashboardHTML_RemoteControlWired(t *testing.T) {
	...
	// helpers.js: the badge reads the best-known state off the agent and is
	// exported for render.js. It shows ONLY when remote control is on.
	must("function remoteControlBadge(m)", "remoteControlBadge helper is defined")
	must("m.state.remote_control", "the badge reads the best-known flag off the agent's state")
	...
	// threaded into the action builders; the badge renders beside the sandbox
	// chip in the agent control cell.
	...
	must("${sandboxBadge(m)}${remoteControlBadge(m)}", "the remote badge renders beside the sandbox chip")
	...
	// CSS: the badge has a style rule.
	must(".remote-badge", "the remote badge has a style rule")
}
