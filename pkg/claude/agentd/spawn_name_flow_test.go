...
func TestSpawn_InvalidNameRejected(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Asserted in a plain loop rather than t.Run subtests: synctest
		// disallows t.Run inside a bubble, and these cases share one flow
		// and only differ by input, so the case label rides in the
		...
		for label, name := range bad {
			...
			assert.Equalf(t, http.StatusBadRequest, resp.Code,
				"[%s] spawn with name %q should 400; body=%s", label, name, resp.Raw)
			assert.Containsf(t, string(resp.Raw), "A-Za-z0-9_-",
				"[%s] the error should name the allowed charset; got %s", label, resp.Raw)
		}
	})
}
