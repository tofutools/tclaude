  ...
  return html`
    ...
    <div class="cfg-section">
      ...
      <div class="cfg-field">
        ...
        <span class="cfg-hint">The spawn dialog's <strong>Auto focus</strong>, per-agent <strong>focus</strong> (👁), <strong>open window</strong> and <strong>open terminal</strong> actions — plus bulk focus from the 🪟 <em>windows…</em> modal, the same commands in the ⌘ palette, a click on a CWD path cell, and a message's <em>focus</em> button — open <em>in-browser</em> terminal panes in the dashboard's <strong>Terminals</strong> tab by default, so you never leave the browser. Bulk unfocus still detaches the selected terminal clients. The dedicated web buttons are always web. <strong>Uncheck</strong> to make those general actions pop native OS terminal windows instead. Stored as <code>dashboard.default_terminal</code> (the default <code>web</code> is omitted; unchecked writes <code>native</code>).</span>
      </div>
      <div class="cfg-field">
    ...
    <div class="cfg-section">
      ...
      <div class="cfg-field">
        ...
        <span class="cfg-hint">Directory fields open an in-browser navigator by default, both locally and remotely. <strong>Uncheck</strong> to use the native OS chooser for loopback/localhost dashboards; remote browsers still use the web picker because a native chooser would appear on the agentd host where it cannot be operated. Stored as <code>dashboard.default_directory_picker</code> (the default <code>web</code> is omitted; unchecked writes <code>native</code>).</span>
      </div>
      <div class="cfg-field">
    ...
    <div class="cfg-section">
      ...
      <div class="cfg-field">
        ...
        <span class="cfg-hint">Off by default while this interaction is refined. <strong>Float</strong> shows the paperclip overlay only while the title is hovered. <strong>Fixed</strong> keeps the paperclip as the last group quick item, marks a saved link with a small dot while its label is folded, and folds that label with the other quick details; only its edit pencil appears on hover. Turning the feature off only hides the controls—existing stored attachments remain intact. Takes effect on the next dashboard refresh. Stored as <code>features.group_attachments</code>.</span>
      </div>
      <div class="cfg-field">
