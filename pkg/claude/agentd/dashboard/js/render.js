...
function renderRealGroup(g, childrenHTML) {
    ...
    <details data-group-key="${esc(g.name)}" data-dnd-target-group="${esc(g.name)}"${detailsClassAttr}${isOpen ? ' open' : ''}>
      <summary draggable="true" data-group-reorder="${esc(g.name)}" title="Drag this header to reorder the group">
        ...
        ${g.virtual ? '' : `<label class="sandbox-profile-quick group-sandbox-profile-quick" tabindex="0" aria-label="Choose sandbox profile for ${esc(g.name)}" title="Sandbox profile for ${esc(g.name)} — composes after the global sandbox profile for newly launched agents."><span aria-hidden="true">🛡</span><select data-sandbox-profile-quick-group="${esc(g.name)}" data-current="${esc(g.sandbox_profile || '')}" aria-label="Sandbox profile for ${esc(g.name)}">${sandboxProfileOptionsHTML(g.sandbox_profile || '', 'inherit')}</select></label>`}
