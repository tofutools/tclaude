...
const NETWORK_ACCESS_HELP = 'Choose Allow or Deny independently for built-in packs and manual '
  ...
  + 'class. For tclaude’s sandbox on Linux filtered networking: '
...
const TMPFS_HELP = 'Empty writable filesystems that exist only for the life of the launch and are '
  ...
  + 'filesystem rule beneath it still applies. tclaude’s sandbox on Linux only: a tmpfs is a mount, so '
...
export function SandboxPolicyResult({ target, context, contextIndex, contexts = [] }) {
  ...
  return html`<div class="sbx-policy-result">
    ...
    : 'Stored in this effective policy, but it does not apply to this target; only the tclaude’s sandbox on macOS consumes it.'}</div>
    </div>`}
...
function NetworkAccessEditor({ draft, setDraft, catalog, newDraft, packVisibilityError, packVisibilityAttention, retryPackCatalog, packCatalogBusy }) {
  ...
  return html`<${SandboxSection} id="sandbox-profile-editor-network-section" className="sbx-access-axis"
    ...
    ${rules.namespace === 'private' && html`<p class="sbx-inline-note sbx-network-namespace-note"><strong>Private, routed:</strong> internet traffic is routed normally, while host localhost services, IDE bridges, and abstract Unix sockets are not shared. tclaude’s sandbox on Linux only.</p>`}
...
function SandboxTmpfsEditor({ mounts, setDraft, validation }) {
  ...
  return html`<div class="sbx-tmpfs-intro"><strong>tclaude’s sandbox on Linux only.</strong> Each mount is empty at launch, writable, and discarded when the agent exits. Nothing on the host is exposed by it.</div>
