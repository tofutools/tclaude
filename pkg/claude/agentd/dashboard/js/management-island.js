...
import { AUTO_MEMORY_TRI_OPTIONS, PEER_MESSAGING_TRI_OPTIONS, CODEX_APP_SERVER_TRI_OPTIONS, COPILOT_API_TRI_OPTIONS, FAST_MODE_TRI_OPTIONS, dirtyDraft, harnessByName, harnessDefaults, profileDraft, profileHarnessDefaults, profilePayload, readTri, roleDraft, rolePayload, TRI_OPTIONS } from './management-model.js';
...
import {
  sandboxTmpfsEditorRows,
  sandboxTmpfsForWire,
  sandboxTmpfsNewEditorRow,
...
const SANDBOX_IMPL_TITLE = 'Which sandbox provides OS-level containment for agents launched from this '
  + 'profile. harness-builtin is offered only when the selected harness owns a real OS sandbox. '
  + "tclaude’s built-in sandbox runs the "
...
const PROFILE_ENVIRONMENT_HELP = 'Applied to every fresh spawn resolving this profile. '
  + 'Per-spawn values override matching names.';
const PROFILE_ENVIRONMENT_HELP_WIZARD = 'Bound to every familiar summoned from this pattern. '
  + 'Per-summon runes override matching names.';
const NETWORK_ACCESS_HELP = 'Choose Allow or Deny independently for built-in packs and manual '
  ...
  + 'class. For filtered networking with tclaude’s sandbox on Linux: '
...
const FILESYSTEM_HELP = 'Path grants widen the sandbox. A row names a directory or a single '
  + 'regular file — a file row reopens exactly that one path (say ~/.gitconfig beneath a denied '
  + 'home) and needs tclaude’s built-in sandbox on Linux; a deny row must name a '
  + 'directory. Included and assignment-layer rules '
...
const TMPFS_HELP = 'Empty writable filesystems that exist only for the life of the launch and are '
  + 'backed by no host directory at all — scratch space, a throwaway build tree, a node_modules that '
  + 'must never touch the host. A mount hides whatever was at that sandbox path, and a narrower '
  + 'filesystem rule beneath it still applies. Requires tclaude’s sandbox on Linux: a tmpfs is a mount, so '
  + 'Seatbelt, harness-builtin, resource-only/off and stacked all refuse the launch rather than '
  + 'starting without it. Leave the size blank for the kernel default of half of host RAM — worth '
  + 'setting, because an agent can fill an uncapped one.';
...
function sandboxEvaluationImplementations(harness, platform, catalog) {
  ...
  if (entry.can_tclaude_layer) options.push(['tclaude-layer', 'tclaude’s built-in sandbox']);
  if (entry.can_stacked && platform === 'linux') options.push(['stacked', `tclaude + ${entry.display_name || harness} sandboxes`]);
  return options;
}
...
export function SandboxPolicyResult({ target, context, contextIndex, contexts = [] }) {
  ...
  return html`<div class="sbx-policy-result">
    ...
    ? 'Allowed by tclaude’s sandbox on macOS for this target.'
    : 'Stored in this effective policy, but it does not apply to this target; only tclaude’s sandbox on macOS consumes it.'}</div>
    </div>`}
...
function NetworkAccessEditor({ draft, setDraft, catalog, newDraft, packVisibilityError, packVisibilityAttention, retryPackCatalog, packCatalogBusy }) {
  ...
  return html`<${SandboxSection} id="sandbox-profile-editor-network-section" className="sbx-access-axis"
    ...
    ${rules.namespace === 'private' && html`<p class="sbx-inline-note sbx-network-namespace-note"><strong>Private, routed:</strong> internet traffic is routed normally, while host localhost services, IDE bridges, and abstract Unix sockets are not shared. Requires tclaude’s sandbox on Linux.</p>`}
  ...
  const updateHarness = (harness) => {
    ...
    setDraft((current) => {
      ...
      return {
        ...
        trust_dir: '', remote_control: '', auto_memory: '', peer_messaging: '', copilot_api: '', codex_app_server: '', fast_mode: '',
...
function ProfileEditor({ descriptor, roles = [], state, actions, confirmDiscard, openProfilePermissions, openProfileContextFeatures }) {
  ...
  const [draft, setDraft] = useState(() => clone(baseline));
	const setEnvironment = (index, patch) => setDraft((value) => ({ ...value, environment: value.environment.map((row, rowIndex) => rowIndex === index ? { ...row, ...patch } : row) }));
  ...
  return html`<${Overlay} id="profile-editor-modal" labelledby="profile-editor-title" onClose=${state.closeDialog} onSubmitHotkey=${saving ? null : submit} dirty=${dirty} blocked=${saving} confirmDiscard=${confirmDiscard} registerClose=${registerClose}><h3 id="profile-editor-title">${local ? wizWord('Custom launch — this agent only', 'Bespoke summons — this familiar only') : options.cloneSourceName ? wizWord(`Clone profile: ${options.cloneSourceName}`, `Mirror pattern: ${options.cloneSourceName}`) : seed && options.editExisting !== false ? wizWord(`Edit profile: ${seed.name}`, `Edit pattern: ${seed.name}`) : wizWord('New spawn profile', 'New familiar pattern')}</h3>
    ...
    <${Row} label="Peer messaging" hidden=${hEntry && !hEntry.can_peer_messaging} title="Claude Code's built-in cross-session messaging (the ListAgents and SendMessage tools over a per-session socket). tclaude closes it by default: it is a second coordination channel outside tclaude's groups, permissions and audit trail — use tclaude agent send instead. In-harness subagents are unaffected; the deny names ListAgents, never SendMessage."><${Select} id="profile-editor-peer-messaging" value=${draft.peer_messaging} onChange=${(value) => change(setDraft, 'peer_messaging', value)} options=${PEER_MESSAGING_TRI_OPTIONS}/></${Row}>
    ...
    <${Row} label="Automatically sync network rules" title="Linux tclaude packet sandbox only. Follows includes and group/global composition; filesystem changes require restart."><select id="profile-editor-network-auto-sync" value=${draft.network_auto_sync} onChange=${(event) => change(setDraft, 'network_auto_sync', event.currentTarget.value)}><option value="">Inherit (default off)</option><option value="1">On</option><option value="0">Off</option></select></${Row}>
    <${Row} label="SSH workaround" hidden=${!hEntry?.can_ssh_workaround} title="Use an agent-owned SSH client config when the launch needs it. It activates only for the Codex managed sandbox or tclaude packet filtering, and overrides Git core.sshCommand."><input id="profile-editor-ssh-workaround" type="checkbox" checked=${draft.ssh_workaround} onChange=${(event) => change(setDraft, 'ssh_workaround', event.currentTarget.checked)} /></${Row}>
    ...
    <${Row} label="Profile context" title="Extra startup guidance delivered to every agent that resolves this profile. It is kept separate from the per-spawn task brief and group context."><textarea id="profile-editor-startup-context" value=${draft.startup_context} onInput=${(event) => change(setDraft, 'startup_context', event.currentTarget.value)} rows="5" placeholder="optional — model/profile-specific guidance injected into every spawn" spellcheck="false" /></${Row}>
		<div class="cron-create-row" id="profile-editor-environment-row"><span class="cron-create-label"><${Words} plain="Environment" wizard="Summoning runes" /></span><div class="cron-create-target"><div class="sbx-rows">${draft.environment.map((row, index) => html`<div key=${index} class="sbx-row sbx-environment-row"><input class="sbx-env-name" value=${row.name} placeholder="NAME" onInput=${(event) => setEnvironment(index, { name: event.currentTarget.value })}/><input class="sbx-env-value" value=${row.value} placeholder="value" onInput=${(event) => setEnvironment(index, { value: event.currentTarget.value })}/><button type="button" onClick=${() => change(setDraft, 'environment', draft.environment.filter((_, rowIndex) => rowIndex !== index))}>×</button></div>`)}</div><div class="sbx-add-row-help"><button type="button" class="sbx-add-row" onClick=${() => change(setDraft, 'environment', [...draft.environment, { name: '', value: '' }])}><${Words} plain="＋ add variable" wizard="✦ bind rune" /></button><${HelpDisclosure} id="profile-editor-environment-help" label=${wizWord('Profile environment variables', 'Pattern runes')} help=${wizWord(PROFILE_ENVIRONMENT_HELP, PROFILE_ENVIRONMENT_HELP_WIZARD)} open=${environmentHelpOpen === 'profile-editor-environment-help'} setOpen=${setEnvironmentHelpOpen} /></div></div></div>
    ${[['Sync worktree', 'sync_worktree'], ['Fetch latest worktree base', 'fetch_latest_worktree'], ['Auto focus', 'auto_focus'], ['Group context', 'include_group_default_context'], ['Group owner', 'is_owner']].map(([label, key]) => html`<${Row} key=${key} label=${label} hidden=${local && key !== 'is_owner'}><${Select} id=${key === 'is_owner' ? 'profile-editor-owner' : `profile-editor-${key.replaceAll('_', '-')}`} value=${draft[key]} onChange=${(value) => change(setDraft, key, value)} options=${TRI_OPTIONS}/></${Row}>`)}
...
function SandboxTmpfsEditor({ mounts, setDraft, validation }) {
  const rows = mounts || [];
  const update = (index, patch) => setDraft((draft) => ({
    ...draft,
    tmpfs: (draft.tmpfs || []).map((row, i) => i === index ? { ...row, ...patch } : row),
  }));
  const remove = (index) => setDraft((draft) => ({
    ...draft,
    tmpfs: (draft.tmpfs || []).filter((_, i) => i !== index),
  }));
  return html`<div class="sbx-tmpfs-intro"><strong>Requires tclaude’s sandbox on Linux.</strong> Each mount is empty at launch, writable, and discarded when the agent exits. Nothing on the host is exposed by it.</div>
    ${validation.profile.map((error, index) => html`<div key=${index} class="sbx-tmpfs-error" role="alert">${error}</div>`)}
    <div class="sbx-tmpfs-list">${rows.map((row, index) => {
    const errors = validation.mounts[index] || { path: [], size: [] };
    const pathErrorID = `sandbox-profile-editor-tmpfs-path-error-${index}`;
    const sizeErrorID = `sandbox-profile-editor-tmpfs-size-error-${index}`;
    return html`<div key=${row._editor_id || index} role="group"
      aria-label=${`Temporary filesystem ${index + 1}: ${(row.path || '').trim() || 'no path'}`}
      class=${`sbx-row sbx-tmpfs-row${[...errors.path, ...errors.size].length ? ' is-invalid' : ''}`}>
      ${/* No Browse… button, deliberately: this is a SANDBOX path, and a
           directory picker would invite the operator to choose a host
           directory whose contents the mount then hides rather than exposes. */ ''}
      <input class="sbx-path sbx-tmpfs-path" value=${row.path || ''} placeholder="/scratch"
        autocomplete="off" spellcheck="false"
        aria-label=${`Temporary filesystem ${index + 1} sandbox path`}
        aria-invalid=${errors.path.length ? 'true' : null}
        aria-describedby=${errors.path.length ? pathErrorID : null}
        onInput=${(event) => update(index, { path: event.currentTarget.value })}/>
      <input class="sbx-tmpfs-size" value=${row.size || ''} placeholder="size (default: ½ RAM)"
        autocomplete="off" spellcheck="false"
        aria-label=${`Temporary filesystem ${index + 1} size limit`}
        aria-invalid=${errors.size.length ? 'true' : null}
        aria-describedby=${errors.size.length ? sizeErrorID : null}
        onInput=${(event) => update(index, { size: event.currentTarget.value })}/>
      <button type="button" aria-label=${`Remove temporary filesystem ${index + 1}`}
        onClick=${() => remove(index)}>×</button>
    </div>
    ${errors.path.length > 0 && html`<div key=${`path-error-${index}`} id=${pathErrorID} class="sbx-tmpfs-error" role="alert">${errors.path.join(' ')}</div>`}
    ${errors.size.length > 0 && html`<div key=${`size-error-${index}`} id=${sizeErrorID} class="sbx-tmpfs-error" role="alert">${errors.size.join(' ')}</div>`}`;
  })}</div>
    <button type="button" class="sbx-add-row sbx-tmpfs-add" disabled=${rows.length >= 32}
      onClick=${() => setDraft((draft) => ({ ...draft, tmpfs: [
        ...(draft.tmpfs || []), sandboxTmpfsNewEditorRow(),
      ] }))}>＋ add temporary filesystem</button>`;
}
...
function SandboxEditor({ descriptor, sandboxProfiles, state, actions, confirmDiscard }) {
  ...
  /* Whether the operator actually EDITED these raw axes, which the text alone
     cannot answer. Both fields use "absent key means leave it alone, explicit
     [] means clear it", and both render as `[]` for a profile that never had
     one — so without a touched bit, typing a row and deliberately deleting it
     again is indistinguishable from never opening the panel, and the explicit
     clear silently degrades to "leave alone". That is only observable when
     another client added a row since load, which is exactly the case the
     tri-state exists for. Reset whenever the panel re-renders from the draft. */
  const [rawPreLaunchTouched, setRawPreLaunchTouched] = useState(false);
  const [rawTmpfsTouched, setRawTmpfsTouched] = useState(false);
  ...
  useEffect(() => { let active = true; fetch('/api/sandbox-profiles?network_sync_status=1').then(r => r.json()).then(rows => { if (active && Array.isArray(rows)) setSyncCount(rows.filter(row => row.profile_ids?.includes(seed?.id)).length); }).catch(() => {}); return () => { active = false; }; }, [seed?.id]);
  ...
  return { filesystem, filesystem_spellings, environment, includes, agent_directories, ...(!rawPreLaunchTouched && baseline.pre_launch === undefined && draft.pre_launch === undefined && pre_launch.length === 0 ? {} : { pre_launch }), ...(!rawTmpfsTouched && baseline.tmpfs === undefined && draft.tmpfs === undefined && tmpfs.length === 0 ? {} : { tmpfs }), network: axes.network, unix_sockets: axes.unix_sockets, resource_limits: sandboxResourceLimitsForWire(resource_limits) }; };
  const rawDraftForWire = (parsed) => { const next = { ...draft, ...parsed }; if (parsed.pre_launch === undefined) delete next.pre_launch; else next.pre_launch = sandboxPreLaunchForWire(parsed.pre_launch); if (parsed.tmpfs === undefined) delete next.tmpfs; else next.tmpfs = sandboxTmpfsForWire(parsed.tmpfs); return next; };
  const applyRaw = () => { try { const parsed = parseRaw(); if (parsed.network.engine !== draft.network.engine || parsed.network.namespace !== draft.network.namespace || parsed.unix_sockets.mode !== draft.unix_sockets.mode) socketGeneratedAlignment.current = null; setDraft((value) => { const next = { ...value, ...parsed, filesystem: sandboxFilesystemEditorRows(parsed.filesystem, parsed.filesystem_spellings) }; if (parsed.pre_launch === undefined) delete next.pre_launch; else next.pre_launch = sandboxPreLaunchEditorRows(parsed.pre_launch, value.pre_launch); if (parsed.tmpfs === undefined) delete next.tmpfs; else next.tmpfs = sandboxTmpfsEditorRows(parsed.tmpfs, value.tmpfs); return next; }); state.error.value = ''; return true; } catch (error) { state.error.value = error.message || String(error); return false; } };
  const toggleAdvanced = () => { if (advanced && !applyRaw()) return; if (!advanced) { const wire = sandboxFilesystemWire(draft, baseline); setRawFS(JSON.stringify(wire.filesystem, null, 2)); setRawSpellings(JSON.stringify(wire.filesystem_spellings, null, 2)); setRawEnv(JSON.stringify(draft.environment, null, 2)); setRawIncludes(JSON.stringify(draft.includes, null, 2)); setRawAgentDirs(JSON.stringify(draft.agent_directories, null, 2)); setRawNetwork(JSON.stringify(draft.network, null, 2)); setRawSockets(JSON.stringify(draft.unix_sockets, null, 2)); setRawResources(JSON.stringify(sandboxResourceLimitsForWire(draft.resource_limits), null, 2)); setRawPreLaunch(JSON.stringify(sandboxPreLaunchForWire(draft.pre_launch || []), null, 2)); setRawTmpfs(JSON.stringify(sandboxTmpfsForWire(draft.tmpfs || []), null, 2)); setRawPreLaunchTouched(false); setRawTmpfsTouched(false); } setAdvanced(!advanced); };
  const submit = async () => {
    let value = { ...draft, ...sandboxFilesystemWire(draft, baseline), resource_limits: sandboxResourceLimitsForWire(draft.resource_limits), ...(draft.pre_launch === undefined ? {} : { pre_launch: sandboxPreLaunchForWire(draft.pre_launch) }), ...(draft.tmpfs === undefined ? {} : { tmpfs: sandboxTmpfsForWire(draft.tmpfs) }) };
    if (advanced) { try { value = rawDraftForWire(parseRaw()); } catch (error) { state.error.value = error.message || String(error); return; } }
    await actions.saveSandbox({ draft: value, original: seed, options: { ...options, syncRunning } });
  };
  useEffect(() => {
  ...
  let predictionDraft = { ...draft, ...sandboxFilesystemWire(draft, baseline), resource_limits: sandboxResourceLimitsForWire(draft.resource_limits), ...(draft.pre_launch === undefined ? {} : { pre_launch: sandboxPreLaunchForWire(draft.pre_launch) }), ...(draft.tmpfs === undefined ? {} : { tmpfs: sandboxTmpfsForWire(draft.tmpfs) }) };
  ...
  // Validated against the filesystem rows as authored, so the one cross-field
  // conflict the daemon refuses — a sandbox path claimed by both a tmpfs and a
  // filesystem rule — is reported on the row that causes it.
  const tmpfsValidation = sandboxTmpfsValidation(
    sandboxTmpfsForWire(draft.tmpfs || []), sandboxFilesystemWire(draft, baseline).filesystem,
  );
  ...
  const predictionAccessErrors = predictionDraftError ? [] : advanced
    ? [
      ...
      ...sandboxTmpfsValidation(predictionDraft.tmpfs || [], predictionDraft.filesystem || []).errors,
      ...(networkPackVisibilityError ? [networkPackVisibilityError] : []),
    ]
    : [
      ...accessErrors, ...preLaunchValidation.errors, ...tmpfsValidation.errors,
      ...(networkPackVisibilityError ? [networkPackVisibilityError] : []),
    ];
  ...
  const configureWithAgent = () => {
    ...
    const handoffErrors = [
      ...
      ...sandboxTmpfsValidation(value.tmpfs || [], value.filesystem || []).errors,
      ...(networkPackVisibilityError ? [networkPackVisibilityError] : []),
    ];
  ...
  return html`<${Overlay} id="sandbox-profile-editor-modal" labelledby="sandbox-profile-editor-title" onClose=${state.closeDialog} onSubmitHotkey=${submitBlocked ? null : submit} dirty=${dirty || rawDirty} blocked=${saving || directoryBusy} confirmDiscard=${confirmDiscard} registerClose=${registerClose} resizeKey="tclaude.dash.modalSize.sandbox-profile-editor">${seed?.id && options.editExisting !== false && !options.cloneSourceName && html`<label class="cron-create-enabled"><input id="sandbox-profile-sync-running" type="checkbox" checked=${syncRunning} disabled=${saving} onChange=${event => setSyncRunning(event.currentTarget.checked)} /> Also apply this save to running agents with automatic sync off (${syncCount == null ? 'checking…' : syncCount} eligible running agents). Network rules only.</label>`}
    ...
    <${SandboxSection} id="sandbox-profile-editor-filesystem-section" label="Filesystem"
      ...
      <div class="sbx-resource-intro">The harness config floor binds the harness's own settings file and its hook/skill/agent/command directories read-only, so a confined agent cannot rewrite the policy that confines it or drop code that runs in your next unsandboxed session. It applies under tclaude’s sandbox, alone or combined with the harness’s sandbox. Writable restores the old posture; a single surface can also be reopened with an explicit write row at or below it.</div>
      <div class="sbx-resource-intro">A separate root exposes only the fixed read-only OS/runtime surface and the directory mounts in this policy. Network or Unix-socket restrictions can still require it even when “inherit” is selected. Explicit separation currently requires tclaude’s sandbox on Linux.</div>
    ...
    <${SandboxSection} id="sandbox-profile-editor-tmpfs-section" label="Temporary filesystems"
      help=${TMPFS_HELP} hidden=${advanced} entryCount=${(draft.tmpfs || []).length}>
      <${SandboxTmpfsEditor} mounts=${draft.tmpfs || []} setDraft=${setDraft}
        validation=${tmpfsValidation}/>
    </${SandboxSection}>
    ...
    ${descriptor.sandboxImpl?.platform === 'darwin' && html`<${SandboxSection} id="sandbox-profile-editor-compatibility-section" label="Compatibility — macOS only"
      help="Optional exceptions for software that cannot start under the default tclaude Seatbelt profile. These settings apply only to tclaude’s built-in sandbox; Claude Code and Codex own their built-in Seatbelt profiles."
      hidden=${advanced} entryCount=${Number(draft.darwin_allow_mach_register) + Number(draft.darwin_disable_keychain_write)}>
      <label class="sbx-global-toggle" title="Adds read/write access to ~/Library/Keychains for every harness. Disabling this can prevent harnesses from saving or refreshing vendor/provider login credentials. This covers the shared user Keychain directory, not just one harness. An opt-out in any included or applied profile wins; explicit filesystem grants still apply."><input id="sandbox-profile-editor-allow-keychain-write" type="checkbox" checked=${!draft.darwin_disable_keychain_write} onChange=${(event) => setDraft((value) => ({ ...value, darwin_disable_keychain_write: !event.currentTarget.checked }))}/> Allow macOS Keychain writes (default)</label>
      <div class="sbx-resource-intro">Lets harnesses save and refresh vendor/provider login credentials in the shared user Keychain. Disabling removes the automatic directory grant; explicit filesystem grants still apply.</div>
