...
const FILESYSTEM_HELP = 'Path grants widen the sandbox. A row names a directory or a single '
  ...
  + 'home) and needs sandbox implementation tclaude-layer on Linux; a deny row must name a '
...
const TMPFS_HELP = 'Empty writable filesystems that exist only for the life of the launch and are '
  ...
  + 'filesystem rule beneath it still applies. Linux tclaude-layer only: a tmpfs is a mount, so '
...
function SandboxTmpfsEditor({ mounts, setDraft, validation }) {
  ...
  return html`<div class="sbx-tmpfs-intro"><strong>Linux tclaude-layer only.</strong> Each mount is empty at launch, writable, and discarded when the agent exits. Nothing on the host is exposed by it.</div>
