...
function SandboxEditor({ descriptor, sandboxProfiles, state, actions, confirmDiscard }) {
  ...
  return { filesystem, filesystem_spellings, environment, includes, agent_directories, ...(baseline.pre_launch === undefined && draft.pre_launch === undefined && pre_launch.length === 0 ? {} : { pre_launch }), ...(baseline.tmpfs === undefined && draft.tmpfs === undefined && tmpfs.length === 0 ? {} : { tmpfs }), network: axes.network, unix_sockets: axes.unix_sockets, resource_limits: sandboxResourceLimitsForWire(resource_limits) }; };
