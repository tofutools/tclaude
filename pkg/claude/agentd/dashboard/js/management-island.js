...
function Manager({ kind, current, state, actions, confirmDiscard }) {
  ...
  const list = all.filter((item) => !q || [item.name, ...(item.aliases || []), item.descr, item.role, item.model, item.harness, item.agent_name].some((value) => String(value || '').toLowerCase().includes(q)));
