...
function SandboxEditor({ descriptor, current, state, actions, confirmDiscard }) {
  ...
  const configureWithAgent = () => { let value = draft; if (advanced) { try { value = { ...draft, ...parseRaw() }; } catch (error) { state.error.value = error.message || String(error); return; } } const targetName = options.editExisting === false ? '' : options.targetName || seed?.name || ''; state.closeDialog(); void actions.configureSandboxWithAgent(value, { targetName, onCreate: options.onCreate }); };
