...
const NETWORK_ACCESS_HELP = 'List rows describe outbound destinations. Host matches one exact DNS '
  ...
  + 'class. Host/domain enforcement is DNS-to-IP, not SNI or application identity: a resolved '
  + 'shared IP can be reused until its DNS lease expires. Only a fresh DNS answer refreshes that '
  + 'lease; there is no fixed grace window. Already-established flows may continue after expiry, '
  ...
  + 'Ollama, LM Studio, or llama.cpp and for host-local development services. On Linux it uses '
