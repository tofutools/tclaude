...
function SandboxEditor({ descriptor, sandboxProfiles, state, actions, confirmDiscard }) {
  ...
  return html`<${Overlay} id="sandbox-profile-editor-modal" labelledby="sandbox-profile-editor-title" onClose=${state.closeDialog} onSubmitHotkey=${submitBlocked ? null : submit} dirty=${dirty || rawDirty} blocked=${saving || directoryBusy} confirmDiscard=${confirmDiscard} registerClose=${registerClose} resizeKey="tclaude.dash.modalSize.sandbox-profile-editor">
    ...
    ${seed?.id && options.editExisting !== false && !options.cloneSourceName && html`<label class="cron-create-enabled"><input id="sandbox-profile-sync-running" type="checkbox" checked=${syncRunning} disabled=${saving} onChange=${event => setSyncRunning(event.currentTarget.checked)} /> Also apply this save to running agents with automatic sync off (${syncCount == null ? 'checking…' : syncCount} eligible running agents). Network rules only.</label>`}
