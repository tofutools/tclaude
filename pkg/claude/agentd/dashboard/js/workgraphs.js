// workgraphs.js — the Workgraphs tab: template list + running instances,
...
// `workgraphs` (instance summary rows) and `workgraph_templates`
...
// GET /api/workgraphs/{id}. Mutations go to the dashboard's
// /api/workgraphs* surface (POST create, PATCH a node to advance, POST
...
// palette so workgraphs read like the rest of the UI).
...
function renderWorkgraphsTab() {
  if (!lastSnapshot) return;
  const q = ($('#filter-workgraphs') && $('#filter-workgraphs').value) || '';
  const templates = lastSnapshot.workgraph_templates || [];
  const instances = lastSnapshot.workgraphs || [];
  const shown = filterInstances(instances, q);

  const countEl = $('#filter-workgraphs-count');
  if (countEl) {
  ...
  // Workgraphs tab is actually showing, so a selected instance doesn't
  // keep polling GET /api/workgraphs/{id} from a backgrounded tab; the
  // next poll resumes within 2s once the user switches back.
  if (selectedInstanceId != null) {
    if (workgraphsTabActive()) refreshDetail();
  } else {
    renderDetailEmpty();
  }
}

// workgraphsTabActive reports whether the Workgraphs tab section is the
// visible one (bindTabs toggles .active on the shown <main> section).
function workgraphsTabActive() {
  const s = $('#tab-workgraphs');
  return !!(s && s.classList.contains('active'));
}

function templatesListHTML(templates) {
  if (!templates.length) {
    return `<div class="wf-empty">No workgraph templates found. Drop a <code>workgraph.yaml</code> + <code>flow.mmd</code> under a project/user workgraphs dir, or use the shipped <code>example:</code> ones.</div>`;
  }
  return templates.map(t => {
    ...
    const instBtn = broken
      ...
      : `<button class="primary" data-wfact="instantiate" data-ref="${esc(t.ref)}" title="Instantiate this workgraph">⎘ instantiate</button>`;
...
function instancesListHTML(instances) {
  if (!instances.length) {
    return `<div class="wf-empty">No workgraph instances yet. Instantiate a template on the left to start one.</div>`;
  }
...
async function refreshDetail() {
  ...
  let detail;
  try {
    const r = await fetch(`/api/workgraphs/${id}`, { credentials: 'same-origin' });
    if (!r.ok) {
      if (seq === detailSeq) {
        const host = $('#wf-detail');
...
function renderWarnings(detail) {
  ...
  if (!warns.length) {
    ...
    const trow = ((lastSnapshot && lastSnapshot.workgraph_templates) || []).find(t => t.ref === ref);
...
function resolveBoundMember(inst, assignee, intended) {
  ...
  const row = (lastSnapshot.workgraphs || []).find(w => w.id === inst.id);
...
async function patchNode(id, nodeId, body, okMsg) {
  try {
    const r = await fetch(`/api/workgraphs/${id}/nodes/${encodeURIComponent(nodeId)}`, {
...
async function spawnNodeAgent(id, nodeId) {
  ...
  try {
    const r = await fetch(`/api/workgraphs/${id}/nodes/${encodeURIComponent(nodeId)}/start`, {
...
async function cancelInstance(id) {
  const ok = await confirmModal({
    ...
    okLabel: 'Cancel workgraph',
  });
  if (!ok) return;
  try {
    const r = await fetch(`/api/workgraphs/${id}/cancel`, { method: 'POST', credentials: 'same-origin' });
    if (!r.ok) { toast((await r.text()) || `HTTP ${r.status}`, true); return; }
    toast('workgraph cancelled');
    await refreshDetail();
    refresh();
  } catch (e) {
    toast((e && e.message) || String(e), true);
...
async function deleteInstance(id) {
  ...
  if (!ok) return;
  try {
    const r = await fetch(`/api/workgraphs/${id}`, { method: 'DELETE', credentials: 'same-origin' });
    if (!r.ok && r.status !== 204) { toast((await r.text()) || `HTTP ${r.status}`, true); return; }
    toast('workgraph deleted');
...
function openInstantiateModal(presetRef) {
  const templates = (lastSnapshot && lastSnapshot.workgraph_templates) || [];
  const usable = templates.filter(t => !t.err);
  if (!usable.length) { toast('no instantiable workgraph templates found', true); return; }
...
async function submitInstantiate() {
  ...
  try {
    const r = await fetch('/api/workgraphs', {
      method: 'POST', credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
    ...
    toast(`workgraph instantiated${resp.id ? ` (#${resp.id})` : ''}`);
    await refresh();
...
export { renderWorkgraphsTab, bindWorkgraphsUI };
