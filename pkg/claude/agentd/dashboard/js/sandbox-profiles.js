...
function sandboxScribeBrief(token, targetName, seed) {
  return [
    ...
    'Profile shape: filesystem entries are absolute-path rules with access "read", "write", or "deny". environment values are ordinary non-secret configuration — never credentials. agent_directories is an array of environment-variable names backed by isolated writable directories created at spawn. includes is an ordered array of other profiles composed first. network_access is "internet", "none", or omitted to inherit; explicit network policies currently require the Codex managed sandbox and control external IP connectivity while preserving agentd; offline is supported on macOS and rejected on Linux/WSL until Codex can preserve the agentd Unix socket there. Managed Codex profiles block the host tmux server independently. The daemon remains authoritative for validation.',
    'Strictness has no separate mechanism: express it as ordinary rows — a broad "deny" (commonly the home directory) plus narrower "read"/"write" rows that reopen exactly what the agent needs. tclaude implicitly reopens ONLY the workspace/worktree, the verified Git admin paths, the profile\'s own write grants, declared agent_directories, and the agentd socket. Everything else you must enumerate: the harness state directory, the language runtime, and the toolchain.',
