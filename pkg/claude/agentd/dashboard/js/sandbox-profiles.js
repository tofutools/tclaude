...
function sandboxScribeBrief(token, targetName, seed) {
  return [
    ...
    'Profile shape: filesystem entries are absolute-path rules with access "read", "write", or "deny". environment values are ordinary non-secret configuration — never credentials. agent_directories is an array of environment-variable names backed by isolated writable directories created at spawn. includes is an ordered array of other profiles composed first. network and unix_sockets are independent objects whose mode is "open", "closed", "list", or omitted/unset. network.allow list rows set exactly one of host, domain (+ optional include_subdomains), cidr, or loopback, plus optional integer ports. unix_sockets.allow rows set exactly one absolute path or a bounded path_glob; ** is forbidden. The agentd socket floor is always reachable and is not an editable row. network_access is legacy compatibility input only; author new drafts with network/unix_sockets. Access-list composition is intersection, so incompatible tiers may intentionally produce an empty allow set; surface that warning but do not treat it as invalid. The daemon remains authoritative for validation and launch capability outcomes.',
