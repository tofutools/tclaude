  ...
  >
    ...
    <label class="cron-create-row">
      ...
      <div id="agent-spawn-name-hint" class=${`spawn-field-hint${nameHint.warn ? ' warn' : ''}`}>${nameHint.text}</div>
		...
		${effectiveEnvironment.length ? html`<div class="cron-create-row" id="agent-spawn-environment-preview"><span class="cron-create-label"><${Words} plain="Effective env" wizard="Runes in force" /></span><div class="cron-create-target"><div class="sbx-caps">${effectiveEnvironment.map((entry) => html`<div class="sbx-cap"><span class="sbx-cap-tag sbx-cap-env">${entry.name}</span><span class="sbx-cap-val" title=${entry.value}>${entry.value}</span><span class="muted">${entry.source}</span></div>`)}</div><div class="muted"><${Words} plain="Common configured values shown here; sandbox-profile variables are additionally resolved by the daemon at launch." wizard="Common bound runes are shown here; ward runes are additionally divined by the daemon during summoning." /></div></div></div>` : null}
    <label class="cron-create-row">
