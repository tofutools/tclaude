...
function SlugsView({ state, current }) {
  ...
  return html`<${Fragment}>
    ...
    <div class="muted" style="font-size:11px;margin-bottom:6px">👑 = group ownership confers this slug without an explicit grant. <b>group</b> = over the groups you own; <b>group members</b> = over an owned group's whole roster only when every affected member is also contained exclusively in groups you own; <b>member</b> = over an agent only when you own all its active groups; <b>any</b> = unscoped, owning any group is enough. A per-agent deny suppresses the bypass.</div>
    ${current.slugs.length === 0
      ...
      ${current.slugs.map((slug) => html`<tr key=${slug.slug} data-key=${slug.slug}>
        ...
        <td>${slug.owner_implied
          ? html`<span class="owner-badge" title=${ownerScopeTitle(slug.owner_scope)}>👑${slug.owner_scope ? ` ${ownerScopeLabel(slug.owner_scope)}` : ''}</span>`
