...
function GroupAttachment({ group, actions, placement }) {
  ...
  const visibleLabel = fixed && rawURL
    ? html`<span class="group-attachment-label qo-text">${label}</span>`
    : null;
  const visibleIcon = fixed
    ? html`<span class="group-attachment-marker" aria-hidden="true">
        <span class="group-attachment-icon">📎</span>
        ${rawURL ? html`<span class="group-attachment-status"></span>` : null}
      </span>`
    : '📎';
...
function GroupMenuItems({ group, members, snapshot, actions }) {
  ...
  return html`
    ...
    <${MenuButton} title=${wizardMode ? `Choose which durable decrees apply to party ${name}` : `Choose which standing orders apply to ${name}`} regular="📋 standing orders…" wizard="📜 standing orders…" onClick=${(event) => {
      event.preventDefault();
      event.stopPropagation();
      interactions.closeMenu(true);
      actions.openStandingOrders(group);
    }} />
    ...
    <${MenuButton} ...${shared} data-act="pull-repositories-group" title="Git pull — update repositories in this home directory and two levels below" regular="↓ pull repositories…" wizard="↓ summon latest code…" />
    <${MenuButton} ...${shared} data-act="sync-repositories-group" title="Git sync — fetch, prune stale remote references, then fast-forward pull" regular="↻ sync repositories…" wizard="✧ harmonize repositories…" />
