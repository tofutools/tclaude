...
export function createAuditState({ activeTab = dashboardState.activeTab, now = () => Date.now() } = {}) {
  ...
  const response = signal(null); const request = signal({ phase: 'idle', token: 0, error: null });
  let lastFetchedAt = 0;
  const view = computed(() => {
  ...
  function beginRequest() {
    const token = request.value.token + 1; lastFetchedAt = now();
    request.value = { phase: response.value ? 'refreshing' : 'loading', token, error: null }; return token;
  }
  function commitRequest(token, data) {
    ...
    request.value = { phase: 'ready', token, error: null }; return true;
  }
  function failRequest(token, error) {
    if (request.value.token !== token) return false;
    response.value = null; request.value = { phase: 'error', token, error: errorMessage(error) }; return true;
  }
