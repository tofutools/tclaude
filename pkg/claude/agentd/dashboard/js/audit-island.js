...
function Rows({ current, state, actions }) {
  ...
  if (!current.rows.length) return html`<div class="empty">${current.totalUnfiltered ? 'No events match the filter.' : 'No audit events recorded yet. Rows are written for coordination commands and managed agent exits.'}</div>`;
