...
// dispatch and never reads the producer DOM again.
  ...
  try {
    ...
    switch (act) {
      ...
      case 'toggle-remote-control': {
        ...
        const intent = data.intent || 'toggle';
      ...
      case 'term-dir': {
        ...
        const which = data.which || 'current';
        // With web terminals as the default (config dashboard.default_terminal
      ...
      case 'clone': {
        ...
        openCloneAgentDialog(agent, label, data.cwd || '');
        return;
      }
      case 'reincarnate': {
      ...
      case 'edit-task': {
        ...
        const oldURL = data.current || '';
        const oldTaskLabel = data.currentTaskLabel || '';
        openTaskLinkDialog({
          conv: agent,
          agentLabel: label,
          url: oldURL,
      ...
      case 'set-group-permissions': {
        ...
        openGroupPermEditor(group, snapshot?.permissions || [], snapshot?.permission_scopes || {},
          snapshot?.unreadable_permission_scopes || [], snapshot?.owner_scopes || {});
        return;
      }
      ...
      case 'link-delete': {
        ...
        // attribute.
        const scope = data.group || from || to;
