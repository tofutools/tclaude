...
// graph + node defs.
func writeToolTemplate(t *testing.T, root, name, workgraphYAML, mermaid string, nodes map[string]string) {
	t.Helper()
	dir := filepath.Join(root, name)
	require.NoError(t, os.MkdirAll(filepath.Join(dir, "nodes"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "workgraph.yaml"), []byte(workgraphYAML), 0o644))
...
// Scenario: a pure tool-node workgraph runs to completion with NO human clicks —
...
func TestWorkgraphEngine_ToolChainAutoCompletes(t *testing.T) {
...
func TestWorkgraphEngine_EnumToolVerifySelectsBranch(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_FailingToolHaltsInstance(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_OnFailContinueFollowsFailEdge(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_CaptureInterpolatesDownstream(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_LargeOutputTruncatedButUsable(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_DisabledIsNoOp(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	...
	agentd.RunWorkgraphEngineTickForTest() // no-op: engine disabled
...
func TestWorkgraphEngine_ReapsOrphanedRunningNode(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	owner := agentd.WorkgraphEngineAssigneeForTest()
	_, err := db.UpdateWorkgraphNode(id, "a", db.WorkgraphNodePatch{Status: &running, Assignee: &owner})
...
func TestWorkgraphEngine_ReaperLeavesHumanDrivenRunningNode(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	running := db.WorkgraphNodeStatusRunning
	_, err := db.UpdateWorkgraphNode(id, "a", db.WorkgraphNodePatch{Status: &running})
	require.NoError(t, err)

	agentd.ReapOrphanedEngineNodesForTest()

	got, err := db.GetWorkgraphNode(id, "a")
	require.NoError(t, err)
...
func TestWorkgraphEngine_LeavesHumanNodeAlone(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func assertRunningAIAssignee(t *testing.T, id int64, nodeID string) string {
	t.Helper()
	n, err := db.GetWorkgraphNode(id, nodeID)
	require.NoError(t, err)
	...
	assert.NotEqual(t, agentd.WorkgraphEngineAssigneeForTest(), n.Assignee,
...
func TestWorkgraphEngine_AINodeAutoSpawnsIntoBoundGroup(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_AINodeUnboundLeftReady(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	_ = newFlow(t)
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
...
func TestWorkgraphEngine_AIDisabledLeavesNodeReady(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	// Engine left at its production default (disabled) — no enable call.
...
func TestWorkgraphEngine_AIPerInstanceCap(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	t.Cleanup(agentd.SetWorkgraphAICapsForTest(1, 8)) // per-instance 1, global 8
	...
	agentd.SetWorkgraphAICapsForTest(2, 8)
	agentd.RunWorkgraphEngineTickForTest()
	agentd.WaitForBackgroundForTest()
...
func TestWorkgraphEngine_AIGlobalCap(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	t.Cleanup(agentd.SetWorkgraphAICapsForTest(1, 1)) // per-instance 1, global 1
...
func countAIStatuses(t *testing.T, id int64, nodeIDs ...string) (running, ready int) {
	...
	for _, nid := range nodeIDs {
		n, err := db.GetWorkgraphNode(id, nid)
		require.NoError(t, err)
		require.NotNil(t, n)
		switch n.Status {
		case "running":
...
func TestWorkgraphEngine_AIVerifyJudgeRoundTrip(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	parked, _ := db.GetWorkgraphNode(id, "build")
	...
	agentd.RunWorkgraphEngineTickForTest()
	agentd.WaitForBackgroundForTest()
	judged, _ := db.GetWorkgraphNode(id, "build")
	...
	assert.NotEqual(t, agentd.WorkgraphEngineAssigneeForTest(), judge, "sentinel swapped for the judge conv-id")
	var sawJudge bool
	for _, m := range f.ListGroupMembers("squad") {
		if m.ConvID == judge {
			sawJudge = true
...
func TestWorkgraphEngine_AIVerifyJudgeFail(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	agentd.RunWorkgraphEngineTickForTest()                          // judge spawns
	agentd.WaitForBackgroundForTest()
...
func TestWorkgraphEngine_AIVerifySelfReportFallback(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
...
func TestWorkgraphEngine_StuckRunningWorkerSwept(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	t.Cleanup(agentd.SetWorkgraphNodeSLAForTest(time.Nanosecond)) // any idle node with no live agent is past terminal
	...
	_, err := db.UpdateWorkgraphNode(id, "build", db.WorkgraphNodePatch{Status: &running, Assignee: &dead})
	require.NoError(t, err)
...
func TestWorkgraphEngine_ParkedNeverJudgedSwept(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	t.Cleanup(agentd.SetWorkgraphNodeSLAForTest(time.Nanosecond))
	t.Cleanup(agentd.SetWorkgraphAICapsForTest(0, 0)) // no judge can ever spawn (cap starved)
	...
	cleared := ""
	_, err := db.UpdateWorkgraphNode(id, "build", db.WorkgraphNodePatch{Status: &awaiting, Assignee: &cleared})
	require.NoError(t, err)
...
func TestWorkgraphEngine_ReapsOrphanedJudgeClaim(t *testing.T) {
	t.Cleanup(agentd.SetPopupBaseURLForTest("http://127.0.0.1:0"))
	f := newFlow(t)
	_ = f.HaveGroup("squad")
	t.Cleanup(agentd.SetWorkgraphEngineEnabledForTest(true))
	...
	awaiting := db.WorkgraphNodeStatusAwaitingVerify
	sentinel := agentd.WorkgraphEngineAssigneeForTest()
	_, err := db.UpdateWorkgraphNode(id, "build", db.WorkgraphNodePatch{Status: &awaiting, Assignee: &sentinel})
	require.NoError(t, err)

	// Startup reaper clears the sentinel; status stays awaiting_verify.
	agentd.ReapOrphanedEngineNodesForTest()
	reaped, err := db.GetWorkgraphNode(id, "build")
	require.NoError(t, err)
	...
	agentd.RunWorkgraphEngineTickForTest()
	agentd.WaitForBackgroundForTest()
	rejudged, err := db.GetWorkgraphNode(id, "build")
	require.NoError(t, err)
