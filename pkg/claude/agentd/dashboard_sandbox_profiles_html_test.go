...
func TestDashboardHTML_SandboxProfilesUI(t *testing.T) {
	for needle, why := range map[string]string{
		...
		`function sandboxProfilesDisabledForSpawn()`:                     "danger-full-access compatibility predicate",
		`applySpawnSandboxProfileAvailability()`:                         "sandbox changes update profile visibility",
		`if (!sandboxProfilesDisabledForSpawn() && sandboxProfile)`:      "disabled profiles are omitted from spawn requests",
