...
func TestDashboardHTML_SandboxProfilesUI(t *testing.T) {
	for needle, why := range map[string]string{
		...
		`id="sandbox-profile-editor-pre-launch-section"`:                               "first-class foldable pre-launch editor section",
		`label="Pre-launch scripts"`:                                                   "pre-launch section uses shared entry-count chrome",
		`class="sbx-prelaunch-list"`:                                                   "ordered multiline pre-launch entry list",
		`class="sbx-add-row sbx-prelaunch-add"`:                                        "plain add-block control",
		`sandboxPreLaunchForWire(draft.pre_launch)`:                                    "editor-only export text is removed from save payloads",
		`id="sandbox-profile-editor-tmpfs"`:                                            "raw temporary-filesystem editor",
		`id="sandbox-profile-editor-tmpfs-section"`:                                    "first-class foldable temporary-filesystem editor section",
		`label="Temporary filesystems"`:                                                "tmpfs section uses shared entry-count chrome",
		`class="sbx-tmpfs-list"`:                                                       "structured temporary-filesystem entry list",
		`class="sbx-add-row sbx-tmpfs-add"`:                                            "plain add-mount control",
		`sandboxTmpfsForWire(draft.tmpfs)`:                                             "editor-only row identities are removed from save payloads",
		`if (draft.tmpfs !== undefined) body.tmpfs = draft.tmpfs;`:                     "absent tmpfs means leave the mounts alone, never clear them",
		`...(body.tmpfs === undefined ? {} : { tmpfs: body.tmpfs })`:                   "an explicit clear survives the preview's omitempty round trip",
		`.sbx-tmpfs-size`:                                                              "size field keeps its own narrower structured styling",
		`.sbx-cap-tmpfs`:                                                               "profile cards tag a temporary filesystem distinctly from a grant",
		...
		`id="sandbox-profile-editor-network-namespace"`:                                "network namespace selector",
		...
		`id="sandbox-profile-import-preview"`:                                          "expandable import policy preview",
		`function SandboxImportProfile(`:                                               "per-profile import policy card",
		`function sandboxImportPolicyRows(`:                                            "all imported policy axes become reviewable rows",
		`profile.filesystem_spellings?.rules`:                                          "retained path spellings are visible before import",
		`class="sandbox-import-script"`:                                                "pre-launch script text has a keyboard-accessible disclosure",
		`class="sandbox-import-warning" role="note"`:                                   "portability warnings stay with the affected profile",
		`id="sandbox-profile-import-effect"`:                                           "conflict policy states the resulting import effect",
		`Resolve name conflict`:                                                        "error-on-conflict cannot look like an executable import",
		...
		`.sandbox-import-profile[open] { border-color: #3d4d60; }`:                     "expanded import cards have a reviewable open state",
		`.sandbox-import-policy {`:                                                     "imported policy rows have a bounded card body",
		`.sandbox-import-warning {`:                                                    "import path warnings receive visible caution styling",
		`.sandbox-import-script > pre {`:                                               "multiline pre-launch policy remains readable",
		...
		"filesystem: [...value.filesystem, ...rows]":                                  "a preset inserts ordinary rows, not a stored mechanism",
		"commonRuleReadOnlyPaths(entry).map((path) => ({ path, access: 'read' }))":    "harness-state presets keep that harness's config floor read-only",
		"const access = entry.access || 'deny';":                                      "a preset without an access inserts deny rows",
		`id="sandbox-profile-editor-harness-state-rules"`:                             "harness-state presets for tclaude run have their own group",
