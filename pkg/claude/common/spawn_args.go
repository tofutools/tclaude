...
// SpawnCwdProofPrefix is the fixed basename prefix for the short-lived marker
// an agent caller creates in a child spawn directory. It lives in this shared
// package so the CLI, daemon, session launcher, and flow simulator agree on the
// wire-visible filename without creating an import cycle.
...
type SpawnArgs struct {
	...
	// CwdWriteProof is the daemon-signed token whose marker must still exist in
	// Cwd after tmux has established the pane's working-directory inode. The
	// forked session launcher checks/removes it immediately before executing the
	// harness, closing pathname-swap races between daemon validation and launch.
	// Fresh agent spawn only; empty on human launches and resumes.
