...
type SpawnArgs struct {
	...
	// Codex profile and Claude Code's per-session allowWrite overlay.
	// CodexGitCommonDirPinned distinguishes a proved empty result
	// (the cwd was not a Git repo) from an unpinned direct-human launch, which may
	// derive the common dir from cwd. Daemon-managed launches set the bit even
	...
	// RouteHelper* are an internal, Unix handoff for a route-capable
	// tclaude-layer launch. Linux uses the namespace helper; Darwin uses the
	// filtering proxy directly. The session child derives the exact exit-launch
	// generation from the daemon-minted capability, so only the pre-enrolled
	// identity, explicit group scope, and a non-secret one-shot credential FD
	// handoff socket cross this boundary. The credential is never persisted or
	// serialized by the session child.
	...
	RouteHelperCredentialHandoffSocketPath string
