...
func TestNormalizeAccessRules(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"loopback cidr", NetworkRules{Mode: AccessModeList, Allow: []NetworkAllowEntry{{CIDR: "127.1.2.3/16"}}}, `use {"loopback": true} for that portion`},
		{"mapped loopback cidr", NetworkRules{Mode: AccessModeList, Allow: []NetworkAllowEntry{{CIDR: "::ffff:127.0.0.1/128"}}}, `use {"loopback": true} for that portion`},
		{"mapped IPv4 space includes loopback", NetworkRules{Mode: AccessModeList, Allow: []NetworkAllowEntry{{CIDR: "::ffff:0:0/96"}}}, `split it and keep CIDR rows for the remainder`},
