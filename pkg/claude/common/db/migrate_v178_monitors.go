...
// migrateV177toV178 adds sessions.monitors_json — the per-session ledger
...
func migrateV177toV178(db *sql.DB) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("migrate v177→v178 (monitors): begin: %w", err)
	}
	...
	if err := tx.QueryRow(
		...
		return fmt.Errorf("migrate v177→v178 (probe sessions): %w", err)
	}
	if haveTable > 0 {
		var haveColumn int
		if err := tx.QueryRow(
			...
			return fmt.Errorf("migrate v177→v178 (probe sessions.monitors_json): %w", err)
		}
		if haveColumn == 0 {
			if _, err := tx.Exec(
				`ALTER TABLE sessions ADD COLUMN monitors_json TEXT NOT NULL DEFAULT ''`,
			); err != nil {
				return fmt.Errorf("migrate v177→v178 (add sessions.monitors_json): %w", err)
			}
		}
	}

	if _, err := tx.Exec(`UPDATE schema_version SET version = 178`); err != nil {
		return fmt.Errorf("migrate v177→v178 (version): %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("migrate v177→v178 (commit): %w", err)
	}
	return nil
}
