...
func TestMigrateV142AddsReadExclusionsWithCompatibleDefault(t *testing.T) {
	...
	require.NoError(t, migrateV141toV142(d))
	var value string
	require.NoError(t, d.QueryRow(`SELECT read_baseline_exclusions_json FROM sandbox_profiles WHERE name = 'legacy'`).Scan(&value))
	assert.Equal(t, "[]", value)
	assert.Equal(t, 142, schemaVersion(d))
}

func TestMigrateV142IsIdempotentAndTableOptional(t *testing.T) {
	d := newV139DB(t, "migrate-v142-idempotent")
	mustExec(t, d, `UPDATE schema_version SET version = 141`)
	require.NoError(t, migrateV141toV142(d))
	mustExec(t, d, `UPDATE schema_version SET version = 141`)
