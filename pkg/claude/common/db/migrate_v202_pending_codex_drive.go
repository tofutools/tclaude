...
// migrateV201toV202 preserves the resolved Codex drive while an asynchronous
...
func migrateV201toV202(d *sql.DB) error {
	tx, err := d.Begin()
	if err != nil {
		return fmt.Errorf("migrate v201→v202 (pending Codex drive): begin: %w", err)
	}
	...
	if err := tx.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'pending_spawns'`).Scan(&pendingSpawnsExists); err != nil {
		return fmt.Errorf("migrate v201→v202 (pending Codex drive): probe pending_spawns: %w", err)
	}
	...
	// convergent for those partial databases; real v201 databases have this
	// table from v59.
	if pendingSpawnsExists != 0 {
		for _, column := range []struct {
			...
			if err := tx.QueryRow(`SELECT COUNT(*) FROM pragma_table_info('pending_spawns') WHERE name = ?`, column.name).Scan(&have); err != nil {
				return fmt.Errorf("migrate v201→v202 (pending Codex drive): probe %s: %w", column.name, err)
			}
			if have == 0 {
				if _, err := tx.Exec(column.ddl); err != nil {
					return fmt.Errorf("migrate v201→v202 (pending Codex drive): add %s: %w", column.name, err)
				}
			}
		}
	}
	if _, err := tx.Exec(`UPDATE schema_version SET version = 202`); err != nil {
		return fmt.Errorf("migrate v201→v202 (pending Codex drive): version: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("migrate v201→v202 (pending Codex drive): commit: %w", err)
	}
	return nil
}
