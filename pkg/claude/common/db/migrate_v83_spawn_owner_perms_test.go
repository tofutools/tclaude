...
func TestSpawnProfileOwnerPermsRoundTrip(t *testing.T) {
	...
	id, err := CreateSpawnProfile(&SpawnProfile{
		...
		Role:                "lead",
		IsOwner:             &owner,
		PermissionOverrides: UnscopedOverrides(map[string]string{"groups.members.spawn": "grant"}),
	})
	...
	assert.Equal(t, UnscopedOverrides(map[string]string{"groups.members.spawn": "grant"}), got.PermissionOverrides)
...
func TestPendingSpawnOwnerPermsRoundTrip(t *testing.T) {
	...
	in := &PendingSpawn{
		...
		Role:                "researcher",
		IsOwner:             true,
		PermissionOverrides: UnscopedOverrides(map[string]string{"groups.members.spawn": "grant", "self.rename": "deny"}),
	}
	...
	require.NotNil(t, got)
	assert.True(t, got.IsOwner, "owner flag round-trips")
	assert.Equal(t, UnscopedOverrides(map[string]string{"groups.members.spawn": "grant", "self.rename": "deny"}),
