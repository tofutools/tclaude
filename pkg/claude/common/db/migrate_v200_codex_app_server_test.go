...
func TestMigrateV199toV200AddsCodexAppServerRuntime(t *testing.T) {
	d, err := sql.Open("sqlite", filepath.Join(t.TempDir(), "v199.sqlite"))
	...
	require.NoError(t, migrateV199toV200(d), "partially applied migration converges")
}
