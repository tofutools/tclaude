...
func collectCronJobs(d *sql.DB, groupID int64) ([]groupexport.CronJob, []string, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&j.ID, &j.Name, &j.TargetKind, &j.OwnerConv, &j.TargetConv,
			...
			&j.Enabled, &j.RunImmediately, &j.QueueWhenOffline, &createdAt,
...
func (c *importCtx) scrubRetiredActorAuthority() error {
	...
	for agentID := range c.createdActors {
		...
		for _, stmt := range []struct {
			...
			{`UPDATE agent_standing_orders SET enabled = 0, disabled_reason = ?, row_version = row_version + 1, updated_at = ? WHERE owner_agent = ? AND enabled = 1`, []any{StandingDisabledReasonAgentRetired, now, agentID}},
		} {
		...
		res, err := c.tx.Exec(`
			INSERT INTO agent_cron_jobs
				(name, target_kind, owner_agent, target_agent, group_id, interval_seconds,
				 ...
				 spawn_worker_deadline_seconds,enabled,run_immediately,queue_when_offline,created_at,last_run_at,last_run_status)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			...
			j.Enabled, j.RunImmediately, j.QueueWhenOffline, requiredImportDBTime(j.CreatedAt),
