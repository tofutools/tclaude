...
// migrateV198toV199 adds the opt-in Codex app-server profile field and the
...
func migrateV198toV199(d *sql.DB) error {
	tx, err := d.Begin()
	if err != nil {
		return fmt.Errorf("migrate v198→v199 (Codex app-server runtime): begin: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	var have int
	if err := tx.QueryRow(`SELECT COUNT(*) FROM pragma_table_info('spawn_profiles') WHERE name = 'codex_app_server'`).Scan(&have); err != nil {
		return fmt.Errorf("migrate v198→v199 (Codex app-server runtime): probe: %w", err)
	}
	if have == 0 {
		if _, err := tx.Exec(`ALTER TABLE spawn_profiles ADD COLUMN codex_app_server INTEGER`); err != nil {
			return fmt.Errorf("migrate v198→v199 (Codex app-server runtime): add profile column: %w", err)
		}
	}
	if _, err := tx.Exec(`
	...
	`); err != nil {
		return fmt.Errorf("migrate v198→v199 (Codex app-server runtime): create runtime table: %w", err)
	}
	if _, err := tx.Exec(`UPDATE schema_version SET version = 199`); err != nil {
		return fmt.Errorf("migrate v198→v199 (Codex app-server runtime): version: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("migrate v198→v199 (Codex app-server runtime): commit: %w", err)
	}
	return nil
}
