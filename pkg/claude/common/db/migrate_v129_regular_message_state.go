...
// migrateV128toV129 separates durable regular-message processing from tmux
...
func migrateV128toV129(db *sql.DB) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("migrate v128→v129 (regular message state): begin: %w", err)
	}
	defer func() { _ = tx.Rollback() }()

	haveMessages, err := txTableExists(tx, "agent_messages")
	if err != nil {
		return fmt.Errorf("migrate v128→v129 (probe agent_messages): %w", err)
	}
	if haveMessages {
		columns := []struct {
			name string
		...
		for _, column := range columns {
			if err := addColumnIfMissing(tx, "agent_messages", column.name, column.sql); err != nil {
				return fmt.Errorf("migrate v128→v129 (add %s): %w", column.name, err)
			}
		}
		if _, err := tx.Exec(`CREATE INDEX IF NOT EXISTS idx_agent_messages_regular_agent_backlog
			...
			return fmt.Errorf("migrate v128→v129 (agent backlog index): %w", err)
		}
		if _, err := tx.Exec(`CREATE INDEX IF NOT EXISTS idx_agent_messages_regular_conv_backlog
			...
			return fmt.Errorf("migrate v128→v129 (conv backlog index): %w", err)
		}
	}
	if _, err := tx.Exec(`UPDATE schema_version SET version = 129`); err != nil {
		return fmt.Errorf("migrate v128→v129 (version): %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("migrate v128→v129 (regular message state): commit: %w", err)
	}
	return nil
}
