...
// Workgraph instance status values — the value stored in
// workgraph_instances.status. An instance is created `running` and walks
// to exactly one terminal state.
const (
	WorkgraphStatusRunning   = "running"
	WorkgraphStatusCompleted = "completed"
	WorkgraphStatusFailed    = "failed"
	WorkgraphStatusCancelled = "cancelled"
)

// Workgraph node status values — the value stored in workgraph_nodes.status.
...
const (
	WorkgraphNodeStatusPending        = "pending"
	WorkgraphNodeStatusReady          = "ready"
	WorkgraphNodeStatusRunning        = "running"
	WorkgraphNodeStatusAwaitingVerify = "awaiting_verify"
	WorkgraphNodeStatusDone           = "done"
	WorkgraphNodeStatusFailed         = "failed"
	WorkgraphNodeStatusSkipped        = "skipped"
)

// Workgraph event kinds — the value stored in workgraph_events.kind. The
...
const (
	WorkgraphEventInstanceCreated    = "instance_created"
	WorkgraphEventNodeReady          = "node_ready"
	WorkgraphEventNodeStarted        = "node_started"
	WorkgraphEventNodeDone           = "node_done"
	WorkgraphEventNodeFailed         = "node_failed"
	WorkgraphEventNodeSkipped        = "node_skipped"
	WorkgraphEventNodeApproved       = "node_approved"        // human-verify gate: approved (settles done + advances)
	WorkgraphEventNodeRejected       = "node_rejected"        // human-verify gate: rejected (recorded, no advance)
	WorkgraphEventNodeAwaitingVerify = "node_awaiting_verify" // ai-verify: executor done, judge round-trip pending
	WorkgraphEventHandoff            = "handoff"              // engine delivered a predecessor's output to a bound successor's inbox (JOH-40)
	WorkgraphEventNodeRetry          = "node_retry"          // engine re-armed a node for an in-place retry after its verify failed (JOH-39)
	WorkgraphEventNodeReentry        = "node_reentry"        // engine re-armed a node + its loop body via a back-edge loop-back (JOH-39)
	WorkgraphEventNodeEscalation     = "node_escalation"     // stuck-node sweep fired an escalation rung (warn/escalate/terminal); Message is the at-most-once marker (JOH-41)
	WorkgraphEventDriverNudge        = "driver_nudge"        // agent-mode: nudged the live driver(s) about a frontier change; Message is the at-most-once marker (JOH-15 B2b)
)

// WorkgraphInstance is a row in workgraph_instances — one instantiation of
// a workgraph template. Templates live on disk; Mermaid / Params / Vars
...
type WorkgraphInstance struct {
	ID           int64
	TemplateRef  string // "user:foo" | "example:foo" | path
	TemplateName string
	Title        string
	Status       string // WorkgraphStatus* — running|completed|failed|cancelled
...
// WorkgraphNode is a row in workgraph_nodes — one node of one instance.
...
type WorkgraphNode struct {
	...
	Label        string
	ExecutorKind string
	Status       string // WorkgraphNodeStatus*
...
// WorkgraphEvent is a row in workgraph_events — one entry in the append-only
// audit/timeline. NodeID is "" for an instance-level event.
type WorkgraphEvent struct {
	ID         int64
	InstanceID int64
	NodeID     string
	Kind       string // WorkgraphEvent* — free text, these are the well-known kinds
	Message    string
	At         time.Time
}

// isTerminalWorkgraphStatus reports whether a status is one of the three
// terminal states. UpdateWorkgraphInstanceStatus uses this to decide
// whether to stamp completed_at.
func isTerminalWorkgraphStatus(status string) bool {
	switch status {
	case WorkgraphStatusCompleted, WorkgraphStatusFailed, WorkgraphStatusCancelled:
		return true
	default:
		return false
	}
...
// InsertWorkgraphInstance writes a new instance row and returns its ID.
...
func InsertWorkgraphInstance(w *WorkgraphInstance) (int64, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
...
// GetWorkgraphInstance returns a single instance by ID, or nil if not found.
func GetWorkgraphInstance(id int64) (*WorkgraphInstance, error) {
	d, err := Open()
	if err != nil {
		return nil, err
	}
	row := d.QueryRow(`SELECT id, template_ref, template_name, title, status,
		...
		FROM workgraph_instances WHERE id = ?`, id)
	w, err := scanWorkgraphInstance(row)
...
// ListWorkgraphInstances returns every instance, ordered by ID asc.
func ListWorkgraphInstances() ([]*WorkgraphInstance, error) {
	d, err := Open()
	if err != nil {
		return nil, err
	}
	rows, err := d.Query(`SELECT id, template_ref, template_name, title, status,
		mermaid, params, vars, group_id, engine_mode, created_at, updated_at, completed_at
		FROM workgraph_instances ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	var out []*WorkgraphInstance
	for rows.Next() {
		w, err := scanWorkgraphInstance(rows)
		if err != nil {
			return nil, err
		}
		out = append(out, w)
...
// UpdateWorkgraphInstanceStatus sets the instance status and bumps
...
func UpdateWorkgraphInstanceStatus(id int64, status string) (int, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
	now := time.Now().UTC().Format(time.RFC3339)
	completed := ""
	if isTerminalWorkgraphStatus(status) {
		completed = now
	}
	res, err := d.Exec(`UPDATE workgraph_instances
...
// UpdateWorkgraphInstanceVars replaces the captured-vars JSON blob and
// bumps updated_at. An empty string is stored as "{}". Returns rows
// affected (0 → no such id).
func UpdateWorkgraphInstanceVars(id int64, vars string) (int, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
	res, err := d.Exec(`UPDATE workgraph_instances SET vars = ?, updated_at = ? WHERE id = ?`,
...
// DeleteWorkgraphInstance removes an instance by ID. Idempotent. The
// ON DELETE CASCADE foreign keys remove its nodes and events too (the
// DSN enables foreign_keys, so the cascade fires).
func DeleteWorkgraphInstance(id int64) error {
	d, err := Open()
	if err != nil {
		return err
	}
	_, err = d.Exec(`DELETE FROM workgraph_instances WHERE id = ?`, id)
	return err
}

// ----- nodes -----------------------------------------------------------

// InsertWorkgraphNode writes a single node row and returns its ID.
...
func InsertWorkgraphNode(n *WorkgraphNode) (int64, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
	res, err := d.Exec(`INSERT INTO workgraph_nodes
...
// InsertWorkgraphNodes bulk-inserts the full node set for one instance in
...
func InsertWorkgraphNodes(instanceID int64, nodes []*WorkgraphNode) error {
	if len(nodes) == 0 {
		return nil
	}
	d, err := Open()
	...
	now := time.Now().UTC().Format(time.RFC3339)
	stmt, err := tx.Prepare(`INSERT INTO workgraph_nodes
		(instance_id, node_id, label, executor_kind, status, outcome, detail,
		 output, assignee, visits, started_at, finished_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
...
// ListWorkgraphNodes returns every node of one instance, ordered by ID asc
// (insertion order, which mirrors the chart order at instantiation).
func ListWorkgraphNodes(instanceID int64) ([]*WorkgraphNode, error) {
	d, err := Open()
	if err != nil {
		return nil, err
	}
	rows, err := d.Query(`SELECT id, instance_id, node_id, label, executor_kind,
		...
		FROM workgraph_nodes WHERE instance_id = ? ORDER BY id`, instanceID)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	var out []*WorkgraphNode
	for rows.Next() {
		n, err := scanWorkgraphNode(rows)
		if err != nil {
			return nil, err
		}
		out = append(out, n)
...
// GetWorkgraphNode returns one node by its (instanceID, nodeID) identity,
// or nil if not found.
func GetWorkgraphNode(instanceID int64, nodeID string) (*WorkgraphNode, error) {
	d, err := Open()
	if err != nil {
		return nil, err
	}
	row := d.QueryRow(`SELECT id, instance_id, node_id, label, executor_kind,
		status, outcome, detail, output, assignee, visits,
		started_at, finished_at, updated_at
		FROM workgraph_nodes WHERE instance_id = ? AND node_id = ?`, instanceID, nodeID)
	n, err := scanWorkgraphNode(row)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	return n, err
...
// ListAssignedWorkgraphNodes returns every workgraph node that has a non-empty
// assignee, across all instances, ordered by instance_id then id. The
// /v1/workgraphs/where handler resolves each row's assignee to its succession
...
func ListAssignedWorkgraphNodes() ([]*WorkgraphNode, error) {
	d, err := Open()
	if err != nil {
		return nil, err
	}
	rows, err := d.Query(`SELECT id, instance_id, node_id, label, executor_kind,
		status, outcome, detail, output, assignee, visits,
		started_at, finished_at, updated_at
		FROM workgraph_nodes WHERE assignee != '' ORDER BY instance_id, id`)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
// CountRunningWorkgraphNodesByKind returns how many nodes of the given executor
// kind are currently `running`, across ALL instances. The workgraph engine uses
...
func CountRunningWorkgraphNodesByKind(executorKind string) (int, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
	var n int
	err = d.QueryRow(`SELECT COUNT(*) FROM workgraph_nodes
		WHERE executor_kind = ? AND status = ?`,
		executorKind, WorkgraphNodeStatusRunning).Scan(&n)
	return n, err
}
...
// workgraph engine uses it to count verify-judges in flight toward the global
...
// mirrors CountRunningWorkgraphNodesByKind.
//
...
func CountAwaitingVerifyAssignedNodes() (int, error) {
	...
	err = d.QueryRow(`SELECT COUNT(*) FROM workgraph_nodes
		...
		WorkgraphNodeStatusAwaitingVerify).Scan(&n)
	return n, err
}

// WorkgraphNodePatch is the partial-update shape for UpdateWorkgraphNode.
...
type WorkgraphNodePatch struct {
	Label        *string
	ExecutorKind *string
	Status       *string
	Outcome      *string
...
// UpdateWorkgraphNode applies a partial update to one node identified by
...
func UpdateWorkgraphNode(instanceID int64, nodeID string, p WorkgraphNodePatch) (int, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
	...
	res, err := d.Exec(`UPDATE workgraph_nodes SET `+strings.Join(sets, ", ")+
...
// AppendWorkgraphEvent appends one timeline entry and returns its ID. A
...
func AppendWorkgraphEvent(e *WorkgraphEvent) (int64, error) {
	d, err := Open()
	if err != nil {
		return 0, err
	}
	...
	res, err := d.Exec(`INSERT INTO workgraph_events
...
// ListWorkgraphEvents returns the timeline for one instance, oldest first
...
func ListWorkgraphEvents(instanceID int64, nodeID ...string) ([]*WorkgraphEvent, error) {
	d, err := Open()
	if err != nil {
		return nil, err
	}
	q := `SELECT id, instance_id, node_id, kind, message, at
		FROM workgraph_events WHERE instance_id = ?`
	args := []any{instanceID}
	if len(nodeID) > 0 {
		q += ` AND node_id = ?`
	...
	defer func() { _ = rows.Close() }()
	var out []*WorkgraphEvent
	for rows.Next() {
		var e WorkgraphEvent
		var at string
...
func nodeStatusOrDefault(s string) string {
	if s == "" {
		return WorkgraphNodeStatusPending
	}
	return s
}
...
// DB layer is deliberately ignorant of the valid set — the workgraph loader
...
func scanWorkgraphInstance(s rowScanner) (*WorkgraphInstance, error) {
	var w WorkgraphInstance
...
func scanWorkgraphNode(s rowScanner) (*WorkgraphNode, error) {
	var n WorkgraphNode
...
// SetWorkgraphNodeUpdatedAtForTest back-dates a node's updated_at directly,
...
// tests.
func SetWorkgraphNodeUpdatedAtForTest(instanceID int64, nodeID string, t time.Time) error {
	d, err := Open()
	if err != nil {
		return err
	}
	_, err = d.Exec(`UPDATE workgraph_nodes SET updated_at = ? WHERE instance_id = ? AND node_id = ?`,
