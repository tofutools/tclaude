...
// migrateV140toV141 adds sessions.bg_shells_json — the per-session ledger
...
func migrateV140toV141(db *sql.DB) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("migrate v140→v141 (bg shells): begin: %w", err)
	}
	...
	if err := tx.QueryRow(
		...
		return fmt.Errorf("migrate v140→v141 (probe sessions): %w", err)
	}
	if haveTable > 0 {
		var haveColumn int
		if err := tx.QueryRow(
			...
			return fmt.Errorf("migrate v140→v141 (probe sessions.bg_shells_json): %w", err)
		}
		if haveColumn == 0 {
			if _, err := tx.Exec(
				`ALTER TABLE sessions ADD COLUMN bg_shells_json TEXT NOT NULL DEFAULT ''`,
			); err != nil {
				return fmt.Errorf("migrate v140→v141 (add sessions.bg_shells_json): %w", err)
			}
		}
	}

	if _, err := tx.Exec(`UPDATE schema_version SET version = 141`); err != nil {
		return fmt.Errorf("migrate v140→v141 (version): %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("migrate v140→v141 (commit): %w", err)
	}
	return nil
}
