...
func TestAWBReadyPollingNormalizesWorkspaceAndRoundTrips(t *testing.T) {
	var cfg Config
	require.NoError(t, json.Unmarshal([]byte(`{"agent":{"awb_proxy":{"url":"https://awb.example/","username":"worker","allow_write":true,"allowed_workspaces":["TCL"],"ready_polling":{" Builders ":{"workspace":" TCL ","labels":["backend","urgent"],"group":"builders","cwd":"/repo","worktree":true,"monitor_commit":true}}}}}`), &cfg))
	resolved := cfg.ResolvedAWBProxy()
	...
	assert.True(t, resolved.ReadyPolling["builders"].MonitorCommit)
