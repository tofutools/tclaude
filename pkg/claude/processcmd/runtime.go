...
func runProcessRunsLs(p *processRunsLsParams, stdout, stderr io.Writer) error {
	...
	tw := newTable(stdout)
	fmt.Fprintln(tw, "ID\tTEMPLATE\tSTATUS\tACTION\tUPDATED")
	for _, run := range response.Runs {
		fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\n", run.ID, run.TemplateRef, run.Status, run.Action, formatTime(run.UpdatedAt))
	}
...
func processShowCmd() *cobra.Command {
	return processRunReadCmd("show <run-id>", "Show one daemon-owned process run", false)
}

func processReconcileCmd() *cobra.Command {
	return processRunReadCmd("reconcile <run-id>", "Inspect whether a run needs explicit reconciliation", true)
}
...
func processResumeCmd() *cobra.Command {
	return processRunMutationCmd("resume <run-id>", "Resume or tick one runnable process run", "/resume")
}

func processReissueCmd() *cobra.Command {
	return processRunMutationCmd("reissue <run-id>", "Explicitly reissue a cold outstanding program command", "/reissue")
}
