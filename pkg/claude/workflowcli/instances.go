...
// /v1/workflows* surface (owned by the agentd side). Every daemon call goes
...
// --- wire shapes (decoded from the frozen /v1/workflows* responses) ---

// wfInstanceRow is one row of GET /v1/workflows {"instances":[...]} — the
// snapshot the daemon builds from collectWorkflowsSnapshot.
...
// wfNode mirrors agentd's workflowNodeJSON.
type wfNode struct {
...
// wfEvent mirrors agentd's workflowEventJSON.
type wfEvent struct {
	ID      int64  `json:"id"`
...
// wfDetail is GET /v1/workflows/{id}.
...
// agent.MapDaemonErrorToRC for the workflow surface: the /v1/workflows*
...
func lsCmd() *cobra.Command {
	return boa.CmdT[lsParams]{
		Use:         "ls",
		Short:       "List workflow instances (and discoverable templates)",
...
func runLs(p *lsParams, stdout, stderr io.Writer) int {
	...
	if err := daemonGet("/v1/workflows", &resp); err != nil {
		fmt.Fprintf(stderr, "Error: %v\n", err)
		return rcForDaemonErr(err)
	}
	templates := workflow.List(projectDirs()...)
...
func renderInstanceRows(rows []wfInstanceRow, w io.Writer) {
	if len(rows) == 0 {
		fmt.Fprintln(w, "  (no workflow instances)")
		return
	}
	tbl := table.New(
...
func statusCmd() *cobra.Command {
	return boa.CmdT[statusParams]{
		Use:         "status",
		Short:       "Show a workflow instance: per-node status, outcomes, vars, recent events",
		ParamEnrich: common.DefaultParamEnricher(),
...
func runStatus(p *statusParams, stdout, stderr io.Writer) int {
	...
	var d wfDetail
	if err := daemonGet("/v1/workflows/"+strconv.FormatInt(id, 10), &d); err != nil {
		fmt.Fprintf(stderr, "Error: %v\n", err)
		return rcForDaemonErr(err)
	}
	if p.JSON {
...
func renderDetail(d *wfDetail, w io.Writer) {
	...
	if len(d.Events) > 0 {
		...
		if len(d.Events) > recent {
			fmt.Fprintf(w, "\nrecent events (last %d of %d — `workflow events %d` for the full timeline):\n",
...
func eventsCmd() *cobra.Command {
	return boa.CmdT[eventsParams]{
		Use:         "events",
		Short:       "Show a workflow instance's audit timeline (optionally one node's)",
		ParamEnrich: common.DefaultParamEnricher(),
...
func runEvents(p *eventsParams, stdout, stderr io.Writer) int {
	...
	path := "/v1/workflows/" + strconv.FormatInt(id, 10) + "/events"
...
func whereCmd() *cobra.Command {
	return boa.CmdT[whereParams]{
		Use:         "where",
		Short:       "Show which workflow node(s) you (the caller) are assigned to",
		ParamEnrich: common.DefaultParamEnricher(),
...
func runWhere(p *whereParams, stdout, stderr io.Writer) int {
	...
	if resp.Caller == "" {
		fmt.Fprintln(stdout, "(no caller identity — workflow assignments are per-agent; humans use `workflow ls`)")
		return rcOK
	}
	if len(assignments) == 0 {
		...
		fmt.Fprintf(stdout, "(you are not assigned to any live workflow node)%s\n", hint)
		return rcOK
	}
	for _, a := range assignments {
