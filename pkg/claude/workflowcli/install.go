...
// install resolves an external workflow source (a dir: path or a git: repo) and
// copies it INTO the user templates dir (~/.tclaude/workflows/<name>), so it
// becomes a stable, offline, locally-editable `user:<name>` template.
//
// The fetch is entirely the workflow package's job — workflow.Resolve loads a
...
func installCmd() *cobra.Command {
	return boa.CmdT[installParams]{
		Use:   "install",
		Short: "Install an external template (dir:/git:) into the user workflows dir",
		Long: "Resolve an external workflow source and copy it into ~/.tclaude/workflows,\n" +
			"making it a stable, offline `user:<name>` template.\n\n" +
			"  tclaude workflow install dir:./my-template\n" +
			"  tclaude workflow install git:https://example.com/repo#workflows/foo --name foo\n\n" +
			"The git fetch/cache is the workflow resolver's job; this verb only copies in.",
...
func runInstall(p *installParams, stdout, stderr io.Writer) int {
	...
	tmpl, err := workflow.Resolve(src, projectDirs()...)
	if err != nil {
		// workflow.Resolve returns untyped errors, so a malformed external source
		...
		// would have to live in the workflow package.
	...
	ud := workflow.UserDir()
	if ud == "" {
		fmt.Fprintln(stderr, "Error: cannot determine the user workflows dir (no home directory)")
		return rcIOFailure
	}
	...
	installedRef := string(workflow.SourceUser) + ":" + name
	if p.JSON {
...
func normalizeInstallSrc(src string) string {
	if pre, _, found := strings.Cut(src, ":"); found {
		switch workflow.Source(pre) {
		case workflow.SourceDir, workflow.SourceGit,
			workflow.SourceProject, workflow.SourceUser, workflow.SourceExample:
			return src
		}
	}
	return string(workflow.SourceDir) + ":" + src
}
