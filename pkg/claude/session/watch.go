	...
	case tea.KeyPressMsg:
		...
		if m.newSessionMode {
			...
			case "enter":
				...
				m.labelInput.Blur()
				m.nameInput.Blur()
				m.newSessionDir = clcommon.ExpandHomePrefix(strings.TrimSpace(m.dirInput.Value()))
			...
			case "tab":
				if m.newSessionField == 0 {
					completed, candidates := clcommon.CompleteDirPath(m.dirInput.Value())
...
func (m model) orderedColumns() []sessionColDef {
	return []sessionColDef{
		...
		{key: sessionColHarness, visible: m.colVisible(sessionColHarness, m.hasNonDefaultHarness()),
			col:  table.Column{Header: "HARNESS", Width: 10, SortKey: "harness"},
