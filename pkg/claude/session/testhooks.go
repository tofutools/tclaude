...
// SetMigrateAgentIdentityForTest swaps the package-private function
// variable that fronts db.MigrateAgentIdentity so flow tests can
// inject a transient failure and assert the post-/clear retry path:
// a failed migration must leave the session row on the old conv-id
// and the predicate must stay true, then the next hook converges.
// Returns a restore closure intended for t.Cleanup.
//
...
func SetMigrateAgentIdentityForTest(fn func(oldConv, newConv, reason, granter string) (db.AgentIdentityMigration, error)) func() {
	prev := migrateAgentIdentity
	migrateAgentIdentity = fn
	return func() { migrateAgentIdentity = prev }
}
...
// SetMigrateAgentIdentityForTest — lives in a regular .go file so
