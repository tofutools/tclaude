...
func TestCodexNativeRegistryAcceptsProfileGeneratedBeforeSocketMove(t *testing.T) {
	opts := nativeRegistryFixture(t)
	oldName := "tclaude-agent-0000000000000031"
	oldProfile := nativeProfile(oldName, "/workspace/old")
	oldProfile = strings.ReplaceAll(oldProfile,
		agentipc.CanonicalSocketPath(), agentipc.LegacyAPISocketPath())
	oldProfile = strings.Replace(oldProfile,
		fmt.Sprintf("%q = \"read\"\n", agentipc.CanonicalSocketDir()), "", 1)
	require.NoError(t, db.UpsertCodexNativePermissionProfile(db.CodexNativePermissionProfile{
		Generation: "launch:old", ProfileName: oldName, ProfileTOML: oldProfile,
	}))

	newName := "tclaude-agent-0000000000000032"
	require.NoError(t, registerCodexNativePermissionProfile(opts, "launch:new", newName,
		nativeProfile(newName, "/workspace/new")),
		"a profile generated by the previous tclaude version must not block a new launch")

	config, err := os.ReadFile(filepath.Join(opts.ManagedDir, "config.toml"))
	require.NoError(t, err)
	assert.Contains(t, string(config), "[permissions."+oldName+"]")
	assert.Contains(t, string(config), "[permissions."+newName+"]")
}
