...
func TestWaitForClearedIdentityIndexUsesMetadataOnlyOrdering(t *testing.T) {
	...
	go func() {
		...
		updated <- db.UpsertConvIndex(&db.ConvIndexRow{
			...
			FileMtime: info.ModTime().Round(0).UTC(), CustomTitle: "fresh", IndexedAt: time.Now(),
		})
	}()
...
func TestRunHookCallback_SessionStartEnrollsLaunchedConv(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, actor)
	assert.True(t, strings.HasPrefix(actor.PendingName, "20260728-1217-"), actor.PendingName)
