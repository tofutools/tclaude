...
// TestCopilotPermissionWebFetchDenyURLSurvivesInPaneAllowAll answers whether
// the blanket URL deny this file discovered is a DURABLE boundary or merely a
// starting posture anything typed into the pane can widen.
//
// The distinction decides what tclaude may honestly say about the flag rather
// than being a curiosity. A pane's permission posture cannot be re-verified
// from outside — it lives inside the session — so a deny that /allow-all could
// lift would be a starting posture wearing a boundary's name, and docs would
// have to stop describing it as one.
//
// It is a genuinely separate question from the one
// TestCopilotPermissionInPaneAllowAllCannotOverrideDeny already settled, which
// is why it is measured rather than inferred. That scenario established runtime
// denial precedence for the SHELL kind, using `shell(echo)`. This is the URL
// kind, reached through a different tool, and this file has already found the
// two axes behaving differently from one another more than once — the wildcard
// spellings enforce for neither, the bare kind enforces for both, and the
// prompt itself has different chrome. Generalizing across them is exactly the
// move the rest of this suite refuses to make.
//
// The measured answer is the good one: the deny survives.
func TestCopilotPermissionWebFetchDenyURLSurvivesInPaneAllowAll(t *testing.T) {
	requireSmoke(t)
	...
	mock := copilotfixture.NewMockProvider(t, []copilotfixture.Turn{
		...
		{Text: "MOCK READY"},
		// Answers the request typed after /allow-all.
		{ToolCall: &copilotfixture.ToolCall{
			ID:   "call_copilotfixture_webfetch_inpane",
			Name: webFetchToolName,
			Args: `{"url":"` + webFetchProbeURL + `"}`,
		}},
		{Text: "MOCK DONE"},
	...
	res := copilotfixture.RunPTY(t, copilotfixture.PTYOptions{
		RunOptions: copilotfixture.RunOptions{
			...
			OmitAllowAllTools: true,
			// Paired deliberately: the deny must beat a blanket allow, not
			// merely fill a gap the allow left open.
			ExtraArgs: []string{"--allow-all-tools", "--no-ask-user", "--deny-tool", "url"},
		},
		// Typed into a settled screen, in order: widen the posture, then ask for
		// the denied capability.
		Input:       []string{"/allow-all", "Now fetch the URL."},
		Deadline:    3 * webFetchDeadline,
		SettledWhen: func() bool { return len(mock.Requests()) >= 3 },
	})

	// Without this the test could pass because /allow-all never reached the
	// pane, proving nothing about whether it can override a deny.
	...
	assert.NotEmpty(t, marker,
		"the launch-time blanket URL deny must still refuse web_fetch AFTER /allow-all "+
			"widened the posture. If this ever passes, --deny-tool url is startup posture "+
			"only and nothing may describe it as a boundary")
	assert.Empty(t, capture.Hosts(),
		"a surviving deny must still stop the call before name resolution, so a widened "+
			"pane reaches no network either")
	t.Logf("permission verdict: blanket URL deny survives in-pane /allow-all (%q)", marker)
	assertCredentialFree(t, mock)
}
