...
func templatesCmd() *cobra.Command {
	return boa.CmdT[templatesParams]{
		Use:         "templates",
		Short:       "List discoverable workgraph templates (project / user / example)",
...
func runTemplates(p *templatesParams, stdout, stderr io.Writer) int {
	entries := workgraph.List(projectDirs()...)
	if entries == nil {
		entries = []workgraph.ListEntry{} // emit [] not null for the empty case
	}
	if p.JSON {
...
func renderTemplateList(entries []workgraph.ListEntry, w io.Writer) {
	if len(entries) == 0 {
		fmt.Fprintln(w, "(no workgraph templates found in project, user, or example sources)")
		return
	}
	tbl := table.New(
...
func runShow(p *showParams, stdout, stderr io.Writer) int {
	...
	tmpl, err := workgraph.Resolve(p.Ref, projectDirs()...)
	if err != nil {
...
// daemon verbs.
//
// It is deliberately a touch stricter than the workgraph package's validRefName
...
// validation (including subpath-traversal guards) is workgraph.Resolve's job.
func refLooksInvalid(ref string) bool {
	name := ref
	if src, rest, found := strings.Cut(ref, ":"); found {
		s := workgraph.Source(src)
		switch {
		case s.IsExternal():
			return false // dir:/git: — defer entirely to workgraph.Resolve
		case s == workgraph.SourceProject || s == workgraph.SourceUser || s == workgraph.SourceExample:
			name = rest
		}
	}
...
func templateToJSON(t *workgraph.Template) showJSON {
...
func renderTemplate(t *workgraph.Template, w io.Writer) {
	fmt.Fprintf(w, "%s\n", t.Ref)
...
func sortedNodeIDs(t *workgraph.Template) []string {
