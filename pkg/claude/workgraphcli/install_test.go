...
// writeTemplateFixture lays down a minimal valid template dir (workgraph.yaml +
...
// installHarness sets HOME to a temp dir (so UserDir() → <tmp>/.tclaude/workgraphs)
...
func installHarness(t *testing.T, tmplName string) (string, string) {
	...
	return src, filepath.Join(home, ".tclaude", "workgraphs")
}
...
func TestRunInstall_DirSourceRoundTrips(t *testing.T) {
	...
	for _, rel := range []string{"workgraph.yaml", "flow.mmd", "nodes/work.yaml", "nodes/done.yaml"} {
	...
	tmpl, err := workgraph.Resolve("user:myflow")
	if err != nil {
...
func TestRunInstall_NameOverride(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(userDir, "renamed", "workgraph.yaml")); err != nil {
...
func TestRunInstall_BarePathNormalizedToDir(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(userDir, "bareflow", "workgraph.yaml")); err != nil {
...
// A --name that tries to escape the user workgraphs dir must be rejected by
// validInstallName before any copy, so it can't write outside ~/.tclaude/workgraphs.
func TestRunInstall_NameTraversalRejected(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(filepath.Dir(userDir), "escape")); err == nil {
		t.Error("a traversing --name wrote outside the user workgraphs dir")
	}
}
