...
func TestRunShow_JSON(t *testing.T) {
	...
	if got.Source != string(workgraph.SourceExample) {
		t.Errorf("Source = %q, want %q", got.Source, workgraph.SourceExample)
	}
	if len(got.Nodes) != 6 {
...
// workgraph.Resolve (which here can't find the template → rcNotFound). Guards the
...
func TestRunShow_ExternalRefNotPreRejected(t *testing.T) {
	var out, errBuf bytes.Buffer
	rc := runShow(&showParams{Ref: "dir:/no/such/workgraph/template/dir"}, &out, &errBuf)
	if rc == rcInvalidArg {
		t.Fatalf("dir: ref was pre-rejected as rcInvalidArg; external refs must defer to workgraph.Resolve (got rc=%d)", rc)
	}
}
...
func TestRenderTemplateList_FlagsErrorsAndWarnings(t *testing.T) {
	...
	renderTemplateList([]workgraph.ListEntry{
		{Ref: "user:broken", Name: "broken", Source: workgraph.SourceUser, Err: "bad yaml"},
		{Ref: "user:smelly", Name: "smelly", Source: workgraph.SourceUser, NodeCount: 3, Warnings: []string{"loop"}, Description: "smelly"},
	}, &buf)
	s := buf.String()
...
func TestRunTemplates_JSONIncludesExample(t *testing.T) {
	...
	var entries []workgraph.ListEntry
