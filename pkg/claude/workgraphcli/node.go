...
// daemon's mutating /v1/workgraphs* endpoints. The node settle path is the
...
// PATCH /v1/workgraphs/{id}/nodes/{node} {status,outcome?,output?} →
...
func nodeCmd() *cobra.Command {
	return boa.CmdT[nodeParams]{
		...
		Long: "Drive one node of a workgraph instance.\n\n" +
			...
			"settles; to abandon a whole instance use `tclaude workgraph cancel`.",
...
func runNode(p *nodeParams, stdout, stderr io.Writer) int {
	...
	path := "/v1/workgraphs/" + strconv.FormatInt(id, 10) + "/nodes/" + node
...
// spawnNodeParams drives `tclaude workgraph spawn` — the agent-engine driver's
// spawn-worker-into-node verb (JOH-15 B2a), mirroring POST
// /v1/workgraphs/{id}/nodes/{node}/start. Distinct from `node <inst> <node> start`,
...
func spawnCmd() *cobra.Command {
	return boa.CmdT[spawnNodeParams]{
		...
		Long: "Spawn a fresh worker agent into a ready ai node of a workgraph instance,\n" +
			...
			"worker also self-orients via `workgraph where` / `workgraph status`.\n\n" +
			"This differs from `workgraph node <inst> <node> start`, which only flips a\n" +
...
func runSpawn(p *spawnNodeParams, stdout, stderr io.Writer) int {
	...
	path := "/v1/workgraphs/" + strconv.FormatInt(id, 10) + "/nodes/" + node + "/start"
...
type driveParams struct {
	Instance string `pos:"true" help:"Workgraph instance id (must be an engine:agent instance)"`
	JSON     bool   `long:"json" help:"Output JSON"`
}
...
func driveCmd() *cobra.Command {
	return boa.CmdT[driveParams]{
		...
		Long: "Spawn a fresh driver agent into the instance's bound group, grant it\n" +
			...
			"`workgraph-engine` skill against this instance — reflect, spawn workers,\n" +
...
func runDrive(p *driveParams, stdout, stderr io.Writer) int {
	...
	var resp driveResp
	path := "/v1/workgraphs/" + strconv.FormatInt(id, 10) + "/drive"
...
func newCmd() *cobra.Command {
	return boa.CmdT[newParams]{
		Use:         "new",
		Short:       "Instantiate a template into a running workgraph instance",
		ParamEnrich: common.DefaultParamEnricher(),
...
func runNew(p *newParams, stdout, stderr io.Writer) int {
	...
	if err := agent.DaemonRequest(http.MethodPost, "/v1/workgraphs", body, &resp, agent.DaemonOpts{}); err != nil {
		fmt.Fprintf(stderr, "Error: %v\n", err)
		return rcForDaemonErr(err)
	}
	if p.JSON {
...
func runCancel(p *cancelParams, stdout, stderr io.Writer) int {
	...
	path := "/v1/workgraphs/" + strconv.FormatInt(id, 10) + "/cancel"
	if err := agent.DaemonRequest(http.MethodPost, path, nil, &resp, agent.DaemonOpts{}); err != nil {
		fmt.Fprintf(stderr, "Error: %v\n", err)
		return rcForDaemonErr(err)
	}
...
func runRm(p *rmParams, stdout, stderr io.Writer) int {
	...
	path := "/v1/workgraphs/" + strconv.FormatInt(id, 10)
